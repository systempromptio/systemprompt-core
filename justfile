...
check-bans-crate CRATE:
    ...
    matches=$(grep -RIn --include='*.rs' -E 'sqlx::query\s*\(' "$crate_dir/src" 2>/dev/null \
        ...
        | grep -vE 'crates/entry/cli/src/commands/admin/setup/' \
...
check-bans:
    ...
    echo "==> raw sqlx::query() outside allowlist (admin/, postgres/{mod,introspection,query_executor,transaction,ext,conversion}.rs, cli admin/setup/, tests)"
