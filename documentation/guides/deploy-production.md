...
- **One identity, generated once.** Run `systemprompt admin identity generate --json` on an operator machine and distribute the four values (`oauth_at_rest_pepper`, `manifest_signing_secret_seed`, `encryption_master_key`, `signing_key_pem`) to every replica through the secrets store. The binary never mints them at boot: every boot without the seed or the master key, and a cloud or deployment-host boot without the signing key, fails with an error naming the command. A node with its own seed or key issues tokens and manifests the next node rejects.
