...
| `accepted_betas` | list<string> | no | all on first-party; none on Vertex AI | The `anthropic-beta` values forwarded from a client request. Vertex AI rejects a beta it does not support, so a Vertex provider forwards only the betas listed here. |
