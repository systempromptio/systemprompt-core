...
This document defines what is stable in systemprompt.io and what is not. It is the answer to "you're on 0.47.x — is this safe to build against?"

## Current Version

`0.47.x` across the workspace. See root `Cargo.toml` for the exact current version.
...
2. **Across minors (e.g. `0.44.x` → `0.47.x`):** breaking changes are possible only on the Stable Surface with a `BREAKING` entry in `CHANGELOG.md`, migration notes, and a deprecation window of at least one prior minor where both forms were accepted. Database migrations between minors are forward-compatible by design; rollback to the prior minor is supported.
