...
| JWT signing key (RS256) | Annual, or on suspicion of compromise | Mint a new RSA keypair with `systemprompt admin keys generate`. The in-process `TokenAuthority` loads the active key from `profile.security.signing_key_path`; new tokens carry the new `kid` and are verified against the public set republished at `/.well-known/jwks.json`. Outstanding tokens signed under the previous `kid` continue to validate until their natural expiry (the JWKS retains the prior public key until cutover completes) — no maintenance window required for the rotation itself. |
| OAuth at-rest pepper (`oauth_at_rest_pepper`) | Annual, or on suspicion of compromise | The pepper is the HMAC key under which refresh-token ids and authorisation codes are stored as HMAC-SHA-256 digests. Rotating it invalidates every outstanding refresh token and pending authorisation code; plan a maintenance window and force all clients to re-authenticate, or stagger by issuing short-lived tokens before the cutover. |
| Trusted-issuer JWKS (federated subjects) | Tracked by the issuer, not this deployment | RFC 8693 subject tokens are verified against the JWKS document served at each `profile.security.trusted_issuers[*].jwks_uri`. The JWKS HTTP client refreshes entries on a bounded LRU; no local key material is held. |
