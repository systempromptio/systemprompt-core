...
Cowork is a single Rust binary with `#[cfg(target_os = "...")]` gates wherever the OSes diverge. Native code paths live under `bin/cowork/src/integration/claude_desktop/{macos,windows,shared}.rs` and `bin/cowork/src/install/{bootstrap,mdm}.rs`. The GUI lives under `bin/cowork/src/gui/` and is gated `#[cfg(any(windows, macos))]`.
...
- Plugin mount path follows XDG conventions; consult `bin/cowork/README.md` for the exact directory.
- Built and signed by `release-sign.yml` on `ubuntu-latest`.

## macOS (`aarch64-apple-darwin`, `x86_64-apple-darwin`)

- **GUI**: tray icon + native settings window via `WKWebView`. Code under `bin/cowork/src/gui/`.
...
- **`.app` bundle**: produced by `bin/cowork/scripts/make-mac-app.sh`. `make-icons.swift` generates `AppIcon.icns` from the source PNGs. Wrapped binary lives at `Contents/MacOS/systemprompt-cowork` with `Info.plist`.
- **Tag tracks**:
...
- **GUI**: tray icon + native settings window via `WebView2`. Code under `bin/cowork/src/gui/`.
...
- **Process discovery**: `tasklist /FO CSV` and `GetConsoleProcessList` via `bin/cowork/src/winproc.rs`. `winproc` also handles console attachment so the credential-helper mode prints to the parent console when invoked from a terminal.
- **PE metadata + icon**: `bin/cowork/build.rs` runs only on Windows, uses `winresource` to embed `bin/cowork/assets/app-icon.ico` and set `FileDescription` / `ProductName` / `CompanyName` in the EXE header.
...
5. If the feature requires a new Windows asset (icon variant, manifest), add it under `bin/cowork/assets/` and reference it from `build.rs`.
6. CI: the existing 3-OS matrix in `release-sign.yml` will pick the new code up automatically. Add a smoke test under `bin/cowork/tests/` if behavior is observable from the binary.
