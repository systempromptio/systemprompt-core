...
|---|---|---|---|
| `bridge-vX.Y.Z` | `systemprompt-core` | `release-sign.yml` | bridge binary, 3-OS, signed |
| `bridge-mac-vX.Y.Z` | `systemprompt-core` | dedicated workflow | bridge macOS x86_64 only (Intel) |
...
| `bridge-vX.Y.Z` | `systemprompt-template` | `release.yml` | bridge end-user bundle (re-attaches core's signed binaries to a branded template release) |
...
Why it's deprecated: bare `v*` triggered bridge in core but gateway in deploy — same string, different product. Backporting or scripted release tooling could ship the wrong product by tagging the wrong repo. Prefixed tags eliminate the ambiguity.
...
- **Gateway** versions independently of bridge. Tagged `gateway-v*` in deploy.
...
- Bridge can ship without a gateway release. It often does.
- Gateway can ship without a bridge release.
...
- macOS x86_64 bridge lags the main 3-OS matrix. Re-tag `bridge-mac-vX.Y.Z` only when an Intel-Mac-specific change merits it.

## Pre-release / RC

There is **no formal RC tag policy** wired up today. `bridge-vX.Y.Z-rc.N`, `gateway-vX.Y.Z-rc.N` etc. would not currently match any workflow trigger and are not published. If you need a pre-release, use `workflow_dispatch` with a non-monotonic tag and treat the resulting GitHub Release as draft.
...
| Do | Don't |
|---|---|
| `git push origin bridge-vX.Y.Z` | `git push --tags` (drags `<crate>@<ver>` cruft) |
| `git push origin gateway-vX.Y.Z` | Push a bare `vX.Y.Z` tag (legacy; will silently stop triggering after the deprecation cycle) |
| `git tag -l '*@*' \| xargs -r git tag -d` to prune local clutter | Tag in the wrong repo (bridge in deploy, gateway in core) — though prefixes now make this a no-op rather than a footgun |
