...
|---|---|---|---|---|
| `systemprompt-core` | public | Rust workspace (30 `systemprompt-*` crates), `bin/bridge/` binary source, signed bridge binary releases | `bridge-v*` (build+sign), `bridge-mac-v*` (macOS x86_64 separate track), `<crate>@<ver>` (cargo-ws cruft, never pushed) | crates.io, `systempromptio/systemprompt-core` GH Releases |
...
| `systemprompt-template` | public | Fork-and-compile evaluation template; consumer-facing GH Releases for both products | `bridge-v*` (re-bundles only); receives `gateway-v*` as an upload from deploy | `systempromptio/systemprompt-template` GH Releases |
...
- **template** is what end users fork. It carries the application skeleton (`src/`, `extensions/`, `migrations/`, `services/`, `web/`, demos, install docs) and acts as the public Release surface for both gateway and bridge. Users read its README and follow `docs/install/*` and `docs/bridge/*`.
...
| Release matrix | main 3-OS (`v*`) | main 3-OS (`v*`) | **separate** `bridge-mac-v*` tag | main 3-OS (`v*`) |
...
| `justfile` (root, recipes `build-bridge`, `build-bridge-all`, `bundle-bridge-mac`) | Authoritative local-build commands. CI mirrors these. |
...
| `../systemprompt-template/.github/workflows/release.yml` | Bridge re-bundle on `bridge-v*`. |
