# Bridge build, release & versioning

`systemprompt-bridge` is a standalone Rust binary shipped from `bin/bridge/`. It plays two roles in one process: a **credential helper** (emits Anthropic's `inferenceCredentialHelper` JSON to stdout) and a **plugin / MCP sync agent** (pulls signed manifests from a `systemprompt-gateway` into the user's `org-plugins/` directory). On macOS and Windows it also runs as a tray app with a native settings GUI; Linux is CLI-only.

It ships across three OS targets: `x86_64-unknown-linux-gnu`, `aarch64-apple-darwin`, and `x86_64-pc-windows-msvc`. macOS Intel (`x86_64-apple-darwin`) is built separately and tracked under `bridge-mac-v*` tags rather than the main 3-OS matrix (the `macos-13` runner queue made the merged matrix unreliable).
...
systemprompt-core (public, source)        systemprompt-deploy (private, CI)
        │                                          │
        │ tag bridge-v*                            │ tag gateway-v*
        ▼                                          ▼
.github/workflows/release-sign.yml        .github/workflows/release.yml
3-OS bridge build + cosign keyless sign   gateway 4-arch build + deb/rpm + cosign
        │                                          │
        ▼                                          ▼
GH Releases on systemprompt-core          GH Releases on systemprompt-template
(bridge binaries + SHA256SUMS)            (gateway tarballs + SHA256SUMS.gateway)
        │                                          ▲
        │                                          │
        │ ┌────────────────────────────────────────┘
        ▼ │
...
downloads bridge artifacts from systemprompt-core's GH Release,
renames SHA256SUMS → SHA256SUMS.bridge, re-attaches to a template release
```
...
|---|---|---|
| `bridge-v*` | core (build+sign), template (re-bundle) | bridge binary |
| `bridge-mac-v*` | core | bridge macOS Intel |
...
Bridge and gateway version independently. Template's `bridge-v*` tag exists to give end users a single, branded release to download from — the binaries themselves are produced by core.
