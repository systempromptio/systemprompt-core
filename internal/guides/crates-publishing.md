...
**Current Version:** `0.11.2` (May 25, 2026)
...
| `systemprompt-marketplace` | [crates.io](https://crates.io/crates/systemprompt-marketplace) | [docs.rs](https://docs.rs/systemprompt-marketplace) |
...
# For all crates:
for crate in systemprompt-{identifiers,provider-contracts,traits,extension,models,client,template-provider,database,logging,events,security,loader,config,cloud,analytics,users,files,templates,content,marketplace,ai,runtime,scheduler,oauth,mcp,agent,generator,sync,api,cli} systemprompt; do
...
5. **`scripts/sync-workspace-deps.sh`** — rewrites every `systemprompt-*` pin in `[workspace.dependencies]` to the new `[workspace.package].version`. This catches the pins cargo-ws silently skips (see gotcha below).
...
| 20 | `systemprompt-marketplace` | Domain |
| 21 | `systemprompt-ai` | Domain |
| 22 | `systemprompt-runtime` | App |
| 23 | `systemprompt-scheduler` | App |
| 24 | `systemprompt-oauth` | Domain |
| 25 | `systemprompt-mcp` | Domain |
| 26 | `systemprompt-agent` | Domain |
| 27 | `systemprompt-generator` | App |
| 28 | `systemprompt-sync` | App |
| 29 | `systemprompt-api` | Entry |
| 30 | `systemprompt-cli` | Entry |
| 31 | `systemprompt` | Facade |
