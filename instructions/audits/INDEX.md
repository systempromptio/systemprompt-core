...
> **Note (post-Wave-E):** The `just check-bans` and `just check-bans-crate` recipes
> referenced throughout these dated reports were removed after the sweep
> completed. The rules they encoded (raw `String` IDs, `*Manager` names,
> sqlx allowlist) remain reviewer-enforced conventions per `CLAUDE.md`;
> historical `✅ check-bans-crate` lines below are point-in-time evidence and
