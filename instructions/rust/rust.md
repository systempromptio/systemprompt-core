...
| `and_then(...).ok()` chains | Preserve error context, don't convert to Option |
| `match { Err(_) => default }` | Propagate error or log with `tracing::error!` |
| `filter_map(\|e\| e.ok())` | Log failures before filtering |
| Error log then `Ok()` | Propagate the error after logging, never swallow |
| `unwrap_or()` hiding failures | Return `Err` or log explicitly before fallback |

**Acceptable `.ok()` usage** (only these two cases):
...
2. **Transaction rollback** - log but don't mask original error:
```rust
...
**Correct pattern** - log before converting to Option:
```rust
serde_json::from_str(s).map_err(|e| {
    tracing::warn!(error = %e, raw = %s, "Parse failed");
    e
}).ok()
```
