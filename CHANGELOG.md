...
- **Bridge (Windows):** the elevated policy writer (`bin/bridge/src/install/policy_writer/`). An elevated `install --apply` that holds a pinned gateway key (`--pubkey`, or the operator pin now carried up into the machine `manifestTrust` anchor) copies the binary to `%ProgramData%\<brand>\policy-writer\bin` under an administrator-only DACL and registers the Task Scheduler task `<Brand>BridgePolicyWriter` (principal SYSTEM, no triggers, `HighestAvailable`; descriptor lets authenticated users run it and only administrators change it — registered through the Task Scheduler COM API, since `schtasks` cannot set one). A sync or a Claude Desktop update that would otherwise need administrator approval writes a request (`PolicyWriteRequest`: the last verified manifest envelope verbatim, the loopback port and host token, the inference facts, the tool-name catalog) into the writer's inbox, runs the task and waits for the result. The writer (`__apply-policy-task`) believes nothing in the request until the envelope verifies against the machine anchor — machine hive only, never the user's config — and derives `managedMcpServers` from the manifest it verified, so a user cannot add a server, change a tool policy from the manifest, or point the task at another binary; requests are symlink-refused and size-capped, results are written for their requester alone. The unelevated side re-reads the hive against the same derivation before reporting success. No anchor, no task: the install says so and connector changes fall back to the approval prompt; `sync` records a `policy_writer` host warning when a registered writer fails and falls back. `doctor` reports the writer's state. The verified envelope is kept beside the registry (`manifest-envelope.json`) by every sync and refresh.
...
- **Gateway:** `GET /v1/bridge/manifest` honours `Cache-Control: no-cache` — the per-user resolved-catalogue memo (60 s) is bypassed and the rebuild stored. The memo's key (disk fingerprint, user, managed-tables stamp) cannot see a connector the user has just linked, so a sync pressed right after linking one answered with the server set from before it. `bridge_resolved::resolve_for_user` takes a `Freshness`.
- **Bridge:** a sync the user presses, `sync --fresh`, and every Claude Desktop generate/Update fetch the manifest with `no-cache`; the scheduled tick and the login sync keep the memo. Generate no longer reuses the registry the last sync published: `sync::refresh_registry` fetches and verifies a fresh manifest and republishes the managed servers (registry + `mcp-servers.json`) before a profile that carries them (`HostApp::profile_carries_managed_servers`, Claude Desktop) is written, so an Update pressed within a minute of connecting an account writes the new server set, not the old one.
- **Bridge:** the Claude Desktop probe compares the installed `managedMcpServers` with the servers the registry would write (`ProfileState::managed_servers_freshness`; `StaleReason::ManagedServers`, `ProbeEnv::expected_managed_servers`, unknown until a sync has published the registry so a launch before the first sync never reads a good policy as stale). A profile behind the gateway verdicts as **Update** (`AgentAction::Update`, or `UpdateAdmin` when the write will raise the administrator prompt — `HostApp::update_needs_approval`, HKLM on Windows, managed preferences on macOS); Repair stays for a stale credential or port. A sync whose only failures need an administrator reads **"Claude Desktop needs approval"** (`OverallCode::NeedsApproval`) instead of "synced with failures"; the Setup health row, the toast and the drawer all offer "Update as administrator". After a Claude Desktop install the bridge reads the policy back and refuses to report success while the installed server list still differs; the success toast says to quit and relaunch the app. `doctor` warns on a server list behind the gateway.
