...
- **Expired-JWT integration test enforces 401 only.** `test_expired_jwt_token_rejected` previously accepted `200` as a passing status under the assumption that local profiles might run with token validation disabled. That branch is gone; an expired token must return 401, full stop.
