...
- **Analytics:** the feedback fact/snapshot engine is removed — `systemprompt_analytics::{feedback, snapshots}`, `AppContext::{feedback_facts_repository, feedback_snapshots_repository, snapshot_wakeup}`, `runtime::reporting::SnapshotWakeup`, `systemprompt_models::feedback::analytics` and the `Analytics{Change,Fact,SnapshotJob,Worker}Id` identifiers. Migration `analytics/015` drops its fifteen tables and two views. A consumer still calling `FeedbackSnapshotsRepository::compact_all_in` or `FeedbackFactsRepository::submit_in`, or reading `analytics_normalized_facts`, must drop those uses.
- **Analytics:** the reporting projection is retired. The statement capture triggers on nine source tables, the `analytics_reporting` outbox consumer, the 1 s projection worker, the boot baseline rebuild, the `analytics_report_*` tables and the reporting privacy routines are gone (migrations `analytics/015`, `events/009`, `ai/037`, `agent/014`, `mcp/017`, `content/005`, `logging/009`, `users/020`). `systemprompt analytics …` reads analytics-owned `report_*` views over the source tables, which hide every row of a user whose status is `deleted`; `systemprompt analytics projection` is removed. The `message_count` triggers on `ai_request_messages` stay.
- **Users SQL:** `public.expire_reporting_sessions(timestamptz)` is replaced by `public.expire_user_sessions(timestamptz)`; `begin_user_privacy`, `finish_user_privacy`, `lock_user_deletion_for_retention` and `reporting_user_is_retained` are dropped. A retention routine outside core that called `expire_reporting_sessions` must call `expire_user_sessions`.
- **Analytics Rust API:** dead surface with no caller in core or any known consumer is removed: `CoreStatsRepository`, `FunnelRepository` (and the `funnels`/`funnel_steps`/`funnel_progress` tables, `FunnelId`, `FunnelProgressId`), `AnalyticsQueryRepository`, `AnomalyDetectionService` (and `anomaly_thresholds`), plus the unused fingerprint, event, engagement and CLI-session reads listed in the analytics changelog. `AnalyticsEventsRepository::new` takes only the event store.
- **Events / traits Rust API:** `OutboxChannel::Reporting` and `SessionStore::update_activity` (plus the users and analytics `SessionRepository::update_activity`) are removed; `increment_request_count` stamps the same activity columns.
- **Logging Rust API:** `services::retention` (`RetentionScheduler`, `RetentionPolicy`, `RetentionConfig`) is removed; it was never constructed and duplicated `database_cleanup`.
- **API:** the managed `/api/v1` surface is cut to the four routes the bridge calls: `POST /consumer-devices/enrollment`, `GET /consumer/resources/{resource}/publications/{publication}/bundle`, `POST /consumer/receipts` and `POST /consumer/session-bindings`. Removed: `GET /openapi.json`; `POST /sources`, `GET /sources/{id}`, `POST /sources/{id}/verification-bindings`, `POST /sources/{id}/captures`, `GET /revisions/{id}/bundle`, `POST /source-verifications`, `GET /source-verifications/{id}`; every `/inventory*` route (including installation status and coverage); `GET /operations/{id}`; the publication and `/resources/{id}/publications` routes; `/analytics/snapshots*`, `/analytics/jobs*`, `/analytics/live` and `/analytics/status`; `POST /consumer-devices/{id}/credential`, `POST /consumer-devices/{id}/revocation`, `POST /resources/{id}/consumer-grants`; `POST /consumer/invocations` and the `GET` status routes for receipts, session bindings and invocations. Bridges get their device credential from `POST /v1/bridge/device`; administration goes through `ManagedRepository` in-process.
- **Marketplace Rust API:** `ManagedRepository::{attribute_invocation, record_consumer_invocation, consumer_invocation_status, consumer_session_status, consumer_receipt_status, issue_api_consumer_credential, installation_coverage_status}` and the types `InvocationAttribution`, `InvocationAttributionRequest`, `AttributionStatus`, `TrafficClass`, `ConsumerInvocationRequest`, `ConsumerAttribution`, `CredentialIssueStatus` and `InstallationCoverageStatus` are removed. `refresh_installation_coverage` returns the number of resources whose coverage changed and sends no `feedback_snapshots` notification. Core no longer creates the `managed_invocation_attributions` table, which only a consumer trigger wrote; migration `marketplace/016` drops it (see Managed orchestration), and a consumer that writes or declares it must own it.
- **Scheduler:** the `feedback_facts_processing`, `feedback_snapshot_processing` and `service_registry_gc` jobs are removed. The scheduler rejects a configuration that still names them, so delete those `scheduler.jobs` entries before upgrading. Dead `services` rows are reaped by every replica's registry heartbeat.
- **Scheduler:** a tick writes nothing to `scheduled_jobs` until the job returns, and a successful run that reports itself idle (`JobResult::is_idle`: `with_stats(0, 0)` and no message) is not recorded. `last_run`, `last_status` and `run_count` now describe runs that did work or failed; the `running` status and `SchedulerRepository::increment_run_count` are gone, and `update_job_execution` advances `run_count` itself.
- **Managed orchestration:** everything the removed managed admin routes drove is gone. That covers durable API operations (`marketplace::managed::operations`), Git dependency verification and its source bindings, baseline preparation and its capture ledger, and `runtime::managed::capture_authoring_input`. It also covers the unread inventory membership, coverage and Git-binding readers, `InvocationAttributionId`, `DependencyVerificationId` and `models::feedback::verification`. Migration `marketplace/016` drops `managed_git_verifications`, `managed_dependency_verifications`, `managed_resource_git_bindings`, `managed_inventory_captures`, `managed_invocation_attributions`, `managed_api_operations` and the consumer invocation evidence, attribution projection and attribution history tables, so an upgraded database matches a fresh install. A table a consumer view still reads (an extension view joined the attributions until 0.53) is left, with a NOTICE, for the consumer to drop after redefining the view. `GitSourceOrchestrator::synchronize`, `revoke_consumer_credential` and `set_consumer_grant` stay.
- **AI / gateway Rust API:** the gateway and the in-process AI service address upstreams through one seam, `systemprompt_ai::{UpstreamTarget, UpstreamCall}`. `OutboundCtx` replaces `endpoint`, `api_key` and `api_key_is_bearer` with `upstream: &UpstreamCall`; `ProviderClientParams` replaces `wire`, `endpoint` and `api_key` with `target: UpstreamTarget`; `ImageProviderParams.api_key` is `target`. The provider clients gain `with_target`, and `new`/`with_endpoint` still take an API key. `systemprompt_models::wire::anthropic::auth_headers` is removed in favour of `UpstreamDialect`. A consumer building these structs must resolve an `UpstreamTarget` (`UpstreamTarget::from_secrets(entry)`, or `UpstreamTarget::api_key(..)` / `UpstreamCall::api_key(..)` for a bare key).

### Added

- **Claude on Vertex AI.** A provider with `wire: anthropic` whose endpoint is on `aiplatform.googleapis.com` (`…/projects/{project}/locations/global/publishers/anthropic`) and whose secret is a Google service account is served from Vertex. Requests go to the model's own `:rawPredict` / `:streamRawPredict` path with an OAuth bearer, `anthropic_version: vertex-2023-10-16` in the body and no `model` field, and a forwarded `anthropic-version` header is dropped. This works through the gateway and through the in-process AI service. Hosting is read from the endpoint host (`Hosting::of`, the one place that decides it) and the envelope for each wire and hosting pair is `systemprompt_models::wire::upstream::UpstreamDialect`.
...
- An `openai-responses` provider is spoken to on `/responses` by the in-process AI service; it was posted a Chat Completions body on `/chat/completions`.
- A provider's `extra_headers` are sent on every upstream request; neither stack sent them.
- Anthropic SSE framing is shared (`wire::anthropic::{SseFrameDecoder, sse_to_canonical_events}`) instead of being implemented once in the gateway and again in the AI service.
...
- **Loader:** a supervised process caught inside `execve` is no longer judged foreign; its identity is re-read briefly before a stop or reclaim is skipped.
- **Gateway:** a route's fallback is matched on the fallback provider by catalog id or upstream name, so Claude on Vertex AI fails over to Anthropic's API; boot validation prices it and checks its governance the same way, and the served model's own governance override and token limits apply to the failover.

### Changed

- Each request now costs one `UPDATE user_sessions` instead of two.
...
- **Gateway Rust API:** `SettlementOutcome::Failed` carries a `SettledFailure` (the usage the stream tap observed, priced) rather than only an error string. `GatewayAudit::fail_with_usage` is the entry point that supplies it; `GatewayAudit::fail` keeps its signature and passes `None`.
...
- **Profile:** the `evaluation:` block is `judge:` (`JudgeProfile`), which is what it has configured since the evaluation engine was removed; `Profile.evaluation` is `Profile.judge` and `PluginHooksRef.evaluation` is `PluginHooksRef.judge`. Both structs deny unknown fields, so both keep `alias = "evaluation"` and a deployed `profile.yaml` or a plugin kit still on the old key continues to load. Rust field accesses must be renamed; the aliases go once every pinned kit has been republished.
- **Config Rust API:** `config::RateLimitConfig` is deleted. `Config.rate_limits` holds `profile::RateLimitsConfig` directly, as its `retention`, `content_negotiation` and `security_headers` neighbours already held their profile types; `production()`, `testing()` and `disabled()` move onto `RateLimitsConfig`. No value changes — the twin's literals and the wire type's `const fn` defaults agreed pairwise.
...
- **Analytics Rust API:** the reporting baseline rebuild is phased, not one transaction. `ReportingProjector::begin_rebuild(conn, cutoff)` opens the generation, `clear_targets(conn, generation)` truncates the report tables, `write_snapshot_page(conn, definition, after, limit)` writes one keyset page set-based from the owner view, and `finish_rebuild(conn, generation)` flips `initialized`; `apply_snapshot` and `SnapshotCursor` are removed, and `AnalyticsError::RebuildSuperseded` fences a run whose generation moved. `AppContext::build()` no longer builds the baseline — `reporting::spawn` does it in the background — and `reporting::initialize` returns `RebuildOutcome` (`Rebuilt`, `AlreadyInitialized`, `InProgressElsewhere`). **A rebuild is no longer atomic to readers.** `clear_targets` truncates the report tables and the pages are written after it, so an analytics query running during a forced rebuild reads zero rows where the single-transaction `apply_snapshot` used to leave the previous baseline visible until it committed. `analytics_projection_state.initialized` marks the window and `projection::is_initialized` reads it, but no reader consults it yet — treat `analytics projection rebuild` on a live deployment as a period of empty reporting rather than a transparent refresh.
- **Analytics schema:** the `logs` and `ai_request_messages` reporting projections are dropped. They were the bulk of every rebuild and one outbox fact per log line, for two readers: agent top errors now group on `analytics_report_agent_tasks.error_message`, and gateway session message counts read a new `message_count` on `analytics_report_ai_requests`, maintained by a statement trigger over `ai_request_messages`. Both replacements sit on parents the retention rules already govern.
- **MCP Rust API:** `ToolExecutionResult.completed_at` is `Option<DateTime<Utc>>`. A hook-only row — an in-process call no server observed — stores no completion time rather than the ~0 ms its own timestamps implied, so averages and percentiles ignore it by construction.
...
- **Migrations:** a migration that rewrites a hot table declares what it measured — `-- @cost: rows=<n> measured=<slowest statement> triggers=<suspended|live>` in its leading comment block, beside `@no-transaction` and `@supersedes-checksum`. A malformed directive fails the build; `systemprompt_database::audit_migration_cost` (pg_query AST, beside `check_migration_references`) warns at boot for an undeclared bulk rewrite and is a hard failure in each repo's test suite. Rationale and the detected statement forms: `systemprompt_extension::cost`.
- **Migrations:** the boot-time `@cost` warning now fires only for a migration that is still pending against a hot table that already holds rows, and reports the row count. It audited the whole catalogue before, which on a populated instance meant twenty-two warnings per boot — every one already grandfathered in a cost baseline, none of them fixable, since a migration that has run cannot be made cheaper by a comment — and buried the one finding it exists to surface. A malformed directive still warns unconditionally. The whole-catalogue check is unchanged and stays in each repo's test suite.
...
- **Extensions:** `Extension::cross_extension_tables()` (default empty) declares the tables of *other* extensions a migration may `ALTER`. Schema installation refuses an undeclared cross-extension mutation (`CrossExtensionAlterUndeclared`), a declaration naming a table no other loaded extension creates (`CrossExtensionTableNotOwned`), and a table two extensions both create (`DuplicateTableOwner`). A table one extension quietly reshapes under another was invisible until the owner's next migration failed on it.
...
- **CLI:** `infra db doctor` audits schema residue — live tables no loaded extension declares, and migration ledgers of extensions that no longer exist — and exits non-zero when it finds any, because residue is the schema a deleted crate left behind and the signal that a drop migration is due. Boot runs the same audit (`audit_schema_residue`, `SchemaResidue`) as a warning.
- **Profile:** one `retention:` block carries every deletion window — `logs` 30 d, `analytics_events` 90 d, stored AI request messages following `ai.history.retention_days`, `mcp_tool_executions` 365 d, processed outbox rows 7 d, raw payload bodies 7 d, `governance_decisions` 180 d. `database_cleanup` enforces all of them in 5 000-row batches under a per-run time budget, so no statement takes a long lock and each capture trigger writes one outbox row per batch. Retention was previously an operator's memory.
- **Scheduler:** `scheduled_jobs.last_message` records what a run achieved — which tables `database_cleanup` pruned and by how much — and is what the console reads.
- **Security:** `systemprompt_security::at_rest` gains `seal` / `open` (ChaCha20-Poly1305 under the deployment's `encryption_master_key`, random nonce per call) for a stored secret that has to be replayed upstream rather than merely matched, where a digest is no use. It is the same key and AEAD the gateway accounting journal already uses on disk, so a deployment holds one at-rest key, not two.
- **MCP:** `tool_call_ledger.is_builtin` (`server_name = source`). A host-native tool such as `Bash` or `AskUserQuestion` is recorded under the vantage point's own name and is not an MCP server; consumers stop re-deriving that predicate, and the CLI's tool leaderboards, overview and activity series exclude those pseudo-servers wherever "tools" or "servers" is meant.
...
- **Gateway:** `/v1/models` carries `max_input_tokens` and `max_tokens` per model, and `/v1/bridge/profile` carries a `model_limits` map, both taken from the catalog's declared limits. Claude Code sizes a gateway model's context window from the first the way it does against Anthropic's Models API, and hosts that take per-model limits — OpenCode's managed config now declares `limit.context` and `limit.output` — take the second. Without them every model was treated as 200k and a 1M model was cut short by the client. `model_limits` is absent from an older server and defaults to empty.
- **Bridge:** the host installers resolve a `[1m]` default to its catalog id. OpenCode and Hermes name the catalog model, so a gateway default of `claude-sonnet-5[1m]` no longer fails the "is it one of the models we just declared" test and drop them back to whichever model sorted first. Claude Code's managed settings keep an existing `/model` choice, with one exception: the bare form of a `[1m]` default (`claude-sonnet-5` where the default is `claude-sonnet-5[1m]`) is what an earlier seed wrote and is the same model at a 200k budget, so it is raised to the variant rather than left capped. Any other existing choice survives.
- **Gateway:** a `[1m]` suffix on a requested model (`claude-sonnet-5[1m]`) resolves to the base catalog entry and is stripped before the request reaches the provider. It is Claude Code's client-side marker for the million-token context, not a vendor model id, so route matching, model lookup and upstream selection all read through it.
...
- **Analytics:** reporting capture is statement-level. `sp_capture_reporting_change` keeps its name and contract but runs once per statement over transition tables, so a bulk insert, a settlement update or a retention delete costs one `INSERT … SELECT` into the outbox instead of one trigger firing per row; the unchanged-projected-columns rule and the tombstone-plus-row pair for a key change are preserved as set operations. It no longer calls `pg_notify` — the worker polls, and the SSE bridge only ever loaded a reporting row to discard it. A row-level branch remains because the installer replaces function bodies before it runs migrations, so a database mid-upgrade still fires the old trigger shape into this body.
- **Analytics:** the outbox worker claims and applies in batches of 1 000 under one transaction instead of one transaction per fact. A batch that fails is re-driven in halves until the offending fact stands alone; that fact stays pending, is skipped for the rest of the drain and is named in the error, so one poison fact no longer blocks the queue. `applied_last_minute` joins the projection status.
- **Analytics:** the privacy barrier waits instead of raising. `prepare_reporting_privacy` fences claims and delivers the pending facts itself — the pattern the user deletion path already used — and raises `55000` only for a claim still in flight or a backlog above 100 000, which is a real operational signal rather than "the worker is a second behind". The compaction jobs stop failing under load.
- **MCP:** the server stamps its own id on its transport sessions (`McpHttpConfig.server_id`) and the gateway proxy stamps the verified user and the routed service as it caches the session's identity. `mcp_sessions` rows were created with `mcp_server_id` and `user_id` both NULL, so nothing downstream could say which server a session belonged to or who was on it. Existing rows are left alone; `COALESCE` keeps whichever side wrote first.
- **MCP:** an in-process execution that finds no tool-call intent to claim now records the session id, the tool and the window at `info`, and a successful claim logs at `debug`. The `Ok(None)` arm was silent, so a missed claim and a genuinely empty window were indistinguishable — 303 in-process executions on one instance carried no `ai_tool_call_id` and 138 of them had a claimable intent. No behaviour changes; this is instrumentation until a run says what is happening.
- **Provider catalog:** the default Anthropic catalog adds Opus 5.5 and drops the retired Opus 4.6, 4.7 and 4.8 and Sonnet 4.6 entries, with pricing reconciled against the published rate card on 2026-09-22. A deployment naming a dropped model in `gateway.routes` must move to a current one; the models were already hidden from discovery, so nothing that read `/v1/models` was offering them.
- **Vertex:** a model whose retirement date has passed is removed from the provider, together with any alias declared in its place, rather than kept with a warning. Past the retirement date the upstream has switched the model off, so an explicit declaration is not a preference the operator is entitled to keep — it is a route to a guaranteed 404 the caller pays latency for. The removal logs at `error`, because the operator has a catalog entry to delete. Inside the notice window (`is_retiring`) nothing changes.
- **Profile:** `RateLimitsConfig` documents the route group every `*_per_second` field governs, so `contexts_per_second` can be sized without reading the router. All thirteen stay: each is wired to a distinct mount site.

### Fixed

- **Rate limiting:** every `*_per_second` limit across the API was wrong by a factor of its own magnitude. `GovernorConfigBuilder::per_second(n)` sets a period of *n seconds per token*, the inverse of a rate, so `gateway_per_second: 100` refilled one token every 100 seconds. Limits are now built with `period(1s / per_second)`. A deployment that appeared merely busy was in practice never refilling.
- **Rate limiting:** the two validators covering these fields had drifted apart — the profile-side one checked `gateway_per_second` but neither registry budget, the runtime-side one checked both registries but not the gateway, and neither checked `bridge_auth_per_second`, so a zero in the wrong field passed whichever validator did not name it. Both now iterate `RateLimitsConfig::per_second_budgets`, a list derived from the struct, so a new budget is validated by existing code.
- **Rate limiting:** `forwarded_headers_ignored` was gated on the peer being in a private range, on the assumption that a misconfigured proxy is always a local one. A reverse proxy on a host network has a public address, so the deployment that most needed the warning never got it. The gate is gone, and the limiter's key extractor now warns (throttled) when it is bucketing by an untrusted proxy address, naming the peer to add to `server.trusted_proxies`. Until then every client behind that proxy shares one bucket and exhausts it together.
- **Gateway:** a request refused by the rate limiter produced no record at all, because the access log sat inside the limiter. An instance refusing every bridge sign-in with a 429 logged nothing, which is most of why it went undiagnosed. The log now wraps both limiters.
- **Gateway:** a stream that broke after the provider had billed tokens was recorded at zero — 34 production requests that ended "stream ended without stop event" show no tokens, no cost and no latency, and every cost view, quota bucket and rollup counted them as free. The failed-path UPDATE now writes the token columns, cost and latencies when the stream tap actually observed usage, and leaves the stored columns alone when it did not.
- **Gateway:** settlement never touched `user_sessions`, so `ai_request_count`, `total_tokens_used` and `total_ai_cost_microdollars` sat at their defaults on every session that had run inference — 105 of 105 sessions on one production dump. The completed arm now increments them through the existing `SessionStore::increment_ai_usage` (reached by a new `AppContext::session_store()` beside `session_usage()`, with no second repository constructed), after the settlement transaction commits and fire-and-forget, since the request is already settled. Migration `ai/035` recovers the history from `ai_requests`; system traffic is skipped.
...
- **Gateway:** a 5xx from the bridge release feed is logged at `error` rather than `warn`. Every client in the fleet polls `/v1/bridge/latest` on a timer, and one production instance answered 577 consecutive 502s over eleven days with no record of the cause.
- **Authz:** a pre-dispatch decision records the context it authorized. `extract/authz.rs` set `context_id: None`, so the audit sink re-derived one from the bridge session and every authz decision landed in a different context from the `ai_requests` row it authorized — 8,784 `governance_decisions` rows on one customer dump. The id is resolved before `enforce_authz_pre_dispatch` is called and is now passed through; migration `security/020` realigns the history on `trace_id`, lifting the append-only row trigger for its own statement and restoring it in the same transaction. No decision, policy, reason or actor is touched.
- **MCP:** a tool call was counted twice. The proxy recorded a call as it ran and the client's `PostToolUse` hook wrote a second row for the same call, and the fingerprint step meant to join them never matched — the hook's `session_id` is the Claude Code uuid while the proxy's is the JWT `sess_…`, and the payload digests differ — so 0 of 34 proxy rows carried an `ai_tool_call_id` and every consumer of `mcp_tool_executions`, `tool_call_ledger` and the reporting projection double-counted. A client-reported result now pairs with the newest unattested server-observed execution of the same tool by the same user inside 30 s before the fingerprint step runs; on a hit the hook's artifact attaches to that execution, its `tool_use_id` is stamped `inferred`, and no second row is written. The fingerprint step stays for OpenCode and other hosts, and migration `mcp/014` folds the duplicated history.
- **MCP proxy:** an external-server execution claims the tool-call intent that asked for it. `with_claimed_intent` was reachable only from `McpToolExecutor::execute`, so a proxied call wrote `ai_tool_call_id: None` and had nothing joining it to the inference turn — 247 proxy executions, none paired, on a customer dump where eight of nine configured servers are `type: external`. The claim mirrors the in-process executor, runs at finalize, makes the pairing `inferred` and never `exact`, and a claim failure leaves the execution unpaired rather than failing a call that has already returned. `INTENT_CLAIM_WINDOW_SECONDS` is exported so both correlators share one window.
- **MCP:** `mcp_proxy_identities.auth_token` held the caller's bearer JWT in clear — 18 live tokens readable from a backup. It cannot be hashed, because the proxy replays it upstream, so it is sealed with the at-rest cipher and a row that will not open is dropped rather than trusted. Migration `mcp/016` deletes the existing rows: this migration has no key, they must not stay readable, and the table is a 24-hour cache the next authenticated `initialize` rebuilds. Sealing needs the deployment's `encryption_master_key`, which until now was required only when the gateway was enabled; the proxy path is mounted either way, so a deployment that proxies external MCP servers with `gateway.enabled: false` must now set that secret or the proxy will refuse to cache an identity it cannot seal.
- **Scheduler:** one bad job no longer drops every job after it. `register_jobs` propagated `?` from `create_job_from_trait` and `scheduler.add`, so a single failing entry aborted the loop and every later job was silently dropped. Registration is per-job, a failure is collected as a `SkippedJob` and written to `logs` through the logging repository, and `SchedulerStartup.scheduled` reports the jobs actually on the cron loop instead of `self.config.jobs.len()`, which was a count of configuration. A job the configuration names and this build does not contain is reported as degraded, because it is the one skip that means the deployment is stale and the existing warning never reached anyone — the database log layer drops any event whose span carries no user, session and trace, and boot-time registration runs outside such a span.
- **Bridge:** a session is derived from the credential and the UTC date instead of generated per call. Every bridge call site minted a fresh `SessionId::generate()` before `adopt_or_mint_session`, and the server adopts an unknown id, so the hourly token refresh created a new `user_sessions` row each time — exactly 24 per user per day, 1,070 bridge sessions for 20 users on one customer instance. The id now comes from the `CredentialBinding` the install already computes for its token cache, so a refresh renews the session it owns and the id rotates only when the credential, gateway or day changes. Sign-in has no credential to bind to yet and still falls back to a generated id.
- **Bridge:** `database_cleanup` closes orphaned pending gateway requests. `fail_orphaned_pending` was reachable only from `journal::recover()` at boot, so a long-lived server never closed a row whose settlement was lost — three had been pending for six days. `ORPHAN_AGE` moves beside the query it bounds so both callers share one value.
- **Feedback:** a plain file's unavailable mode check no longer fails receipt verification. Windows has no POSIX mode bits, so the bridge reported every file's mode check as unavailable and no receipt from such a host was ever fully verified: the installation counted for nothing in adoption and every invocation on it was attributed as `revision_unknown`. A file that is not meant to be executable has no mode to satisfy — the content digest is the whole check — and migration `marketplace/014` re-judges the stored evidence by the same rule.
...
- **Analytics:** `ghost_session_cleanup` flagged any session with no page views as a behavioural bot, and a bridge, API or MCP session has no page views by construction. Migration `users/019` is the missing backfill for the predicate fix — 1,199 rows unflagged on one restore, with the 32 genuinely web ghost sessions correctly left flagged.
...
- **MCP:** `terminate_gracefully_verified` returns only once the process is actually gone. It verified the PID *before* signalling — that it is our child and not a recycled one — then sent SIGTERM, slept a fixed 500 ms, sent SIGKILL and returned. SIGKILL is asynchronous, so the call reported a termination it had never observed, and a caller restarting the service on the same port raced the child it believed it had stopped. It now waits for the child to disappear (briefly after SIGTERM, then up to five seconds after SIGKILL) and returns an error if it does not, so the name is something a caller can rely on.
- **AI:** `AiRequestTrace::sample` failed for ordinary requests. The tool-catalogue dedup replaced the payload's `offered_tools` column with a `LEFT JOIN` onto `ai_tool_catalogs`, whose `tools` column is `NOT NULL`, so the query decoded it non-nullably and every completed request with no catalogue row — which is most of them — came back `unexpected null`. The column is read as nullable again.
- **Analytics:** a forced reporting rebuild could livelock. Each fence opens a generation that supersedes any in flight, and a superseded run retried in an unbounded loop with no backoff, so two concurrent rebuilds superseded each other indefinitely. Retries are now bounded with a backoff, and `reporting::rebuild` returns its `RebuildOutcome` instead of discarding it — a caller that did not rebuild is told so rather than being handed `Ok(())`. `analytics projection rebuild` says when another replica owns the generation, because its status card reads the same either way.

- **Packaging:** `systemprompt-ai`'s committed sqlx cache was missing the query its failure-settlement path issues. The failed-path UPDATE was changed to write the token and cost columns without the offline cache being regenerated, so the old entry was orphaned and the new query had none — the crate did not build offline from the registry, which is the only way a crates.io consumer builds it.

### Removed

- **Indexes:** 73 indexes across ai, mcp, users, oauth, agent, content, files, analytics, marketplace, security, scheduler and `extension_migrations` whose column list is a strict prefix of another index on the same table. A prefix index answers no query its superset cannot and costs a write on every insert. Each was proven against a production restore rather than taken from a report: none is UNIQUE, none backs a constraint, and each has at least one covering index that is not partial — a filtered superset does not answer what a full index does. The `CREATE INDEX` lines go from the base schema in the same commits, because the declarative phase runs after migrations on every boot and would otherwise recreate them before the server finished starting.

## [0.58.0] - 2026-09-21

### Breaking

- **Gateway/model contracts:** `CanonicalRequest` gains top-level `cache_control`, `ModelCapabilities` gains `prompt_caching`, and `GatewayConfigSpec` / `GatewayConfig` gain `automatic_prompt_caching`. Existing YAML defaults automatic caching on and the model capability off; Rust struct literals must supply the new fields.
...
- **Migrations:** `-- @supersedes-checksum: <old>` in a migration's leading comment block names the checksum of the text it replaces, so an applied migration can be corrected in place. A migration that must run in its own slot — before the migrations built on it — cannot be fixed by a later one, and editing it made every database that applied it refuse to boot on checksum drift. The build script emits `.superseding(old)`, `verify_checksum` accepts the old checksum, and the tracking row moves to the new one without executing anything. The directive is one per replaced text, since a file may have shipped under more than one.
- **Gateway:** the default Anthropic catalog declares prompt-caching support for every active Claude model, and the canonical Anthropic codec preserves top-level automatic-cache directives as well as explicit block breakpoints.

### Changed

- **Provider catalog:** models added through `ProviderCatalogService` default to visible discovery entries under the new `ProviderModel::hidden` contract.

### Fixed

- **Installer:** a migration may execute a function only a declarative schema file defines. Migrations run before the dependent phase that applies declarative functions, views and triggers, so such a reference worked on every database that had already booted on the schema shipping it and failed the first upgrade from one that had not. A routine pre-pass now applies every extension's declarative `CREATE OR REPLACE FUNCTION` with `check_function_bodies = off` after the structural phase and before any migration, whatever the registry order; the dependent phase re-issues each routine with bodies checked against the migrated schema. A declarative `CREATE FUNCTION` without `OR REPLACE` is refused at prepare.
- **Installer:** `check_migration_references` parses every non-tombstone migration before any database write and refuses one that names a declarative-only trigger or view — `ALTER TABLE … ENABLE/DISABLE TRIGGER`, `DROP TRIGGER`/`VIEW` without `IF EXISTS`, or a query over the view — with `LoaderError::MigrationReferencesDeclarativeObject`. A catalog-guarded reference inside a `DO $$` body is the sanctioned form, since those bodies are opaque to the parser. The check runs on every boot regardless of database state, so it surfaces on the author's workstation and on every fresh CI database.
- **Installer:** the routine pre-pass leaves alone a function a migration reshapes. `CREATE OR REPLACE` cannot change a function's return type or parameter names (`42P13`), and a release that reshapes a declarative function does it with `DROP` + `CREATE` in a migration that runs after the pre-pass. Each routine now runs under a savepoint and, on `42P13` only, the savepoint is rolled back with a warning; a signature nothing reshapes still fails in the dependent phase.
- **Database:** the cross-extension ownership check counts tables that an extension's own earlier migrations created. A table created in migration N and dropped in migration M is nowhere in `schemas()`, so an `ALTER` between N and M was refused as a cross-extension mutation whenever the chain replayed on a database older than N.
- **MCP:** migration `010_reporting_capture_contract` recreated the `mcp_tool_executions` capture trigger with `EXECUTE FUNCTION sp_capture_reporting_change()`, a function only the events extension's declarative schema defines. A database that had never booted on a reporting-era core failed every upgrade at `function sp_capture_reporting_change() does not exist`. The slot is tombstoned — the view and trigger are declarative state, re-established on every boot — and the outbox fact repair moved to `013_reporting_fact_repair`, guarded on `event_outbox.fact` existing so it is also safe ahead of the events migrations.
- **MCP:** migration `008_artifact_narrow_waist` declares both checksums it shipped under. Its original text shipped in 0.54 and again from 0.55.1, and an edited text in 0.55.0, so a database fresh-installed on 0.55.0 was stamped with the edited checksum and refused every later boot; either row now moves to the current text.
- **Marketplace:** migration `012_local_tree_roots_follow_current` disabled the `managed_sources_immutable` trigger around its rewrite. That trigger is declarative state first shipped in 0.53, so a database last booted on 0.52.0 failed every upgrade at `trigger "managed_sources_immutable" does not exist`. The slot is tombstoned and the rewrite moved to `013`, which lifts the trigger only where `pg_trigger` says it exists; databases that applied `012` keep their tracking row and `013` is a no-op there.
...
- **Gateway:** `/v1/models` and `/v1/bridge/profile` advertise canonical visible model IDs only; aliases and hidden legacy models remain routable without appearing in managed agent pickers. Managed host profiles for Claude Desktop, OpenCode, Codex CLI and Hermes consume the same canonical catalogue, so alias duplicates and hidden legacy models leave their configuration.
- **Gateway:** the default Anthropic catalog advertises the current Fable 5.1, Opus 5, Sonnet 5 and Haiku 4.5 lineup and hides active legacy generations.

### Removed

- **Provider catalog:** the retired Opus 4.1, Opus 4 and Sonnet 4 entries. Configured routes must migrate to Opus 4.8 or Sonnet 4.6.

## [0.57.0] - 2026-09-19

### Breaking

- **Security:** `GovernanceWarningRow.user_id` uses `UserId`; Rust callers construct typed IDs and convert to text at display boundaries. CLI warning grouping output is unchanged.
...
- **Bridge:** `HostWarnings::push(kind, host_id, message)` takes a `HostWarningKind` first (`cowork_session_missing`, `plugin_dependencies`, `evidence_unacknowledged`, `permission_rules`, `tool_catalog`, `node_packages`, `manifest`) and the wire `HostWarning` gains `kind`; `reapply_stale_profiles` takes an `Attendance`; `reg_profile::profile_entries` / `render_reg` return `Result<_, MdmError>`; `recover_pending` takes a deadline and returns `RecoveryProgress`, `capture_host` returns `CaptureOutcome`; `device.action.open` is no longer an IPC command.

### Added

- **Gates:** two source gates, on `just check-gates` and in the Quality workflow. `just lint-swallowed-errors` (`scripts/rust-contracts` mode `swallowed-errors`) fails a `map_err(|_…| …)` on a network or database result (`.send()`, `.json()`, `.execute()`, `.fetch_*()` …) whose closure neither names the error nor logs it; `// Why: discard-ok: <reason>` is the only exemption. `just lint-bridge-typed-warnings` fails bridge control flow that reads a host warning's message text or a `host_warnings` predicate that never reads `.kind`. `scripts/check-test-value.sh` additionally rejects a `wiremock` JSON-RPC mount that matches on `method` alone; the frame is the contract. Both scripts are tracked — they first landed gitignored and the linter job ran `not found`.
...
- **MCP:** the internal-MCP proxy dialled the port stored in the `services` row, so a row written by an earlier run under a different `services.port_offset` left the proxy answering 502/503 forever against a port nothing listens on while the live sidecar served elsewhere. `ServiceResolver` cross-checks a running internal server's row against the port this instance spawns (`McpServerConfig::spawn_port()` from the offset-applied registry) and on a mismatch rewrites the row (`ServiceRepository::update_service_port`) and proxies to the real port. The scheduler's state verifier no longer reads a `running` row whose PID is alive but whose port is unresponsive as an indefinite `Starting`: past a 45 s startup grace (`state_verifier::is_wedged`, measured from the row's `updated_at`) it is `Crashed` and the reconciler restarts it; a genuine boot keeps the grace. The first requests after an offset change still 502 until that restart.
- **MCP:** a `connector:`-backed MCP server is published to the bridge manifest through the gateway (`/api/v1/mcp/{name}/mcp`), as an `external_auth` server already was. The registry resolver synthesises the per-user accessor for a connector, but the manifest catalog only checked `external_auth` and published the raw upstream endpoint, so the bridge sent the gateway JWT straight to the provider and every `google_drive` tool call from Claude Code failed 401 while the same call through the gateway succeeded.
- **Proxy:** an external MCP response is reassembled without the upstream's `content-length`, `transfer-encoding` and `content-encoding`. The audit tap rewrites a buffered `tools/call` body to stamp the execution id and reqwest has already decoded any content-encoding, so carrying the upstream length through made hyper panic on the mismatch and close the connection — every stamped tool result from an external server reached the client as a dropped connection.
- **Marketplace:** the "not selected by any enabled plugin" warn-once set lives on `MarketplaceCache` (the application context's), not in a process-global static.
- **Bridge:** every GUI request since 0.53.0 was refused (`IpcRequest` required a `mount` nonce the page never sent) and the welcome wizard re-opened on an upgraded machine whose profiles read `Stale`; the IPC bus is one file (`web/js/ipc-bootstrap.js`) shared by the webview, the preview and the node tests, the wizard honours the on-disk completion record and the GUI re-applies its own stale profiles at launch without a prompt. Windows: the Claude Desktop profile renders the whole `claude_desktop_policy`, so Repair replaces a machine hive that differs beyond the six inference keys. The GUI opens plain `http://` to `localhost` / `127.0.0.1` / `[::1]`, so a self-hosted gateway's Connectors "Connect" button works. The Cowork tick re-requests a sync only for a `cowork_session_missing` warning (`HostWarning.kind`) instead of every 30 s for any Claude Desktop warning; installation-evidence capture runs the hosts concurrently under one budget and reports "N of M receipts still pending" instead of "deadline has elapsed". The MCP probe sends `tools/list` with `params: {}` and reports a JSON-RPC error by code. A file the installed tree already holds at the manifest digest is copied instead of downloaded. Full list in `bin/bridge/CHANGELOG.md`.
...
- **Profile:** `Profile` gains `observability: ObservabilityConfig` (default empty); struct literals must name it.
- **Gateway:** the canonical request carries Anthropic's prompt-cache breakpoints. `CanonicalContent::Text` and `::Image` are struct variants (`{ text | source, cache_control }`; build them with `CanonicalContent::text(..)` / `::image(..)`), `::ToolUse` and `::ToolResult` gain `cache_control`, `CanonicalTool` gains `cache_control`, and `CanonicalRequest.system` is `Vec<SystemBlock { text, cache_control }>` instead of `Option<String>` (`system_text()` is the joined view, `set_system_text` the flat setter). `protocol::inbound::anthropic_messages` parses `cache_control` on system blocks, content blocks and tools; `wire::anthropic` re-emits it, and emits a single uncached system block as the string the client sent; the OpenAI and Gemini wires ignore it.
- **Gateway:** `audit::payload::slice_payload` takes the cap as its second argument; `GatewayRepositories` carries `payload_cap_bytes` (`with_payload_cap`). `AiRequestPayloadRepository::upsert_prepared_sha256` is `upsert_prepared(id, sha256, tools)`. `profile::GovernanceConfig` gains `audit: AuditConfig`; struct literals must name it.
- **MCP:** an `external` server declares no `binary`, `package` or `port` — it is proxied to its `endpoint` and never bound locally, so `Deployment.binary` / `.port` and `McpServerConfig.binary` / `.port` are `Option`, `package` no longer needs to be present as `null`, and an external server that still carries any of the three is a startup validation error (as is an internal one missing `binary` or `port`). Port uniqueness (`McpConfigValidator`, `ServicesConfig::validate`, the registry validator) is checked over internal servers only, so two externals can no longer "collide". `McpServerInfo.port`, `McpServerState.port`, `McpServiceStatus.port`, `McpServerSummary.port` and the `/api/v1/mcp/registry` `port` field are `Option<u16>` (`None` for external; the `0` sentinel is gone), and `McpStatusEntry.binary` is `Option<String>`. Spawn, port-probe and reconciliation paths resolve the pair through `systemprompt_mcp::services::SpawnTarget` (`spawn_port()` / `spawn_binary()`) and return `McpDomainError::Configuration` for a server with neither instead of binding port 0. `DeploymentService::get_server_{port,binary,package}` are removed.
- **MCP:** every tool result is ingested as a typed artifact through `systemprompt_mcp::ArtifactIngest`. `McpToolExecutor::new` and `McpResponseBuilder::build` take `Arc<ArtifactIngest>` / `&ArtifactIngest` in place of `McpArtifactRepository`; `ArtifactIngest::new(ArtifactIngestRepositories, Option<Arc<SecretScanner>>)`, with the bundle at `systemprompt_mcp::repository::ArtifactIngestRepositories::new(&DbPool)`. `ToolExecutionRequest` gains `source: ExecutionSource`; `ToolExecution` gains `source` and `correlation: Correlation` (`systemprompt_models::mcp::{ExecutionSource, Correlation}`; `ExecutionSource` is `in_process | proxy | gateway | hook_claude_code | hook_opencode`). `ToolUsageRepository::find_by_fingerprint(&SessionId, tool_name, payload_sha256, window)` and `find_unclaimed_intent` take a typed session id; `log_execution_sync_with_id`, `mark_correlated` and `claim_intent` are new. `McpArtifactRecord` / `CreateMcpArtifact` carry keyed columns (`session_id`, `trace_id`, `ai_tool_call_id`, `tool_name`, `source`, `payload_sha256`, `payload_bytes`, `ArtifactShape { is_structured, has_ui_resource, is_error, secret_redactions }`) instead of the JSON envelope; `ArtifactType::ToolResult` (`tool_result`) is new. `McpAudit::new` (proxy audit) takes `Option<Arc<ArtifactIngest>>` as its second argument. `GovernanceDecisionRecord` / `AuditTarget` gain `tool_use_id: Option<_>`. `NormalizedAnalyticsFact::Artifact(Box<NormalizedArtifactFact>)` and `AnalyticsFactKind::Artifact` are new variants. `McpToolHandler` and `object_input_schema` live in `tool::handler`. Migrations: mcp `008_artifact_narrow_waist` (`artifact_payloads`, `mcp_artifact_findings`, new `mcp_artifacts` / `mcp_tool_executions` columns and CHECKs, unique `mcp_artifacts(mcp_execution_id)`), mcp `009_mcp_sessions_cascade`, mcp `010_reporting_capture_contract`, security `019_governance_decisions_tool_use_id`, analytics `011_artifact_fact_kind`, and the `tool_call_ledger` view (domain/ai schema) joining intent, execution and result.
- **Users:** `UserRepository::delete` and `UserService::delete` return `Vec<PurgeCount>` (rows removed per table); `UserRepository::purge_preview` is new. `user_sessions.user_id` and `mcp_sessions.user_id` are `ON DELETE CASCADE` instead of `SET NULL` (users migration `016_user_sessions_cascade`, mcp migration `009_mcp_sessions_cascade`). `systemprompt_extension::purge` — `UserPurgeTable`, `registered_user_purge_tables()` and the `user_purge_tables!("<owner>", [...])` macro — is the `inventory` registry every crate uses to declare user-keyed tables that cannot carry a foreign key; `UserRepository::delete` clears them inside the privacy transaction.
- **Wire:** `wire::gemini::request_parts` is a module (`content_to_part`, `plain_text_part` moved out of `request.rs`); `wire::gemini::streaming_parts` holds the text/thought/function-call emitters. `services::gateway::{slugify_pattern, synthesize_route_id}` live in `route_id.rs` (re-exported unchanged). `CanonicalContent::text` / `SystemBlock::text` take `impl Into<String>`.
- **Models:** `PluginHooksRef` gains `evaluation: bool` (`#[serde(default)]`); struct literals must name it. Validation requires one owner for `hooks.evaluation`, and that owner must also own the governance hooks.
- **Loader:** `ActiveServicesRoot` gains `base: PathBuf` (the profile services path the composition is layered on; equals `path` when nothing is composed); struct literals must name it.

### Added

- **Gateway:** a route's `upstream_model` / `fallback_upstream_model` is mapped through the provider catalog like a requested model: a catalog id or alias resolves to that entry's wire `upstream_model`, and a name the catalog does not list passes through verbatim (`ProviderEntry::upstream_model_for`). Previously an override was sent verbatim, so a Vertex fallback declared by catalog id (`vertex-gemini-3.8-flash`) hit the wire under the catalog id.
- **Observability:** OTLP export. A profile block `observability.otlp { endpoint, protocol: http, headers, signals: [traces, logs], batch_seconds }` (`profile::{ObservabilityConfig, OtlpExportConfig, OtlpProtocol, OtlpSignal}`, validated: http(s) URL, non-empty distinct signals, `batch_seconds` 1–3600, header names/values; `grpc` is refused as not yet supported) names a collector, and the new scheduler job `otlp_export` (`OtlpExportJob`, default every 15 s, must be enabled in the scheduler config) tails `ai_requests`, `tool_call_ledger`, `governance_decisions` and `logs` by a per-signal `(timestamp, id)` watermark in the new `otlp_export_state` table and POSTs OTLP/HTTP protobuf: one span per completed request (user, session, provider/`served_provider`, model, tokens, cost, latency, finish reason, client) with tool-call and governance-decision child spans, and one log record per `logs` row with trace/span correlation. Ids are digests of row keys (a 32-hex `trace_id` passes through), so a re-sent batch overwrites rather than duplicates; a failed batch retries three times with backoff on transport errors and 429/502/503/504, then keeps its watermark for the next tick. `otlp_export_state` records `watermark`, `last_attempt_at`, `last_success_at`, `last_error`, `batches_total`, `failures_total`, `rows_total`; `OtlpExportStateRepository::list_states` reads them with a derived `lag_seconds`, and `otlp_export_now` runs every configured signal without pacing. Metric: `otlp_export_batches_total{signal,status}`. Metrics stay on `/metrics`.
- **Gateway:** provider failover. A route may name a `fallback_provider` (and `fallback_upstream_model`): when the primary exhausts the 429/503 retry budget, answers any other 5xx, or the connection fails, the governed request is re-bound to the fallback and sent under the same retry policy — never re-governed or re-scanned, and never via the raw passthrough lane. Boot validation requires the fallback to exist, differ from the primary, and satisfy the route's pricing and `requires` (`RouteFallbackProviderNotInRegistry`, `RouteFallbackIsPrimary`, `RouteFallbackModelWithoutProvider`). A per-provider circuit breaker built from the provider's `resilience:` settings (`service::failover::ProviderBreakers`) skips a primary that keeps failing without spending its retry budget, and never leaves a request unsent. The audit row gains `served_provider` (`ai_requests` migration `029`) and `route_match` carries `failover:<from>-><to>`; cost is repriced at the serving provider's catalog rate (`GatewayAudit::reprice`). Metric: `gateway_upstream_failovers_total{from,to,reason}`. `GatewayRoute` gains the two optional fields; `GatewayRoute::fallback_view` is the route as the fallback serves it.
- **Gateway:** `governance.audit.payload_cap_bytes` (default 1 MiB, minimum 64 KiB) replaces the hard-coded 1 MiB cap on stored request and response bodies; a body over the cap keeps its SHA-256 digest and a head+tail excerpt. `ai_request_payloads.prepared_tools` (migration `030_prepared_tools`) stores the exact `tools` array of the prepared, provider-transformed body beside `prepared_body_sha256`, so the client-vs-provider tool schema can be diffed from the audit row; `AiRequestPayloadRepository::find_prepared` reads both.
- **Gateway:** prompt caching survives a rebuild. A request that leaves the same-wire passthrough lane (system-prompt override, cross-wire translation, secret-sanitising re-prepare) is re-rendered with every `cache_control` breakpoint where the client placed it; a system-prompt override keeps the conversation and tool breakpoints. On the passthrough lane the prepared body is the request body, digest for digest. `wire::anthropic::{cache_control_from_anthropic, cache_control_to_anthropic}` are the codec; `CacheControl { ttl: Option<CacheTtl> }` models `ephemeral` with the `5m` / `1h` TTL.
...
- **Runtime:** `AppContext` owns the process's `AiService` and `ArtifactIngest`: `ai_service()` / `ai_service_arc()` return `Option<Arc<AiService>>`, assembled at boot from the services catalog (`None`, with a warning, when no default provider is usable); `artifact_ingest()` / `artifact_ingest_arc()` return the one ingest path. Every extension router receives `Extension<Option<Arc<AiService>>>`, `Extension<Arc<ArtifactIngest>>` and `Extension<ServicesRefresh>` beside the governance engine. The server builder takes the gateway-policy repository from `AppContext::ai_repositories().gateway_policies` instead of constructing one.
- **Profile:** `evaluation.automatic` (`profile::EvaluationProfile`, default `false`) declares whether closed sessions are queued for scoring without an operator's click; the block is `deny_unknown_fields`.
- **AI:** every `ai_requests` row records the upstream's own terminal reason in `finish_reason` (migration `028_ai_requests_finish_reason`): Gemini `STOP`/`MAX_TOKENS`/`SAFETY`/`MALFORMED_FUNCTION_CALL`, OpenAI `content_filter`, Anthropic `refusal`, beside the normalised status. Every inbound wire codec surfaces it (`CanonicalEvent::MessageStop.raw_finish_reason`, `CanonicalStopReason`), the stream tap and settlement journal carry it to the row, and `infra logs request list` / `audit` print it. Inference made as a job actor is recorded with `request_kind = utility` (`ActorKind::Job` → `RequestKind::Utility`), so judge and housekeeping calls can be excluded from user figures by kind as well as by actor.
- **MCP:** `ArtifactIngest` is the one path from a wire tool result to storage: it normalises the result into the typed artifact model, resolves the execution by exact key (`_meta` execution id, `ai_tool_call_id`) or by fingerprint within `FINGERPRINT_WINDOW_SECONDS` (120 s) recorded as `inferred`, runs the secret scanner (redacting in place) and the gateway safety scanners (`ArtifactScanner`) before any row is written, stores the body once by digest in `artifact_payloads`, and enriches rather than duplicates when the same call is seen from another vantage point. A caller-minted `mcp_execution_id` unknown to `mcp_tool_executions` is recorded under that id before the artifact row references it. The in-process builder, the proxy tap (which stamps the execution id into the response `_meta`), gateway `tool_result` replays, and the Claude Code / OpenCode hooks all use it. An MCP call from a Claude Code session is one artifact: the executor claims the gateway's unclaimed intent for the same session and tool suffix and writes `ai_request_tool_calls.mcp_execution_id`. Governance decisions keep the client `tool_use_id` they governed. OpenCode reports every tool completion, not only skills. The feedback plane gains the `artifact` fact kind.
- **MCP:** an `external` server is never spawned and carries no service row: the tool loader lists its tools straight from the `endpoint` (registry metadata `status: external`), and `plugins mcp call|tools` / `admin agents tools` reach it by its direct URL (`shared::mcp_tools::direct_url`) instead of a loopback port.
- **Services:** `ServicesRefresh` (`routes::admin::services::ServicesRefresh::run(actor)`) is the in-process handle to the fetch-verify-compose-reconcile pipeline behind `POST /admin/services/refresh`, so a console that has authorised a caller by its own rule runs the same refresh without minting an admin token. One process-wide `RefreshLock` guards the route and every handle.
- **Plugins:** `hooks.evaluation` (server-side switch beside `governance`; installs no client hook, `is_empty` ignores it) marks the plugin whose track hook feeds the session-evaluation engine. The OpenCode adapter translates `session.created` → `SessionStart`, `session.idle` → `Stop`, `session.deleted` → `SessionEnd`. `services/evaluation` is on `BUNDLE_ALLOWED_DIRS`, so a bundle carries it into the composed tree (previously `governance_bootstrap` failed at boot with `services/evaluation/config.yaml is missing`).
- **CLI:** `admin users delete --dry-run` reports the per-table purge counts without deleting. `infra db tables` labels planner estimates and gains `--exact` (`COUNT(*)` every table); `infra db query` renders `regclass`, `"char"` and `oid` columns instead of blank cells.
- **Bridge:** `astound-bridge install --apply-schedule` degrades to a warning naming the activation commands where `systemctl --user` has no user manager (containers, WSL) instead of failing the install; an activation a live user manager refused is still `InstallError::ScheduleActivation`.

### Fixed

- **Wire:** mid-history `system` messages (Claude Code's environment block with the skills listing and MCP instructions, and the `<total_tokens>` reminder after every tool result) reach Gemini and Anthropic-wire upstreams instead of being dropped: Gemini folds the text as a user part into the neighbouring user turn, the Anthropic block mapper renders `System` as user. Text blocks beside function responses are folded into the Gemini function result (a `[functionResponse, text]` user turn returned `STOP` with an empty part on Vertex). `thinking: {type: adaptive}` parses as on with the budget left to the model and is re-emitted as `adaptive` on the Anthropic wire rather than an `enabled` block with no budget.
- **MCP:** the wire `structuredContent` carries the typed tool output again (the stored artifact envelope was sent, so every call failed the client's `outputSchema` validation); the stored copy is substituted only under redaction and unwrapped first. A declared known artifact type that does not deserialise as itself is stored as declared and logged instead of being demoted to `tool_result`.
- **MCP:** the `mcp_tool_executions` reporting capture emitted `source`/`correlation`/`payload_sha256`, which the reporting contract does not list, so `begin_user_privacy()` (user delete, anonymous cleanup) raised on any instance that had run an MCP tool. Migration `010_reporting_capture_contract` restores the column list and repairs queued facts in `event_outbox` in place.
- **Marketplace:** a `local_tree` source captured while bundle sources were composed recorded the hash-keyed composed directory, which rotates on every import; every configured resource on that source then failed capture and `publish_latest` blocked them all (45/45 skills on production) while plugin manifests advanced. The binding now records the root as configured (the cache's `current` link under composition, the profile path otherwise) and matches the active root by path, resolved directory or base tree; migration `012_local_tree_roots_follow_current` repoints existing bindings, lifting the `managed_sources_immutable` trigger for that one rewrite.

## [0.54.0] - 2026-09-16

### Breaking

- **Evaluation engine removed.** The `systemprompt-evaluation` crate, its `eval_*` tables and every campaign/experiment/budget/suggestion/approval/holdout surface are gone: `/api/v1/campaigns*`, `/campaign-runs`, `/campaign-diagnostics`, `/source-changes`, `/experiments*`, `/budgets*`, `/evaluation-revisions*`, `/evaluation-suggestions*`, `/evaluation-approvals*`, `/revisions/{id}/workspace`, `/evaluator-capabilities` and `/api/v1/evaluation/worker/*`; `admin evals`; the `evaluation_supervisor` job (`EvaluationSupervisorJob`, `services::evaluator`); `Profile::evaluator` / `profile::EvaluatorConfig` (a profile carrying an `evaluator:` block is rejected as an unknown key); every `Eval*Id`; `ManagedRevisionOwnership`; `AppContext::evaluation_repositories`; the marketplace attestation seam (`managed::evaluation`, `managed_evaluation_attestations`, `managed_publication_reviews.experiment_id` — marketplace migration `011_drop_evaluation` drops the tables with CASCADE and the extension's `extension_migrations` rows). `ComparisonEvidence` is the flattened `recorded` map, `PublicationAdmission::Attested` is `Reviewed`, `TrafficClass` is `Production | Fixture`, and `systemprompt_runtime::optimization` is `managed`. The gateway no longer consults an evaluation policy: `dispatch_policy` returns the policy alone, pricing resolves from the catalog, and the `spexec_` execution credential / `AuthedPrincipal::Execution` are removed. The managed routes that shared the tree (`/sources*`, `/revisions/{id}/bundle`, `/source-verifications*`, `/operations/{id}`, `/inventory*`, `/publications`, `/resources/{id}/publications`, `/analytics/*`, `/consumer*`, `/consumer-devices*`) are unchanged on the wire and live in `routes::managed`.
...
- **AI:** `RequestOrigin` carries a third half, `attestation: ClientAttestation`; `RequestOrigin::gateway` takes it and `RequestOrigin::INTERNAL` pairs all three. `GatewayRequestContext` gains `evidence: ClientEvidence`. `ClientKind::from_user_agent_and_body` is removed. Migrate by calling `wire::origin::classify` (or `ClientAttestation::None` where no request is classified).
- **Gateway:** `POST /v1/bridge/heartbeat` answers 401 when the body's `session_id` differs from the token's session.

### Added

- **Services:** `POST /admin/services/refresh` imports a recomposed services bundle in place: when the composition changed it loads the new tree through the loader's atomic `current` link, projects the bundles' entitlements into the authz tables and refreshes the skill inventory in the running process; the reply carries `reconciled` and `restart_recommended` (governance hooks are read once at boot, so `restart=true` stays an explicit opt-in). A repeat import measures `changed` against the served composition and is a no-op; `reconciled` is true only when a projection ran. A profile that pins only kits composes the image's baked tree as member zero; OCI blob pulls follow up to three registry redirects without carrying the registry credential. The public `/marketplace.json` carries `allowCrossMarketplaceDependenciesOn`.
- **Marketplace:** inventory generations move only when an entry record changes, and every observation records the composed hash, base tree hash and pinned bundle digests that produced it (`managed_inventory_state.sources` / `managed_inventory_observations.sources`, migration `010_inventory_sources`; `systemprompt_loader::bundle::provenance`). Publications record `composed_hash`, `bundle_content_hash` and `inventory_generation` in their review evidence. The inventory root may be the loader's composed-root link. Installation receipts decode `consumer_evidence` and the consumer identity columns leniently.
- **MCP:** a deployment's outbound `connector` gains `display_name`, `authorization_params` (only `access_type`, `prompt`, `login_hint`, `hd`; single non-empty tokens) appended verbatim to the authorization request, and `identity: userinfo`, which reads the connected account's label from the issuer's OIDC `userinfo_endpoint` and requires the `openid` scope. `ConnectorConfig` / `ConnectorIdentity` live in `systemprompt_models::mcp::connector`.
- **Feedback:** `NormalizedInvocationFact` gains an optional `skill: InvocationSkillIdentity` (`plugin_id`, `skill`, `marketplace_id`, `source`, `source_hash`) — the identity every invocation has whether or not the skill is a managed resource, kept distinct from `attribution`, which stays the revision proof. Facts written without the key still deserialise; an instance's projection may set it so a skill-keyed figure can also name the published tree (`base` or `bundle:<name>` and its content hash) the skill ran from.
...
- **AI:** every `ai_requests` row records `client_kind` (`claude-code`, `claude-desktop`, `codex`, `opencode`, `hermes`, `other`, `internal`, `unknown`) and `wire_protocol` (`anthropic.messages`, `openai.chat`, `openai.responses`, `internal`, `unknown`) as `NOT NULL` CHECK-constrained columns (migration `026_ai_requests_client_origin`, backfilled from the retained request body where it survives). `infra logs request list` shows the client.
- **AI:** client attribution is evidence-tiered. `ai_requests.client_attestation` (`host-token`, `bridge-secret`, `declared`, `native-marker`, `user-agent`, `none`, `internal`, `unknown`) records how strongly the client is evidenced and `ai_request_client_evidence` records what the wire carried (migration `027_ai_request_client_attestation`, with a down file). `client_kind` gains `pi`. `systemprompt_models::wire::origin::classify` is the one ladder: a bridge-verified host token, then an `x-systemprompt-client` declaration, then a structural body marker, then the exact first User-Agent product token — never a substring. A malformed declaration, or an `x-systemprompt-client-attestation` header from anything but the bridge, is a 400 whose rejected row keeps the value as evidence. `infra logs request list` shows the tier; `show` prints the evidence.
- **Identifiers:** `headers::CLIENT_KIND` (`x-systemprompt-client`) and `headers::CLIENT_ATTESTATION` (`x-systemprompt-client-attestation`).
...
for Claude Code, Codex, OpenCode and Hermes under one container supervisor,
device-scoped consumer credentials with installation receipts and session
bindings, marketplace inventory and complete Git bundle verification,
analytics rebuilt on durable outbox facts, projections and snapshots with
privacy-coordinated retention, and a bridge that enrols a device and keeps a
bounded feedback outbox. Review-2 wave 1: the shared layer carries typed
contracts and no I/O, the extension registry fails closed, and the bridge
loses the mTLS provider and every unbound trust path. Review-2 wave 2: infra
honours `verify-full` TLS, reports foreign-key drift as a typed outcome, holds
advisory locks and circuit-breaker probes as RAII guards and injects the
governance engine; the agent domain binds tasks to their owner and owns its
stream pipeline; MCP runs authz on proxied identities; AI refuses unpriced
models; and the headline security defects (OAuth open redirect, MCP anonymous
fallback, unthrottled gateway, anonymous OTLP writes, `spexec_` scoping, JWT
lifetimes, ID-JAG `email_verified`, reusable approvals, CLI double execution,
`admin config` overwrite) are closed.

### Breaking

- **Gateway:** `POST /v1/auth/bridge/mtls` and the `mtls` entry in `GET /v1/auth/bridge/capabilities` are removed; the bridge has no device-certificate provider. `POST /v1/otel` and `/v1/otel/{rest}` require a gateway credential (bridge JWT, API key or execution capability) and an attested `x-session-id`. The whole `/v1/*` group is rate-limited by the new profile field `rate_limits.gateway_per_second` (default `100`, env `RATE_LIMIT_GATEWAY_PER_SECOND`, `admin config rate-limits`).
- **Bridge:** the `mtls` credential source (`[mtls] cert_keystore_ref`, `<PREFIX>_DEVICE_CERT*`), `[sync] pinned_pubkey` / `<PREFIX>_POLICY_PUBKEY`, `SP_BRIDGE_OPENCODE_MANAGED_DIR` and every pre-0.49 on-disk migration are removed; a bridge older than 0.49 upgrades through 0.52 first. `hooks.json` and the Claude Desktop managed preferences carry derived per-plugin `hook:<plugin_id>` / `host:claude-desktop` tokens instead of the loopback secret — re-run `sync` after upgrading. `/otel*` is forwarded only for the raw loopback secret. Public API: `sync::run_once(bridge, &SyncOptions)`, `GatewayClient::*` and `update::*` take `&BearerToken`, `ProfileState::classify(ProfileProbe)`, `HostFailure`/`HostWarning` carry `host_id`, `mdm::policy::PolicyInputs` takes `host_token` and returns `Result`. Full list in `bin/bridge/CHANGELOG.md`.
- **OAuth:** `JwtConfig.expires_in` is a `chrono::Duration` in seconds (was `expires_in_hours: Option<i64>`); `IdJagGrant` / `EnterprisePrincipal` carry `email_verified`. `SessionCreationService::new` takes a `SessionProvider` (was `AnalyticsProvider`) and `with_event_publisher` is removed; `issue_bridge_access(repo, &dyn SessionProvider, BridgeAccessRequest)` (built with `BridgeAccessRequest::bridge(headers, ip, &user_id)`) replaces `issue_bridge_access_with`, and the code exchange takes a `BridgeExchangeRequest`.
- **Analytics:** `AnalyticsRepositories::new(db, SessionStore, AnalyticsEventStore, ContentCatalogStats)` — the users, logging and content owners are injected; `FeedbackSnapshotsRepository::new(pool, FeedbackFactsRepository)` takes the facts repository. Session lifecycle (`SessionRepository`, `UsersAiSessionProvider`) lives in `systemprompt-users`, `SessionCleanupService` is gone (the `cleanup_inactive_sessions` job runs on the users `SessionRepository`), and reports read `analytics_report_*` projections that must be initialised (`analytics projection rebuild`) before a report command answers.
- **Users:** the extension now depends on `events` (reporting capture rides the outbox); `UserRepository::merge` runs `begin_user_privacy()` / `finish_user_privacy()` so a merge or deletion waits for pending reporting evidence.
- **Evaluation:** the unreleased migration slots are renumbered — `013_campaigns`, `014_campaign_completion`, `015_suggestion_operations` (previously 013/015/016 with 014 skipped). A database that applied the unreleased 015/016 slots must be reset or re-stamped before upgrading; released databases are unaffected. `ManagedWorkspaceReference::manifest` is a typed `RevisionBundle`.
- **Database:** `Database::connect(read_url, write_url, &pool_config)` replaces `from_config*` (no `db_type`); `DatabaseProvider::get_postgres_pool` returns `Arc<PgPool>` and `is_postgres` is gone; `CircuitBreaker::acquire` returns an RAII `Probe` and `ResilienceGuard::admit()` replaces `acquire_permit`; `install_extension_schemas*` return a `SchemaInstallReport`; `CleanupRepository` is removed (log retention on `LoggingRepository`, OAuth sweeps on `OauthCleanupRepository` behind the `oauth_cleanup` job); `Migration::checksum` is xxh64 and rows stamped with the historical digest are transitioned in place.
- **Security:** `GovernanceEngine::global()` is removed — the engine is built once from the services root and reached through `AppContext::governance()`; `secret_scan` in `enforce` mode with no patterns, a malformed `require_approval` rule or an unknown `exempt_scopes` value is `ConfigRejected`. `DecisionAudit::context_id` and `ApprovalRequest::session_id` are typed. `RoleDirectory` / `register_role_directory!` replaces the ingestion service's own `users` query.
- **Events:** `EventRouter::route_*` return `RouteOutcome { local, relay }`; `PostgresEventBridge::start` returns an `EventBridgeHandle` (no process-global `is_listening()`); `GenericBroadcaster::connected_users` is keyed by `UserId`.
- **Logging:** the `trace` module (`TraceQueryService`, `AiTraceService`) moves to `systemprompt_runtime::trace`; `delete_orphaned_logs` / `count_orphaned_logs` become `distinct_log_user_ids` + `delete_logs_for_users` composed with `UserRepository::missing_ids`; `with_terminal` / `with_database` are removed.
- **Shared layer:** `systemprompt_models` no longer performs I/O — the subprocess supervisor is `systemprompt_loader::subprocess`, the SSRF-guarded client `systemprompt_client::guarded`, `AppPaths` and the path resolvers `systemprompt_config::paths`. `ProviderError` / `ProviderResult` become `AiInferenceError` and `McpRegistryError`; `CanonicalRequest::new(model, messages, max_tokens)` replaces `Default`; `RevisionBundle` and its closure types live in `systemprompt_models::managed`; validated identifiers drop `new` / `new_unchecked` in favour of `try_new` and named constructors (`ContextId::generate`, `AgentName::system`, `Email::local_admin`, `LocaleCode::english`); `ActorKind::Agent` carries an `AgentId`; `ToolProvider::list_tools` returns `ToolInventory { tools, failed_servers }`; `ToolDefinition::service_id` is a `McpServerId`; the unimplemented `llm` seam, `Module` / `Service` / scheduler traits, event publishers and the traits `web` feature are removed; `ExtensionRegistry::enabled_*` return `Result` and refuse to disable a required extension or a dependency; `Extension::llm_providers`, `discover_and_merge`, `HasHttpClient`, `CapabilityContext` and `FullContext` are gone; `ExtensionContext::system_owner_id()` is required.
- **Agent:** `A2ARepositories::new(db, A2aDependencies { .. })` requires the managed-skill resolver and the mcp `ToolExecutionLookup`; `AgentState::new` takes a `DynWebhookBroadcaster` (the process-global broadcaster is gone); `MessageProcessor`, `SkillService`, `ArtifactPublishingService`, `WebhookService` constructors change; `process_message_stream` returns an owned `MessageStream`; `Server::run(shutdown)`; `ContextRepository::ensure_legacy_context` replaces `ensure_system_context`; the POST router carries an 8 MiB body limit. Full list in `crates/domain/agent/CHANGELOG.md`.
- **MCP:** `McpOrchestrator::new` no longer takes a pool and the SQLite-flavoured schema validator is gone; `fetch_external_bearer` requires the broker secret; `HttpClientWithContext::{new, forwarding, external}` are fallible; `try_proxy_verified_auth` returns `Option<AuthenticatedRequestContext>`; port probes are `async`; `UiRenderer::render` is synchronous.
- **AI:** `AiProvider::get_pricing` / `catalog_pricing` return `Option<ModelPricing>` and an unpriced model is `AiError::UnknownModel`; `ImageProvider::cost_per_image_cents` is a trait method; `synthesize_or_fallback` returns `SynthesisOutcome`; `insert_with_id` reports `RepositoryError::AlreadyExists`.
- **Config / Cloud / Loader:** `init_config` / `init_config_from_profile` are removed (`try_init_config` only); `SessionStore::load_or_reset` is removed and a corrupt session index is `SessionStoreCorrupted`; the loader's `expose-internals` feature is removed.
...
- **Runtime:** `TraceQueryService::list_audit_messages` / `list_audit_tool_calls` take an `AuditPage` (`AuditPage::ALL` for the previous behaviour). **Analytics:** `RequestAnalyticsRepository::list_requests(start, end, &RequestListFilter)` replaces the `(limit, model)` arguments; `ConversationAnalyticsRepository::{list_agent_contexts, list_gateway_sessions}` take a trailing `user: Option<&UserId>`; `CostUserBreakdownRow::user_id` is a `UserId`; `ConversationListRow` and `GatewaySessionListRow` gain `user_id`. **CLI:** `infra logs audit` no longer includes messages or tool calls unless asked.

### Added

- **Plugin dependencies and Node packages:** a plugin config carries `dependencies` in Claude Code's `plugin.json` vocabulary (bare name or `{name, marketplace, version}`), a marketplace carries `allow_cross_marketplace_dependencies_on` and `external_marketplaces` (`github` / `git` sources), and services validation refuses a dependency on a marketplace that is neither allowlisted nor declared. The bundle ships a plugin's `package.json` with its accepted lockfile and writes `dependencies` into `plugin.json`; the importer round-trips both. The bridge runs the frozen, script-less Node install Claude Code would (`npm ci --ignore-scripts` / `bun install --frozen-lockfile --ignore-scripts`, 60 s, warning on failure), writes `allowCrossMarketplaceDependenciesOn` into the mirrored `marketplace.json`, registers external marketplaces in `extraKnownMarketplaces`, enables foreign dependencies at user scope and records every such key in its sidecar so uninstall takes back exactly what it wrote.
- **Evaluation:** organisational optimisation campaigns — durable policy with immutable experiments, source-change provenance, holdout proposals reviewed independently of execution admission, retained diagnostics for blocked work, development-only suggestions with atomic reservations, and a human approval lifecycle (`/api/v1/campaigns`, `/campaign-runs`, `/campaigns/{id}/holdout-proposals`, `/campaign-diagnostics`, `/evaluation-approvals/{id}/decisions`, `/evaluation-suggestions`, `/experiments/{id}/cancellation`). Migrations 013–015.
- **Evaluation:** a versioned, fail-closed evaluator capability registry (Claude Code, OpenCode, Codex, Hermes, Claude Desktop) and reviewed native proofs with immutable image identities; a native target stays unavailable until both isolation and gateway-metering proofs are retained. Execution admission is rechecked before every claim and spend reservation.
- **Evaluator:** native adapters for Claude Code, Codex (`codex-native-v1`, Codex 0.154.0), OpenCode (`opencode-native-v2`, OpenCode 1.18.29) and Hermes (`hermes-native-v1`, Hermes 0.21.3) under one container supervisor that verifies retained image and executable digests, applies network and resource limits, runs each client behind an authenticated loopback relay with a per-run credential, captures raw evidence and confirms cleanup. Gateway requests remain the accounting authority; native token fields are advisory. Pinned client checks run without credentials, mounts or networking.
- **Marketplace:** consumer installation evidence — device-scoped credentials issued by an administrator (`POST /api/v1/consumer-devices/{id}/credential`, revoked by `/revocation`), bridge enrollment (`POST /api/v1/consumer-devices/enrollment`), installation receipts compared file-by-file against the retained publication bundle (`POST /api/v1/consumer/receipts`), session bindings and immutable invocation evidence with correctable resource attribution (`/consumer/session-bindings`, `/consumer/invocations`), consumer grants (`POST /api/v1/resources/{id}/consumer-grants`) and deterministic host installation plans (`GET /api/v1/consumer/resources/{resource}/publications/{publication}/bundle?host=`). Migrations 003–008.
- **Marketplace:** complete server-side Git verification of immutable dependency closures with source-scoped bearer credentials resolved by the runtime `GitSourceOrchestrator` (cleared environment, no redirects or submodules, bounded output, deadlines and disk), administrative verification bindings (`POST /api/v1/sources/{id}/verification-bindings`) and retained verification manifests (`POST /api/v1/source-verifications`, `GET /api/v1/source-verifications/{id}`). Publication attestations bind evaluation measurements to the verified source.
- **Marketplace:** canonical configured-and-managed inventory with explicit adoption, observed membership, retained baselines and captures, Git bindings and reconciliation conflicts (`/api/v1/inventory*`), refreshed by the `managed_inventory_refresh` job at startup and after catalog changes; installation coverage cached per host. Durable fenced administrative operations retain typed input checkpoints and are addressed by `Idempotency-Key` (`GET /api/v1/operations/{id}`); publication review history and distribution evidence are retained (`GET /api/v1/resources/{id}/publications`).
- **API:** the feedback contract surface is served as OpenAPI 3.1 at `GET /api/v1/openapi.json`; every route under it answers `application/problem+json`, bounds path identifiers, cursors and JSON bodies (1 MiB, 32 levels, 512-byte ids), and refuses a cookie-authenticated mutation whose `Origin` does not match `api_external_url`. Snapshot analytics: `GET /api/v1/analytics/snapshots[/portfolio|/{resource}]`, bounded custom-range jobs (`POST /api/v1/analytics/jobs`), a durable generation stream (`GET /api/v1/analytics/live`) and `GET /api/v1/analytics/status`.
- **Analytics:** durable normalised facts for skill analytics (invocation, request, assessment and resource-association facts with source-qualified deduplication keys, monotonic revisions, corrections and tombstones; leased processing with owner checkpoints; idempotent backfill pages) processed by the `feedback_facts_processing` job; fenced daily aggregate snapshots processed by `feedback_snapshot_processing`; resource metrics that count each request and assessed conversation once per cohort.
- **Analytics:** reporting projections fed by source transactions through the event outbox (see Changed) with `analytics projection status|sync|rebuild`.
- **Events:** opt-in transactional facts and durable consumer acknowledgement in the existing SSE outbox (`services::durable::DurableOutbox`). Pending facts survive cleanup; existing routing and SSE payloads retain their contracts. Apply events migration 004 and upgrade all relays before enabling durable producers.
- **Bridge:** `device-enroll --token-file <file>` verifies an administrator-issued device credential against the gateway and stores it 0600 (Windows: verified private ACL) with the consumer, device and gateway identity; installed bundles are verified by readback (native `SKILL.md`, every active file, retained source, Unix modes) and acknowledged through a bounded per-identity feedback outbox (512 receipts, 512 pending plans, 16 MiB) that survives restarts and never evicts pending evidence; `feedback-status` reports acknowledged, unacknowledged, verified and superseded states. Native hook evidence and session observation (Claude session UUIDs, Codex thread metadata, OpenCode and Hermes session ids) are authenticated with the enrolled device; the governance owner gets an OpenCode hook plugin carrying a scoped token; the Claude Code model picker is seeded from the bridge profile after a sync.
- **Bridge:** `login --stdin`; `sync --watch --interval <secs>`; per-route credential classes and derived scoped tokens (`proxy::credential`, `proxy::scoped_token`); `ProxyRole::Failed(ProxyFailure)`; fail-closed `AutoUpdateDecision`; MCP sidecar records; private generated host profiles; `.draining` inbox recovery; per-host probe sequencing. Full list in `bin/bridge/CHANGELOG.md`.
- **Users:** `SessionRepository` / `UsersAiSessionProvider` implement the shared `SessionStore` / `SessionProvider` contracts; `UserRepository::missing_ids`; `UsersRoleDirectory`; SQL device-eligibility interface (`active_device_identity`, `active_devices_for_consumer`, migration 012); retention barrier and privacy delivery functions (migrations 013–015).
- **Shared traits:** `SessionProvider`, `SessionStore`, `AnalyticsEventStore`, `ContentCatalogStats`, `OwnerReassignment` (implemented by agent, ai and mcp for cross-domain user merges) and `ToolExecutionLookup`. **Shared models:** `feedback::{receipts, inventory, verification, analytics}` contracts with `validate()`, `EvaluatorClient`, `managed::{RevisionBundle, RevisionManifest, RevisionFiles, …}` with `same_content`, `rate_limits.gateway_per_second`, a hand-written `Config` `Debug` that redacts URLs and tokens, xxh64 migration checksums.
- **Runtime:** `AppContext::governance()`, `schema_install()`, `feedback_facts_repository()`, `feedback_snapshots_repository()`; `runtime::reporting::{spawn, initialize, rebuild, process_pending, status}`; `runtime::optimization` (source verification, attestation, holdout, inventory and Git credential resolution); `runtime::trace`.
- **Database:** `BootstrapLockGuard` and `BOOTSTRAP_ADVISORY_LOCK_KEY` are public; the `services` process-registry table is declared by the database extension.
- **Config:** `VaultError::Body`; `paths` module. **Loader:** `subprocess` module. **Client:** `guarded` module. **Identifiers:** `AiToolCallId::generate()`.
...
- **CLI:** `infra logs request list --until <time>` and `--before <cursor>`; every row carries a `cursor` (`<created_at RFC3339>@<request_id>`) in JSON output and `--before` returns strictly older rows, keyset on `(created_at, id)`. `RequestCursor` (`FromStr`, refusing a malformed value with a `RequestCursorError`) and `AiRequestFilter::{with_until, with_before}` in `systemprompt-runtime`; a malformed `--before` is a usage error, never an empty page.
- **CLI:** `infra logs audit` is counts-only by default (`message_count`, `tool_call_count`) and takes `--messages` / `--tools` to include the rows, `--offset` / `--limit` (0 = all) to page them and `--max-content <chars>` to bound each body; the card reports `offset` and `has_more`; a negative `--offset` / `--limit` is refused at parse time, as is a non-positive `analytics conversations list --limit` or a negative `analytics requests list --offset`. `TraceQueryService::{count_audit_messages, count_audit_tool_calls}` and an `AuditPage` on the two list queries.
...
- **Analytics:** reporting queries use analytics-owned `analytics_report_*` projections fed by source transactions (users, agent, AI, MCP, content, logging) through the existing outbox on a `reporting` channel the SSE bridge skips; capture views and triggers are installed through each owning extension's schema (`reporting_capture.sql`, `reporting_privacy.sql`). Runtime initialises the baseline and processes pending facts on an owned worker (256 deliveries per pass, once per second); `analytics projection status`, `sync` and `rebuild` expose maintenance; rebuild blocks source writes while taking its snapshot and a report command refuses an uninitialised baseline. Session lifecycle and usage move to users, event ingestion to logging, authoritative public-page counts to content; reporting is eventually consistent. Mixed-version rolling operation is not supported: stop old replicas, migrate, then start the upgraded ones.
- **Analytics:** retention erases evidence only behind committed producer, fact and snapshot barriers; a user deletion or merge takes the retention lock (`lock_user_deletion_for_retention`), delivers pending reporting evidence atomically and advances the projection `evidence_cutoff` before identity is removed. Stream cursors are canonical digit-only strings. Every analytics table is declared by its own schema file.
- **Database:** the declarative-schema installer splits `FOREIGN KEY` constraints out of every `CREATE TABLE` and applies them in a final phase after every extension's migrations and dependent DDL, as `ALTER TABLE … ADD CONSTRAINT`, so an established database whose referenced `UNIQUE`/`PRIMARY KEY` only arrives through a migration no longer fails at boot with "no unique constraint matching given keys" while fresh installs pass. The phase is idempotent: a key is skipped when `pg_constraint` already holds a foreign key on the same constrained and referenced columns, whatever its name; unnamed keys get Postgres's own `<table>_<col>[_<col>…]_fkey`. A missing key is added `NOT VALID` and validated in a savepoint — rows that violate it leave the key `NOT VALID` with a warning. On an established database only the `ADD CONSTRAINT` itself may be recorded as drift (`SchemaInstallReport::foreign_key_drift`, which fails `infra db migrate` and names each key); any other failure in the phase fails the install. The schema linter rejects a foreign key whose referenced table is declared in the same extension unless that table declares a `PRIMARY KEY` or `UNIQUE` on exactly the referenced columns, lints an extension's schema files as one graph (`lint_declarative_schemas`), returns its warnings instead of dropping them, compares identifiers exactly and skips dollar-quoted bodies. Repeated schema installation is idempotent.
- **Database:** a database URL carrying `sslmode=verify-full` / `verify-ca` (and `sslrootcert`) is honoured instead of being downgraded to `prefer`; `SqlExecutor` splits statements with `pg_query::split_with_parser`; `AdminSql::parse_readonly` parses with `pg_query`, refuses data-modifying CTEs, DDL and utility statements, and runs in a `READ ONLY` transaction; `infra db query` decodes `uuid`, `numeric` and `bytea`; column introspection is scoped to `public`; `DatabaseHandle::is_connected` reports both pools. An applied migration whose stored checksum is the historical `DefaultHasher` digest of the current SQL is rewritten to xxh64 in one verified transaction — never executed — and a malformed `extension_migrations` row fails the run. Evaluation and marketplace declare every table they create in its own schema file, so `infra db doctor` reports none as undeclared.
- **Security:** authz ingestion reads the roles YAML through `tokio::fs` and fails the reconcile on an unreadable path; the first loaded signing key wins; a credential token response whose body cannot be read is retryable. `ClaimedAgent` / `PrincipalSnapshot::claimed` are removed.
- **Events:** a slow SSE consumer whose channel is full has its stream closed instead of being dropped from fan-out while still receiving heartbeats; `ConnectionGuard` unregisters synchronously; `/health` reports the relay as `not_started`, `reconnecting` or `stopped` from the handle and shutdown cancels and joins it.
- **Agent:** `GetTask` / `CancelTask` are bound to the caller (`-32001` for another owner's task, `-32002` for a terminal one) and `CancelTask` cancels the running pipeline; a task is completed in the same transaction as its messages and announced `Failed` exactly once; every stream emits exactly one `final: true` frame; bounded backpressure stops the pipeline when the consumer closes instead of truncating a task; a withheld managed skill or a failed history load fails the request; artifact publishing verifies `mcp_execution_id` through `ToolExecutionLookup`; one row→`Part` converter on every read path; `check_a2a_agent_health` validates the served `AgentCard`; CORS comes from `cors_allowed_origins`; agent-card `securitySchemes` / `security` are typed and a malformed block fails config load.
- **MCP:** failed-auth diagnostics no longer log the `authorization` header; a proxy-verified identity runs the per-server authz hook like a JWT identity; `list_tools` reports servers that could not be listed in `ToolInventory::failed_servers` (AI tool discovery fails on an incomplete inventory); a tool call without `x-user-id` is `AuthorizationFailed` rather than running as the server owner; `has_server_permission` withholds an unknown server; `ServiceFailed` writes the shared `error` status so crashed services are reaped; renderers only render `http(s)` / `data:` media URLs; `ToolUsageRepository` no longer writes the agent-owned `user_contexts` table.
- **AI:** a provider whose `api_key` secret is missing is withheld from the registry and named in the boot error when it is the default; `ResilientProvider` settles the breaker through the RAII probe; `is_available` reports circuit state in `health_check`; `SchemaValidator` rejects an uninterpretable `type` keyword; a stream dropped by its consumer settles a `failed` row with the usage so far; the `ai` crate no longer reads `mcp_tool_executions`.
- **Gateway:** an accounting failure is retained on the `ai_requests` row (`accounting_failed_at`, `accounting_error`, migration 024) and never replaces settled spend; an upstream error body that cannot be read is recorded as `<unreadable body: …>`; a deployment that registers request guards but has no pool denies with `503` (`GatewayDenyKind::Unavailable`); the gateway and the external-MCP proxy evaluate governance through the `AppContext` engine; the cross-replica event bridge always starts.
- **API:** manifest and plugin-file routes read through the context-owned `MarketplaceCache`; the manifest's `issued_at` / `not_before` / `min_bridge_version` are typed; `GET /bridge/profile` answers 503 and `POST /admin/services/refresh` 500 when the secrets store is not initialised; extension routers that declare a reserved `/api/` prefix are refused at mount time; the Slack reply pipeline is its own module.
- **Marketplace:** a managed MCP server declared without `tool_policy` is withheld from the signed bridge manifest and reported as `mcp_servers.<name>.tool_policy` at startup; organization publications enforce consumer grants; delivery identity is fenced and reconciliation state retained; managed skills resolve for the requesting user; Git credentials are redacted in diagnostic request types; Git verification compares trees through `RevisionFiles::same_content`.
- **Evaluation:** an approved privileged operation is consumed on first authorisation (`status='consumed'`) and a second `authorize_operation` is a conflict; execution claims order by `variant_index` and `repetition` within a creation instant; `execution_accounting` rejects negative counts; campaign eligibility requires every frozen execution pair; workspace inspection is bounded and unsafe materialisation paths are rejected; terminal evidence is bound to the frozen workspace configuration; startup diagnostics are retained behind cleanup fences.
- **Scheduler:** `database_cleanup` sweeps only `logs` by composing `LoggingRepository::distinct_log_user_ids` with `UserRepository::missing_ids`; a `JobLockGuard` dropped without `release` closes its session; a failed evaluator workspace cleanup is logged with its path.
- **Runtime:** `AppContext` and `DatabaseContext` connect through `Database::connect`; the composition root builds `A2ARepositories` with `A2aDependencies` and re-homes the legacy context onto the current system admin through `ensure_legacy_context`; construction fails when the governance audit sink cannot obtain the write pool.
- **CLI:** `infra db migrate` fails, after committing everything else, when a declared foreign key could not be created on an established database; `admin session login` refuses a corrupt session index; `infra logs delete` / `cleanup` are mutating and require an explicit cloud profile; `admin config validate` fails when a section cannot be enumerated; `cloud init` / `cloud profile` and `admin config catalog discovery` fail when the embedded catalog or rate card is unreadable; `admin agents validate` fails when secrets are not initialised; `core files upload --context` / `core artifacts list --context` validate the id; `infra logs` reads traces through `systemprompt_runtime::trace`; `infra migrations history` prints `unstamped`.
- **Config:** with the `env` secrets source only an absent secrets file falls back to the environment; a malformed one is `InvalidSecretsFile`.
- **Loader:** catalog discovery propagates an unreadable listing body; `ServicesRootBootstrap::install` returns the root that won.
- **Shared models:** `validate_outbound_url` / `is_blocked_ip` additionally refuse `0.0.0.0/8`, `192.0.0.0/24`, `198.18.0.0/15`, `224.0.0.0/4`, `240.0.0.0/4`, `ff00::/8`, NAT64 `64:ff9b::/96` and IPv4-compatible `::a.b.c.d`; `Profile::is_local_trial` is `false` for a cloud target without a `cloud` section; a malformed `x-user-permissions`, `x-context-id` or `x-call-source` header is `ContextPropagationError::InvalidHeader`; an unparseable Responses or Gemini SSE frame yields an `Err` item instead of being skipped.
- **Identifiers:** `define_token!` types redact their `Debug` output like `Display`, and `redacted()` slices on character boundaries.
...
- **Security (MCP):** an MCP request carrying an invalid, revoked or orphaned bearer is refused with 401 instead of falling through to the anonymous session context; only a request with no `Authorization` header reaches the RFC 9728 challenge.
- **Security (gateway):** the `/v1/*` group is rate-limited; anonymous OTLP writes answer 401 instead of being stored.
- **Security (capabilities):** an execution capability (`spexec_`) is scoped against the original request path, so the nested `/api/v1/mcp/{server}/mcp` route admits it (it was always 403); the scope check runs before the capability is verified.
- **Security (OAuth):** token lifetimes are computed in seconds — a `jwt_access_token_expiration` below 3600 no longer mints an already-expired anonymous JWT and a bridge TTL under an hour no longer rounds up to one. An ID-JAG `email` claim is no longer treated as verified by its presence; `email_verified` is carried verbatim (default `false`), so a token naming an existing account's email creates a fresh federated user instead of linking to it.
- **Security (evaluation):** approvals are single-use.
- **Security (bridge):** the proxy binds only on its advertised loopback address and an occupied advertised port refuses to start instead of binding elsewhere; `plugin_oauth::store_creds` writes the keystore before the metadata names it; the macOS MDM policy writer and every sync writer mint the loopback secret; `open_target` no longer runs `cmd /C start` with an untrusted URL; retained error sources are sanitised.
- **Security (cloud):** the session index and per-session files are created owner-only (0600) from the first byte and published by rename; a corrupt read never overwrites every tenant's session.
- **Security (gateway):** `GET /bridge/manifest` answers 500 and signs nothing when the revocation list or the enabled-host preferences cannot be read, instead of serving a manifest with no revocations and every instance host enabled.
- **Scheduler:** `feedback_snapshot_processing` and `feedback_facts_processing` process the system admin's queues — the owner the analytics routes read under — whatever actor the schedule is configured with; a failed range job is logged and the pass continues. A judgment that cannot be scored is logged; a malformed managed `config.yaml` fails workspace materialisation; a lost-lease cleanup names the step that failed.
- **API:** device-credential consumer routes keep the 5xx classification of a storage failure and log profile and services-config failures; campaign setup diagnostics carry the code derived from the failure; the snapshot stream owns its forwarder and the `feedback_snapshots` relay lives on the `AppContext` (`Subsystems.snapshot_wakeup`) and is joined on shutdown, as does the inventory publish memo (`Subsystems.publish_guard`).
- **Runtime / Events:** optimization source errors carry their cause; the reporting projector claims through the new `OutboxConsumer` instead of a `DurableOutbox` stamped with a fabricated instance id.
- **CLI:** a command routed to a remote tenant runs exactly once; the runner no longer dispatches it a second time locally after the remote run succeeds. `admin config catalog|gateway` leave `services/config/config.yaml` untouched and report the error when the file cannot be read instead of overwriting it with a bare `includes:` block.
- **Evaluation:** migration 012 creates `UNIQUE(owner_id,operation_key)` on `eval_budget_accounts` and `UNIQUE(owner_id,id)` on `eval_experiments` wherever no unique index covers those columns: a 0.51.0 fresh install stamped migration 004 without running it, and the second never had a migration. Migration 011 adds the `UNIQUE(owner_id,id)` index on `eval_budget_accounts` that the declarative schema has declared since shared budget accounts landed but no migration ever gave an established database. Both were found by a downstream upgrade gate that diffs an upgraded database against a fresh install; the deferred foreign-key phase creates the owner-paired keys on the next boot.
- **Evaluator:** OpenCode terminal-runner evidence is required and denied reviewer edit capabilities are suppressed; Codex path permissions are encoded as TOML tables, a read-only pinned Codex runtime is permitted and the explicitly scoped fixture tool is allowed; Hermes credential isolation is verified and SQL aliases are bounded; the adapter registry is static; a single-use lifetime that failed the MSRV check under `-D warnings` is elided.
- **Runtime:** boot no longer fails with `Context … not found for user` after the system admin changes; construction fails on a missing audit pool instead of installing a null audit sink.
- **Database:** a `BootstrapLockGuard` dropped without `release` closes its session so the advisory lock cannot survive in the pool; a cancelled circuit-breaker probe releases its half-open slot; a `DEFERRABLE` after a trailing `UNIQUE` stays on that constraint.
- **Events:** `/health` no longer reports a listening relay before the bridge has started.
- **AI:** `get_user_usage` returns zero for a user with no requests; Anthropic `generate_with_schema` fails on a response without a `tool_use` block; image generation stores `cost_microdollars` from the per-image cents and the decoded byte size; the first synthesis failure is logged before retrying.
- **Files / Teams / Cloud / Config / Storage:** a file whose metadata cannot be serialised is an error instead of `null`; unreadable error bodies carry an `<unreadable body: …>` marker; `LocalFileStorage::store` stages and renames so a reader on another replica never sees a truncated file.
- **Extension:** extension routers are validated before mounting — a base path is the web root `/` or nested under `/api/` outside the reserved prefixes; a duplicate `register_extension!` id is `DuplicateExtension`; `discover()` fails on a missing dependency.
...
- **CLI:** `--json` / `--yaml` (and `SYSTEMPROMPT_OUTPUT_FORMAT`) now emit the artifact for a command whose result was flagged terminal-skip — every empty-result path, 66 sites — instead of printing nothing. An MCP wrapper reading stdout saw `""` where it should have seen an empty table. `skip_render` suppresses terminal rendering only, as documented. `admin config validate --schema`, `admin session login --token-only` and `admin keys issue --token-only` return their payload as the text artifact instead of printing it beside a placeholder card, so `--json` carries one document.

### Removed

- **Bridge:** `auth/keystore`, `auth/providers/mtls`, `config/trust/legacy`, `install/proxy_service`, `integration/claude_desktop/gateway_probe`, `integration/stub_host`, the `dev-stub-host` feature and every test-only seam (`#[doc(hidden)]` items, `UninstallSummaryBuilder`, `unique_stem`).
- **Agent:** `models::web`, `services::mcp::{task_helper, tool_result_handler}`, `PersistenceService`, `ConversationService`, `SkillInjector`, the mock/multiturn task builders, `execute_tools_sequentially`, `validate_agent_token`, `get_user_context`, `services::shared::{auth, config, resilience, slug}`, `services::external_integrations`, `classify_database_error`, `monitor::{check_agent_health, check_agent_responsiveness}`, `ExecutionStepRepository::mcp_execution_id_exists`.
- **MCP:** `ToolUsageRepository::{update_context_timestamp, find_context_id, list_tool_stats}`, `models::ToolStats`, `services::schema`, `find_available_port`, `cleanup_port_resources`.
- **AI:** `AiRequestRepository::link_tool_calls_to_recent_executions`.
- **Shared:** `McpProvider`, `ServiceLifecycle`, `a2a::AgentAuthentication`, struct-literal construction of `CreateSessionInput` / `InsertAiFileParams` / `ContextWithStats`, the `execution::context` publisher hooks, `LlmProvider` and the `llm` module, `Module` / `ApiModule` / `ModuleRegistry` / `register_module!` / `Service` / `AsyncService` / `traits::scheduler`, `LogEventPublisher` / `UserEventPublisher` / `AnalyticsEventPublisher`, `OptionalStartupEventExt`, `DatabaseProvider::fetch_scalar_value`, `SqlExecutor::table_exists` / `column_exists`, the `_raw` transaction twins, `ProviderError` / `ProviderResult`.
- **Security:** `GovernanceEngine::global()`, `ClaimedAgent`, `PrincipalSnapshot::claimed`.
- **Analytics:** `SessionCleanupService`, the analytics-owned session mutations and `ai_provider` (moved to users).
- **CLI:** `plugins capabilities llm-providers`.
...
- **Evaluation:** the standalone judge-run surface is gone — `admin evals run|replay|list|show`, the `eval_runs`, `eval_results`, `eval_pairs`, `eval_judge_calls` and `eval_rubrics` tables (dropped by migration `010`) and their repositories and identifiers. Every paid evaluation is a supervised experiment; `admin evals promote` remains. Agents resolve managed skills through the `ManagedSkillResolver` seam: a published managed skill replaces its disk copy and a managed key that is not published is withheld — never served from disk.
...
- **Evaluator:** the supervisor is configured by `profile.evaluator` (`worker_id`, `client_image`, `relay_image`, `control_network`, `docker`, `workspace_root`) instead of `SYSTEMPROMPT_EVALUATOR_*` process variables; each execution runs on a verified internal Docker network that is torn down with the lease.
...
- An execution whose lease expired before its requests settled left their budget reservations held forever; restart reconciliation now settles them at the recorded cost, or the full reserved bound when the provider never reported usage.
- Gateway receipts live on the replica that wrote them, so a replaced machine could never settle its in-flight requests and they stayed `pending`. The recovery task fails any `pending` row older than one hour with an explicit unknown-usage reason.
...
- **Marketplace:** rules — markdown instruction fragments an organisation ships to its agents. A `rules/<id>/` directory with a `config.yaml` and an `index.md` under the services tree is loaded into a signed `RuleEntry` on the bridge manifest, selected per plugin by `PluginConfig.rules`, laid out in the bundle as `rules/<kebab-id>.md`, and written back from a Claude Code `rules/*.md` tree by `core marketplace import`. New `RuleId`, `RuleName` and `DiskRuleConfig`. `SignedManifest.rules` is omitted when empty, so an older bridge reading a manifest without rules is unaffected.
...
- **Gateway:** `gateway.quota_fault_mode` (`open`, the default, or `closed`) selects what happens when the gateway cannot evaluate a quota window or read its policy row. It lives in file config rather than in the policy spec because one of the faults it governs is the failure to read that spec. `QuotaFaultMode` in models.
- **AI:** `ai_requests.upstream_latency_ms` with migration 022, bracketing the provider call alone so gateway overhead is `latency_ms` minus it. The bracket closes when the provider response is fully received and never reopens. It is NULL for a request that never reached a provider and for every row written before the migration; on `AiRequest` as `upstream_latency_ms`.
- **Marketplace:** rules — markdown instruction fragments an organisation ships to its agents. A `rules/<id>/` directory with a `config.yaml` under the services tree is loaded into a signed `RuleEntry` on the bridge manifest, selected per plugin by `PluginConfig.rules`, and laid out in the bundle as `rules/<kebab-id>.md`. New `MarketplaceRuleId`, `RuleId`, `RuleName`, `DiskRuleConfig`, `RuleSummary` and `RuleDetail`. `SignedManifest.rules` is omitted when empty, so an older bridge reading a manifest without rules is unaffected.
- **Bridge:** rules are a marketplace category in the GUI listing, counted in the sync summary line and in the `last-sync.json` sentinel.
- **Models:** `net::guarded_client` builds an outbound HTTP client whose DNS resolver re-checks every resolved address against the SSRF block list before a socket is opened — on the first request and on each redirect hop — and whose redirect policy re-runs the parse-time guard. `GuardedClientConfig` carries the trusted-host allowance, the loopback allowance, the redirect cap and the timeouts.
- **Security:** migration 018 refuses every `UPDATE` on `governance_decisions` with a `BEFORE UPDATE` trigger. A recorded decision is evidence, so rewriting one is an audit-integrity failure; `DELETE` is deliberately left available for retention and erasure.
- **Supply chain:** `just audit` runs `cargo-audit` across every workspace lockfile with `--deny unsound`, taking its ignore list from `deny.toml` on every run so a suppression is still written in exactly one place. cargo-deny's `advisories` check does not report advisories the RustSec database marks `informational = "unsound"`, so those were invisible. Supply Chain runs it as a third job.

### Changed

- **Gateway:** a quota window that cannot be evaluated — the subject-attribute provider errored, returned no value, or is not registered — is logged with its subject and window, and under `quota_fault_mode: closed` denies the request instead of being skipped. A failed policy read behaves the same way; the permissive fallback it takes under `open` drops safety scanning as well as quota windows. A failed post-response accounting write cannot deny a request that has already been served, so under `closed` the request is recorded as failed instead.
- **CI:** the Postgres service image is `postgres:18` in CI, Coverage and Exercise Suites.
...
- **Gateway:** `QuotaWindow.max_input_tokens` and `max_output_tokens` are compared in the pre-check alongside `max_requests` and `max_cost_microdollars`. They were accumulated into the bucket and never checked, so a configured token ceiling had no effect.
- **Security:** outbound HTTP to a destination a caller can influence runs on the guarded client — the governance authz hook, JWKS fetches, OAuth client-metadata fetches, MCP transport and token-accessor calls, gateway image fetches, agent webhook delivery, Slack replies to a caller-supplied `response_url`, and Teams replies to an activity's `serviceUrl`. Parse-time validation cannot tell that a hostname resolves into a blocked range; the resolver can. A caller whose guarded client cannot be built fails the delivery rather than falling back to an unguarded one.
- **Agent:** `tasks/pushNotificationConfig/set` validates the callback URL through the shared SSRF guard and answers `-32602` instead of storing whatever it was given.
- **Users:** merging an account writes a new `governance_decisions` row naming both user ids instead of re-pointing the source user's decisions at the target. The table is append-only, and a decision records what was authorised for whom at the time.
...
- **Security (authz):** `try_gather_subject_attributes` propagates provider failures to the rule-based hook's denial path. `GatewayDenyKind::Unavailable` distinguishes unavailable decision inputs from a policy denial.
- **Bridge:** Windows startup faults identify port conflicts, unreadable loopback keys and conflicting machine policy with paths and remedies. **Remove everything** clears installation state, enrolled profiles and machine Claude policy, reporting removal failures. Diagnostics include `state.txt` with process, port, permissions and policy state; startup logs include version and commit.
...
- **Models:** `SchemaFeatures::tuple_items` controls tuple-array support. Unsupported `prefixItems`, `additionalItems` and `unevaluatedItems` become `items`, using a common prefix schema when available. Sanitization recurses into retained tuples and removes `$comment`.
...
- **Bridge manifest:** assembly combines all enabled marketplaces. `MarketplaceMembership` records plugin, agent and MCP ownership; any admitting marketplace grants membership, subject to entity and plugin denials. The additive `marketplaces` field excludes denied or empty marketplaces. Empty lists retain the `org-provisioned` fallback, and multiple marketplaces do not require a default ID.
- **Security:** a marketplace can grant access by attribute dimension. `MarketplaceAccess.rules` carries attribute grants beside `roles`; each rule names an extension subject-dimension slug and the values it admits, and is projected into `access_control_rules` alongside roles, one row per value, so the authz cascade reads both the same way. `ChainSources` is many-to-many — a set of owning marketplaces per plugin and per member — and `member_attribute_floor` merges the `access.attributes` bags of every enabled marketplace that includes the entity, in marketplace-id order with first key winning.
- **Gateway:** upstream `429` and `503` responses receive up to four total attempts before any response bytes are relayed. Backoff starts at 1s, caps at 30s, adds 25% jitter and honors longer `retry-after` values. Other and exhausted responses relay verbatim. `gateway_upstream_retries_total` records retries.
- **Gateway:** Gemini URL images are fetched and inlined with a whole-operation timeout, a streaming 5 MiB cap, a content-type allowlist and per-hop redirect validation. The fetch validates resolved DNS addresses and rejects implicit loopback access. Anthropic and OpenAI retain native URL handling.
- **Gateway:** Google secrets with `type: service_account` use RFC 7523 token exchange, per-secret caching and early expiry. Exchanged tokens use bearer authorization; API keys use `x-goog-api-key` on buffered and streaming requests.
- **AI:** buffered and streaming adapters persist reasoning usage in `ai_requests.reasoning_tokens` as a subset of output tokens. `CanonicalUsage::normalise_reasoning` normalizes additive provider reports and logs inconsistent counts. Internal AI requests, audit CLI output and analytics summaries include reasoning and cache usage.
- **Database:** `NNN_<name>.tombstone` and `NNN-MMM_<name>.tombstone` reserve retired migration numbers without executable SQL. Builds reject number reuse. Runtime identity checks return `MigrationSlotReused` with both names on a collision; `migrate-status` reports unclaimed applied versions as `orphaned`, with a nonfatal startup warning.
- **Bridge:** the Claude Code CLI emitter mirrors one marketplace per marketplace the gateway manifest lists — `~/.claude/plugins/marketplaces/<marketplace-id>/`, cache under `plugins/cache/<marketplace-id>/`, and `<plugin>@<marketplace-id>` keys in `installed_plugins.json`, `known_marketplaces.json` and the settings `enabledPlugins` / `extraKnownMarketplaces` maps. A plugin two marketplaces carry is mirrored under each. The owned marketplaces are recorded in `~/.claude/plugins/.systemprompt-marketplaces.json` and only those are pruned, so a marketplace the user registered by hand survives; the legacy single-marketplace layout is purged one-way on the first marketplace-aware sync, and a manifest listing no marketplaces still mirrors as `org-provisioned`. The managed-skills sidecar OpenCode and Hermes share records the marketplace ids too. (ships in `bridge-v0.38.0`)
- **Models:** `ProviderEntry::upstream_model_for` resolves a request's upstream model name — an operator's route override wins, otherwise the catalog's per-model mapping applies, which is the only place it can live because one route matches a whole glob. `ModelCapabilities.reasoning` declares a thinking model. `net::is_blocked_ip` exposes the SSRF block list to a caller that resolves a hostname itself.

### Changed

- **Loader:** providers with unresolved credential references are demoted to `surface: backend` before validation and omitted from model discovery. An uninitialized secret store remains an unknown state. Warnings identify the provider; `cloud doctor` reports missing secret names and checks the repository services tree outside a container.
- **CLI:** `migrate-repair` rejects slot collisions in dry-run and apply modes and directs operators to migration tombstones. `migrate-status` labels collisions; repair output reports applied counts and identifies reconciliation that executes no SQL.
- **Bridge (operator-visible):** OpenCode is offered every model the gateway can serve it. `accepted_surfaces()` declared only `OpenAi`, so the provider block offered nine models and hid all fourteen `claude-*` and all three `gemini-*` the same gateway was serving happily: `surface` governs advertisement, not dispatch, and the gateway normalises any inbound wire to canonical before rendering any provider wire outbound. `Backend` stays excluded. (ships in `bridge-v0.38.0`)
- **Security:** a marketplace access rule rejected for a malformed `rule_type` slug carries the validator's own message.
- **Marketplace:** ingestion is split into named steps and the manifest's per-marketplace plugin list is ordered by id, so the listing no longer depends on hash order.

### Fixed

- **Gateway:** Anthropic `usage.output_tokens_details.thinking_tokens` is recorded as reasoning usage for buffered, streaming and passthrough responses. It remains a subset of output usage and does not increase totals or cost; absent details yield zero.
- **Security (admin provisioning):** `get_or_create_admin` looked up, provisioned and assigned the `admin` role on whatever string reached it, so an arbitrary literal became an admin identity — on the path that mints admin-tier session tokens. It validates through `Email::try_new` before any lookup or write.
- **Security (governance):** the entropy detector exempts provider-signed reasoning fields at their defined JSON paths and, for generic field names, only within the corresponding reasoning block type. Vendor-specific credential patterns still scan those fields.
- **Gateway (cost):** cached prompt tokens were billed twice on three of four wires. OpenAI Chat, OpenAI Responses and Gemini report `cached_tokens` as a subset of the prompt count while Anthropic reports the two disjoint, and every adapter mapped them as disjoint, so the cached slice was charged at the input rate and again at the cache-read rate. `input_tokens` is exclusive of cache reads everywhere now.
- **Gateway:** `buffered_body_defect` rejects upstream 2xx responses that contain no turn with HTTP 502. The audit records a failed request and body excerpt; valid responses without text remain supported.
- **Gateway:** a reply that fails to parse is an upstream error, not a free empty turn. All four buffered parsers turned a top-level deserialisation failure into an empty response that succeeded with zero usage and logged nothing — found when a Gemini fixture lacking `content.role` billed nothing. An explicit `null` no longer blanks a completion either: Vertex MaaS sends `"tool_calls": null` on ordinary completions, and nine models returned blanks for it.
- **Gateway:** a stream that ends with no terminal event states an abort on every inbound surface — `event: error` for Anthropic callers, an error chunk closed by `[DONE]` for Chat Completions, `response.failed` for Responses, on both the translated and byte-passthrough lanes. The caller previously saw only a closed socket. Gemini's error chunk and blocked-prompt feedback both parsed as an empty success, and Chat Completions' `[DONE]` synthesised a clean `stop` over an error chunk.
- **Gateway:** a streamed Chat Completions turn reported zero usage to a client sending `stream_options.include_usage` — what OpenCode and the OpenAI SDKs send — because the codec ended the turn on `finish_reason` while this wire sends usage in a later chunk of its own. The finish reason is held until the stream states its end and the closing frames are the contract's usage-only chunk followed by `[DONE]`. Anthropic's terminal `message_delta` stated a hardcoded `output_tokens: 0`.
- **Gateway:** terminal-reason normalization reports `ToolUse` for complete tool calls and prioritizes truncation for incomplete arguments. The rule applies to buffered and streaming codecs, stream accumulation and Anthropic passthrough. Anthropic stream state tracks open tool blocks.
- **Gateway:** streamed OpenAI Chat `reasoning_content` maps to canonical thinking. Model reasoning support uses the catalog's `max_thinking_budget`. Implicit Gemini thinking omits `thinkingConfig` and adjusts only `maxOutputTokens`.
- **Gateway:** model limits resolve from the requested catalog ID and upstream names from `ProviderModel::upstream_model`. Truncated buffered Responses turns report incomplete status; Responses output omits the unsupported `stop_reason` key.
- **API:** the MCP proxy resolver's restart retry is bounded. It recursed on `attempt_restart` returning `Ok`, but starting nothing also reports `Ok`, so a crashed row that never recovered recursed until the stack was exhausted, killing the API process on a single proxied request.
- **Database:** fresh-install baseline stamps commit atomically with structural DDL. Tombstone slots bypass migration identity checks, and pending migrations preserve their `no_tx` setting.
- **CLI:** `--content-types ""` no longer writes a template bound to a single empty-string content type; the guard against an empty list could never fire because `str::split(',')` yields one empty element for an empty string.
- **Dead paths deleted rather than tested:** `validate_self_issued` re-decoded a JWT header its only caller already held, `resolve_context_id` branched on an empty `ContextId` no construction path can produce, `monitor_all_agents` re-checked a pid `list_all_agents` had already downgraded, and the bridge's `cmd_login` usage arm was unreachable by its own control flow.

### Breaking

- **Usage:** `CanonicalUsage` is the only usage type. `CapturedUsage`, `CostTokens` and the internal `TokenUsage` builder are deleted, and `ModelPricing::cost_microdollars(&CanonicalUsage)` is the single cost function for the gateway and the internal agent path alike — so the agent path bills cache tokens, and an unknown provider yields zero with a warning instead of a fabricated $1-per-million. `CanonicalUsage::billable_total` is the one definition of `tokens_used`.
- **Wire semantics:** `input_tokens` is exclusive of cache reads on every wire. Anyone reconciling historical `input_tokens` against new rows is comparing two different definitions.
- **Pricing:** `ModelPricing::cache_read_per_million` and `cache_write_per_million` are `Option<f64>`, and a gateway route that can dispatch a token-billed model declaring no cache-read rate fails validation at boot — an absent rate and a declared free rate must not look alike now that `input_tokens` excludes cache reads. Declare the provider's published rate, or an explicit `0.0`; the seeded default catalog declares one for every model.
- **Wire parsing:** every wire's `parse_response` returns `Result<_, WireParseError>`; the gateway maps a failure to a 502 with the wire named. `AnthropicStreamState::events_from_sse` replaces the stateless free `events_from_sse`.
- **Request validation:** a `tool_choice` outside a surface's grammar is rejected with `invalid_request_error` instead of being silently dropped. A string `"required"` on the Anthropic Messages surface previously returned 200 and dispatched.
- **Upstream retry:** a transient 429/503 is no longer relayed immediately; a caller that treated it as an immediate signal now sees backoff latency before the same status arrives.
- **Marketplace:** `MarketplaceService` drops `active`, `active_entry` and `resolve_active` for `enabled`; `resolve_default` survives, naming only the marketplace the rendered catalogue JSON describes. `ServicesConfig::enabled_marketplaces` returns every enabled marketplace, ordered by id.
- **Security (authz sources):** `ChainSources` is many-to-many — `marketplaces`, plus a set of owning marketplaces per plugin and per member, replace the single active marketplace — and `member_attribute_floor` returns an owned merged map. `ParentChainIndex` loads every enabled marketplace in one bulk pair of queries, so query count stays independent of catalogue size.
- **Configuration:** several enabled marketplaces no longer require `settings.default_marketplace_id`; the selector governs only which single marketplace the public `/marketplace.json` and the CLI's rendered catalogue describe, while the manifest and the authz cascade see all of them.
- **Logging / analytics rows:** `AiRequestListItem` and `AuditLookupResult` gain `reasoning_tokens: Option<i32>`; `RequestStatsRow` and `CostSummaryRow` gain `reasoning_tokens`, `cache_read_tokens` and `cache_creation_tokens`. Struct literals need the new fields. The column is nullable so rows written before it mean "not recorded", not "zero".

## [0.46.0] - 2026-09-04

### Added

- **Governance:** per-policy `mode: enforce | warn` inherits from `governance.mode`. `Decision::Warn` retains the denial reason and permits execution through `Decision::permits`; evaluation continues to collect subsequent findings. Unknown modes fail parsing.
- **Gateway:** `safety.mode: warn` runs scanners and persists findings without blocking. `ai_safety_findings.blocked` records the enforcement decision. `quota_mode: warn` evaluates quota windows, audits breaches under policy `quota` and permits dispatch.
- **CLI:** `infra logs governance report` rolls both planes up over one window, grouped by policy, tool or user, with CSV output; `infra logs trace list --decision warn` filters traces by verdict.
- **Bridge:** `install --host <id>` enrolls a host; repeat the flag or use `--hosts all` for multiple hosts. `uninstall --host <id>` removes enrollment while retaining the bridge. Enrollment and re-apply share live profile inputs; missing-credential responses identify the enrollment command.
- **Security:** `ApprovalRepository::list_decided` — `list_pending`'s mirror, `status <> 'pending'`, newest decision first, expired rows included because nobody deciding them is itself the answer a reviewer needs. It lives in core rather than in the extension that renders it because core owns `approval_requests`, and a second reader with its own copy of the column list is how two readers of one table start disagreeing.
- **Users:** `bulk_update_status` and `bulk_delete` apply a single statement over `ANY($2)` for an id set instead of one round trip per row. `bulk_delete` is a soft delete — it sets `UserStatus::Deleted` and leaves the row resolvable for audit.
- **Users:** `--include-anonymous` on `admin users list` and `admin users search`.

### Fixed

- **Security (governance):** entropy scanning evaluates filesystem paths per slash-separated segment. Path-shaped tokens are exempt from whole-token detection; tokens containing `+` or `=` remain eligible for base64 credential detection.
...
- **Users:** user listing, search and count operations exclude anonymous visitors by default. `list_including_anonymous` and related methods provide explicit inclusion; `--role anonymous` retains role-specific lookup.
- **Bridge:** browser sign-in tries `LOOPBACK_PORTS`, starting with 8767 and advancing only on `AddrInUse`. Other bind failures identify the affected port and are written to bridge logs.
- **Bridge:** "Remove everything from this computer" can no longer be undone by a sign-in already in flight. The login task's `CancelScope::Login` token was fired only by the user's explicit Cancel button, never by purge or sign-out, so a callback landing mid-purge wrote the credential back through `setup::login` *after* the purge deleted it.
- **Bridge:** `validate` no longer races the sync it describes. `on_login_finished` sent `ValidateRequested` before dispatching provisioning or sync on the same UI queue, so validate ran first and inspected the machine policy and host files the sync was about to write — reporting the state *before* provisioning as if it were the state after. Both branches end in a sync and `on_sync_finished` re-validates either way, so the eager call is removed rather than reordered.
- **Bridge:** failed trust-on-first-use persistence returns a configuration error and records its cause in the activity log. `BridgeContext` distinguishes a fetched but unpersisted pin from an unprovisioned pin.
- **Bridge (macOS):** managed-preference reads serialize property-list values to JSON, including arrays. String values retain their plain representation.
- **Bridge:** the Claude Code policy-dir check decided on filenames alone, so *any* file named `managed-settings.json` failed it permanently while `managed_mcp::clear_policy`, which decides on content, would deliberately leave an empty `{}` alone. The check now asks the remover's own question — would sync act on this? — so checker and remedy can no longer diverge.
- **Bridge:** MDM snippets use the brand's `default_gateway_url`; documentation links target `/documentation`. The HTML template uses `__BRIDGE_VERSION__` for the bridge compatibility version.
- **UI renderer:** artifacts are dense enough to read in a chat column. A blank line became a full-height `<p>&nbsp;</p>` while `.text-content` also carried `white-space: pre-wrap`, so one blank line in the source cost roughly two on screen; separation now comes from `p + p`, which is one gap however many were typed. `format_prose` renders the three markdown constructs tools actually emit — `**bold**`, `` `code` `` and `- ` bullets — instead of showing them to a reader verbatim.
- **Gateway:** `persist_findings` took its block predicate as `&dyn Fn(&Finding) -> bool`, which is `Send` only when the closure behind it is `Sync` — and the buffered response path hands the resulting future to `tokio::spawn`.

### Changed

- **Bridge:** `policy::claude_desktop_policy` defines the managed policy key set. Platform-neutral `reg_values` and `plist_body` render Windows registry and macOS plist representations; installation and GUI templates consume those renderers.
...
- **OAuth:** concurrent passkey account-link starts for the same setup token return the same challenge and challenge ID. Completion accepts the shared ceremony.
- **API:** the global context middleware seeds request context without opening a tracing span. The route-resolved context opens one `request` span with authoritative identity fields.
...
- `InstanceId` is a first-class replica identity. `server.instance_id` (or `HOSTNAME`) keys the `services` registry rows, the `event_outbox` origin and node-scoped scheduler runs, and is stamped on every log line (`instance=`), every `logs` and `ai_requests` row, every Prometheus series (`instance` label) and the `x-served-by` header. A cloud profile that resolves to neither refuses to boot rather than take a random id per restart.
- `GET /livez` and `GET /readyz`. Liveness answers as soon as the port is bound; readiness answers 503 while booting, after the drain signal, and when the database probe fails. `/health` keeps answering 200 during boot so a single-probe platform does not kill a machine mid-migration; balancers point at `/readyz`.
- `systemprompt admin identity generate [--json]` mints `oauth_at_rest_pepper`, `manifest_signing_secret_seed` (standard base64 of 32 bytes) and `signing_key_pem` (standard base64 of the PKCS#8 PEM) in the encodings the loader expects, so operators distribute one identity instead of hand-rolling three.
- `server.metrics_port` serves `GET /metrics` on its own listener, labelled with the instance and drained on the same readiness signal as the API.
- `scope: cluster | node` on scheduler jobs. Cluster jobs run once under the advisory lock as before; node jobs run on every replica, and the 900 ms tick dedupe is keyed by the instance that last ran them so replicas no longer starve each other. `content_prerender` and `page_prerender` declare `node`, so every replica renders its own `web/dist`; set `scope:` per job in `services.yaml` to override.
- `systemprompt-storage`: a `FileStorage` implementation over a configurable root, a `storage: { backend: local, shared: bool }` profile section, and a boot-time shared-mount probe that writes a per-instance marker and warns when `shared` disagrees with what it finds. Uploads and generated images go through the trait, so a mounted shared filesystem works unchanged and an object-store backend has a seam to land in.
- `services.heartbeat_at`, a 15 s heartbeat per replica, and the `service_registry_gc` job that reaps rows whose replica stopped heartbeating for 90 s.
...
- **Bridge:** sync removes legacy Claude Code enterprise MCP policy files and excludes exclusive-mode lock keys from managed settings. CLI MCP servers are delivered through plugin `.mcp.json` files, preserving Claude built-in tools.
- **Bridge:** Windows sync reconciles the full Claude Desktop managed policy and creates the trusted workspace directory, requesting elevation only for changes. Unelevated `install --apply` stages the same elevated operation. Policy probes prioritize `HKLM` over `HKCU`.
- **Bridge:** `validate` checks the managed policy and the workspace directory on Windows and macOS, failing the verdict when a required value is missing or unreadable, and runs automatically after every sync and sign-in instead of only from the Re-check button. The health panel can no longer report "all green" over a half-written policy.
- **Bridge:** managed policy sets `disableNonessentialServices: false` for MCP display content. Validation rejects `true`, policies denying `Bash`, and legacy `managed-mcp*` or `managed-settings*` files in the Claude Code policy directory.
- **Bridge:** sync stages dashboard manifests and pages under `systemprompt/artifacts/` in the trusted workspace. Cowork setup uses file tools to install the staged content.
- **Bridge GUI:** Fluent block values and `{ $count -> [one] … *[other] … }` selects render (the MCP rows showed the raw select), the Activity page controls are styled, the signed-in splash shows a "Finishing setup" state instead of the sign-in form while hosts settle, and the Agents summary card that counted every compiled-in host as an agent is gone.

- **CLI:** setup and catalog/gateway editors register `../ai/providers.yaml` and `../ai/gateway.yaml` relative to `services/config/config.yaml`, while writing files under `services/ai/`.
- **Security:** `webauthn_challenges.user_id` now carries an `ON DELETE CASCADE` foreign key to `users`. The table is listed in `MERGE_EXCLUDED_SECURITY_TABLES`, whose stated invariant is that its rows die with the source identity, but it shipped with no foreign key at all — so a user merge or delete left passkey challenge rows behind, bound to an id that no longer existed. Migration `015_webauthn_challenges_user_fk` deletes the existing orphans before adding the constraint.
- **MCP:** the orchestrator owns service startup and emits `ServiceFailed` on failure. `ServiceStartRequested` and `ServiceRestartRequested` are notifications; the daemon starts servers explicitly and `restart_services` does not manipulate processes.
- **Billing:** `claude-fable-5-1` priced cache reads at `0.25` per million against a `10.0` input rate — 0.025x input where every other Anthropic entry, and Anthropic's own schedule, is 0.1x. Cache-heavy loops were billed a quarter of the correct rate for cached input. Corrected to `1.0`; the catalog parity test asserts the ratio.
- **CLI:** `admin identity generate` emits exactly one JSON document under global `--output json`. It wrote its fragment through `CliService::output`, which does not mark the structured artifact as emitted, so the runner appended a second document and line-based extraction of `signing_key_pem` picked up trailing junk and decoded to an invalid PEM — which cost a production boot. The command's own `--json` paste mode is unchanged.
- **Security:** CLI help hides values of secret- and PII-bearing environment arguments, including the global database URL. Help retains environment variable names and non-sensitive configuration values.
- **Security:** the admin CLI gateway redacts the argv it logs. `admin config secret set <name> <value>` passes the secret as a bare positional, and the route logged the whole argv at INFO, so every secret set through that endpoint was written to the log store in plaintext. Flag values, the following element after a sensitive flag, the `secret set` positional and any credential-bearing URL are replaced; the names are kept, since which secret was set is the part with audit value.
...
- Passkey registration, authentication, link and verified-authentication state, MCP proxy session identities and Gemini thought signatures live in Postgres (`webauthn_challenges`, `mcp_proxy_identities`, `ai_gateway_thought_signatures`). A passkey ceremony or an MCP session that started on one replica failed, or was silently downgraded to anonymous, when its next request landed on another.
...
- Gateway access logs contain a `phase: headers` record and a `phase: terminal` record. Terminal records include streaming outcome, complete elapsed time and upstream errors.
...
- **Breaking:** identity secrets are inputs, never generated at boot. A missing `manifest_signing_secret_seed` is a boot error everywhere, and a missing `signing_key_pem` is a boot error on cloud profiles and deployment hosts (local profiles may still keep the RSA key at `security.signing_key_path`). Boot used to mint a seed silently — ephemeral on a read-only profile dir — so every replica had its own manifest identity. The setup wizard and cloud profile template write all three.
- **Breaking:** a cloud profile with an empty `server.trusted_proxies` fails validation instead of logging a warning; without it every caller behind the balancer shares one rate-limit bucket and one ban target.
- **Breaking:** `/metrics` is no longer mounted on the public router; set `server.metrics_port` to expose it.
- **Breaking:** `ServiceRepository::new`, `AgentServiceRepository::new` and `A2ARepositories::new` take an `InstanceId`; `PostgresEventBridge::new` and `EventRouter::install_relay` take the origin `InstanceId`; `FileUploadService::new` and `ImageStorage::new` take an `Arc<dyn FileStorage>` and `ImageService::with_providers` takes `ImageServiceParts`; `SchedulerRepository::update_job_execution` takes a `JobRunRecord`; `ProxyEngine::new` takes the proxy identity repository; `install_recorder` takes the instance id; `ThoughtSignatureCache::global()` is gone in favour of a per-context cache over the repository.
- **Breaking:** the non-transactional `get_next_sequence_number` repository method is gone; the transactional variants are the only path.
- File records store the storage id, the path relative to `paths.storage`, rather than an absolute path. Public URLs and the on-disk layout are unchanged; deleting a generated image whose row still holds a pre-change absolute path fails with a validation error rather than touching the disk.
- `distributed.md`, the catalogue this release retires, is deleted; its normative content is deploy-production.md §3.4.
- **Breaking:** the provider catalog and the gateway routes are services configuration, not profile sections. `providers:` and `gateway:` are gone from `profile.yaml` and live in the services tree — `services/ai/providers.yaml` and `services/ai/gateway.yaml` by convention — loaded, include-merged (providers concatenate, a duplicate name is an error; the first `gateway:` wins) and validated by the services loader, and read at runtime through the new `systemprompt_loader::ServicesBootstrap` cell. A deployment's catalog is now shipped with the image like its agents and MCP servers, so every environment boots the same models and pricing and only the credentials the catalog names differ. Migration: move `providers:` and `gateway:` from `profile.yaml` into `services/ai/providers.yaml` and `services/ai/gateway.yaml` and list them in `includes:`; a profile that still carries either key fails with `ProfileError::MovedToServices`, which names the key and the file it belongs in. The types moved with the data (`systemprompt_models::services::{ProviderRegistry, ProviderEntry, ProviderModel, GatewayState, GatewayConfigSpec, GatewayConfig, GatewayRoute, …}`; no re-exports remain under `profile`), `ServicesConfig` gained `providers` and `gateway`, `admin config catalog` and `admin config gateway` edit the services files and never the profile, `admin setup` seeds both files from the embedded catalog and appends them to the root `includes:`, `cloud doctor` reads the registry from the services tree, and a services tree that does not load is now a boot failure rather than a warning.
...
- `systemprompt_models::bridge::profile::KNOWN_HOSTS` is the single list of host ids the gateway accepts from a bridge. It was a private constant in the API route; the bridge's registry test now asserts against it, so a host added on one side without the other fails a test instead of never being enabled.
- `SYSTEMPROMPT_DEPLOYMENT_HOST` identifies the deployment host independently of the hosting platform. `deployment_host` and `is_deployment_host` also accept `FLY_APP_NAME` for compatibility. Generated Dockerfiles set the marker; `SYSTEM_MANAGED` prevents client configuration from overriding it.
...
- `ProfileUsageService` assembles bridge usage reports in the analytics domain. Recent conversations include optional `context_name`, omitted when absent. `ModelShare`, `ConversationGroup` and `BridgeProfileUsage` implement `Default`.
...
- Bridge MCP status rendering uses the serialized state names. Pending probes display “Checking…”, and only confirmed authentication rejection requests sign-in.

## [0.41.0] - 2026-08-28

### Added

- Governance supports `Decision::Pending` and approver-addressed `approval_requests`. MCP protocol `2026-07-28` exposes pending calls through `InputRequiredResult`. **Breaking:** handle the new `Pending` variant; surfaces without approval support must deny it.
...
- **Breaking:** the root and bridge MSRV is 1.96. Upgrade older toolchains. The MSRV job sets `RUSTUP_TOOLCHAIN` explicitly and verifies that the running compiler matches the manifests.
- Static content routes are merged outside the IP-ban layer. API routes retain fail-closed ban-list checks; public assets do not require a ban-list database query.
...
- **Breaking:** remove `rate_limits.tier_multipliers` from profile YAML. The `admin config rate-limits` commands `tier`, `compare` and `docs`, `--tier`/`--multiplier` on `set`, and `--tier` on `reset` are removed. Profiles containing the removed key fail validation.
- Removed the bridge “Governed requests” table, its in-memory ring, decisions poller and `GET /v1/bridge/decisions`. `AuthzRequest` carries enforcement surface, credential `client_id` and access scope; `governance_decisions` stores an indexed `client_id`.

### Fixed

- Rate limits use signature-verified identity when available and otherwise use the client address resolved through `trusted_proxies`. Unresolvable addresses are rejected; untrusted forwarding headers cannot select quota buckets.
...
- `HttpClientWithContext` includes the negotiated protocol and client capabilities in each request’s `_meta` for SEP-2575 stateless operation. The version matches `MCP-Protocol-Version`; rmcp supplies SEP-2243 method and name headers for protocol `2026-07-28`.
...
- The secret-scan entropy backstop exonerates `sha256/384/512`-prefixed tokens only when the payload decodes to exactly the algorithm's digest length, so forwarded surfaces carrying SRI hashes stop being denied while a smuggled credential behind a digest prefix still is. `entropy_from_yaml` reports unknown keys and mistyped values at error level instead of falling back.
...
- Gateway policies carry an integer `priority` declared in `services/gateway/policies.yaml` and persisted through ingestion. The effective policy folds rows in ascending order with last-write-wins per section, so precedence is stated rather than expressed by naming convention — the `zz-` prefix trick that ordering-by-name forced is no longer needed. Ties still order by name.
- `EntryKeepSets` and `allowed_ids`/`BulkKeepQuery` resolve per-user marketplace entry visibility in two queries instead of one per entry, and the marketplace ABAC floor now follows the parent cascade through `MarketplaceParent`.
- `PreparedDispatch`/`GovernedDispatch`/`ScannedDispatch` make the gateway dispatch order — build wire payload, govern, scan, send — a property of the types: each stage is constructible only from its predecessor.
- `MERGE_EXCLUDED_SECURITY_TABLES` names the credential and auth-state tables `merge_users` deliberately does not transfer, and a test asserts each one really does carry an `ON DELETE CASCADE` foreign key to `users` — the mechanism that list depends on. A new table holding credentials belongs on the list, not in a `transfer_*` helper.
- `SenderIdentity` states the identity-linking rule for inbound chat-platform messages in the type: a sender is `Linked` only when the platform verified the claims.
...
- **Breaking:** an extension registering a safety scanner under a built-in's name is now rejected and logged as an error. It previously shadowed the built-in with only a warning, so an extension could silently replace `heuristic` for the whole deployment.
- `users.name` is no longer `UNIQUE`. Display names are not identities and collide legitimately; `find_by_name` orders by `created_at` and takes the first row so the lookup stays deterministic.
- The BSD/GNU `ps` divergence and the Cowork version pins each live in one module (`process/ps.rs`, `cowork_compat.rs`) instead of being restated at every call site.
...
- **Breaking:** a duplicate skill or plugin id across on-disk catalogue directories is now a hard loader error, matching the existing duplicate-marketplace behaviour, instead of keeping the first directory scanned. A root-config declaration still shadows an on-disk descriptor of the same id.
...
- macOS child identity and zombie checks use `sysctl(KERN_PROCARGS2)` and `proc_pidinfo`. Startup reconciliation, shutdown, port reclamation and scheduler process management use these checks before signaling a process.
...
- Slack and Teams A2A tokens include both `a2a` and `mcp` audiences. Permissions derive from the linked account: `Admin` for the admin role and `User` otherwise.

- macOS startup identifies and reclaims orphaned MCP children belonging to the installation. macOS does not provide a parent-death signal equivalent to Linux `PR_SET_PDEATHSIG` that survives `execve`; startup reconciliation remains necessary after forced termination.
...
- `admin bootstrap` requires a validated owner email from `system_admin.email`, `SYSTEM_ADMIN_EMAIL` for environment-built profiles, or `--email`. Generated profiles include the field; existing owner rows remain unchanged.
...
- A CLI session store that exists but cannot be parsed fails profile resolution with a message naming the store file and `admin session switch <profile>`, instead of degrading into profile discovery and reporting "Multiple profiles found"; older stores missing newer fields keep parsing via serde defaults, and the recovery commands themselves still work over a corrupt store.
...
- On Windows, a sync that targets the Cowork host from a non-elevated process fails with explicit elevation guidance instead of writing plugins to a per-user directory Cowork never scans; `doctor` reports the same condition.
...
- **Breaking:** `services::Settings` rejects unknown fields. A services config that spells the settings keys in camelCase (`mcpPortRange`, `agentPortRange`, `autoStartEnabled`, `schemaValidationMode`) now fails to load instead of falling back to the defaults. Migrate by renaming them to snake_case.

### Added

- `mcp::ClientProfile` (with `supports_ui` / `supports_structured_content`), `client_profile_from_peer`, and `client_profile_from_stored` expose the client's `initialize` declaration to the response path; a client whose declaration is unknown gets the conservative text-only shape every conforming host accepts.
- `McpOutputSchema::text_body` names the plain-text body of a tool output so text-only clients still receive the data; outputs without one fall back to pretty-printed JSON under the summary.
- `bridge_policy.allow_claude_ai_connectors` in a services YAML flows into the signed bridge manifest and instructs the bridge's Claude Code managed-MCP policy to emit `allowAllClaudeAiMcps: true`, so claude.ai first-party connectors keep working alongside the managed server allowlist; withdrawing the policy removes the key.
- A wire-conformance test suite validates every client-profile shape of `McpResponseBuilder::build` against the official MCP `CallToolResult` JSON schemas (2025-03-26 and 2025-06-18), replacing the log-only schema check.
...
- The bridge self-updates from the gateway: `/v1/bridge/latest` and `/v1/bridge/download/{platform}` serve the feed configured by `gateway.bridge_releases` (the gateway resolves and proxies the private release assets; digests come from the release's signed SHA256SUMS), and the bridge gains an `update` CLI command (`--check`/`--yes`) plus GUI update prompts, with SHA-256 verification before anything executes and per-platform rollback-safe swaps. `Brand` now carries the shipping crate's version, so white-label builds report their own version rather than core's.
...
- `McpOutputSchema::validated_schema` guarantees the advertised `outputSchema` is an object schema (`"type": "object"` at the root), which the MCP spec requires and Claude Desktop enforces strictly. A tagged-enum tool output previously advertised a bare `oneOf` with no `type`, and the client rejected every server at connect time on the first such tool.
...
- Bridge Codex profiles declare `ApiSurface::OpenAi`. macOS probes read managed preferences, and `install --apply-mobileconfig` reports the required System Settings approval. Managed profiles set `approval_policy = "never"` and `sandbox_mode = "workspace-write"`.
...
- **Breaking:** repositories are constructed once at composition roots and injected. `UserService`, `ApiKeyService`, `DeviceCertService` take `Arc<UserRepository>`; agent orchestration, `MessageProcessor`, `ArtifactPublishingService`, `ContextProviderService`, `OAuthState`, `ClientValidator`, analytics, AI, files, MCP, content, generator, and evaluation services take their repositories (or a bundle) rather than a pool. Migrate by passing the repository from an `AppContext` accessor or building it once at your composition root; `AppContext` gains `ai_repositories()`, `analytics_repositories()`, `file_repository()`, and `mcp_session_repository()`.
- **Breaking:** `AiService::new` groups its trait objects in `AiServiceProviders { tools, sessions }`, `JudgeService::new` takes a `JudgeSpec`, and `AnalyticsService::new` is infallible, taking `&AnalyticsRepositories` in place of the pool. Migrate by wrapping the existing arguments in the new structs.
- **Breaking:** `OauthRepositories` is renamed to `OAuthRepositories`. Migrate by updating the import; behaviour is unchanged.
- **Breaking:** `ai_requests.context_id` and `governance_decisions.context_id` are NOT NULL (migrations `014_ai_requests_context_not_null`, `012_governance_decisions_context_not_null`), and `AiRequestRecord::builder` requires a `ContextId`. `ContextId` gains deterministic derivations (session, task, evaluation run, MCP service, probe) plus a fixed legacy context that absorbs pre-invariant rows. Migrate by deriving the context id at the boundary that mints it.
- **Breaking:** `UserProvider` gains `promote_anonymous`, and `ContextMaterializer` (with `EnsureContextParams`) joins `systemprompt-traits`. An implementation of either trait needs the new method.
...
- `scripts/lint-repo-construction.sh` gates ad-hoc repository construction: a `*Repository::new(` call outside a composition root (repository modules, the AppContext builder, CLI one-shot bodies, job bodies, or an eight-file justified allowlist) fails `just check` and CI, and stale allowlist entries fail the gate too.
...
- `ai_request_payloads.offered_tools` records the tool definitions offered to the model on both the gateway and in-process inference paths.
- `/health` reports `degraded` with `events: { "relay": "not_listening" }` when an enabled event relay loses its listener. `systemprompt_events::is_listening` treats deployments without a relay as healthy.
...
- **Breaking:** an ID-JAG redeemed at `/token` now yields a token for the employee it names rather than for the OAuth client's owner. A deployment that used the ID-JAG path as a fixed-identity delegation shim will see the token's `sub`, its session row, and its permission ceiling move to the linked enterprise user. The plain token-exchange path (access-token, id_token, and jwt subjects) is unchanged.
- **Breaking:** `OAuthRequirement` and `ServiceOAuthConfig` gain an `ema` flag, and `WellKnownResponse` gains `authorization_grant_profiles_supported`. Service YAML needs no edit — the field defaults to `false` — but a struct built by literal needs the extra initialiser.
...
- **Breaking:** `GovernanceConfig` gains an `enabled` field, so a struct built by literal rather than through `Default` needs the extra initialiser. `governance.enabled: false` switches the whole chain off in one key while leaving every per-policy declaration intact.
- **Breaking:** `wire::anthropic::event_from_sse` is replaced by `events_from_sse`, returning a `Vec<CanonicalEvent>` rather than an `Option`. One `message_delta` frame carries both the terminal `stop_reason` and the final cumulative `usage`, and the old signature could return only one of the two. Migrate by iterating the returned events instead of matching one option.
- **Breaking:** `ModelPricing` gains `cache_read_per_million` and `cache_write_per_million`, and `gateway::pricing::cost_microdollars` takes a `CostTokens` struct rather than two `u32` arguments. Both pricing fields are `#[serde(default)]`, so a catalog written before this release still parses — its cached tokens simply bill at zero until rates are supplied.
...
- **Breaking:** `CanonicalEvent::UsageDelta` carries a `CanonicalUsageUpdate` — four `Option<u32>` counts — rather than a `CanonicalUsage`. Providers differ in what a mid-stream usage frame states, and `CanonicalUsage` cannot tell an unreported count from a reported zero. Migrate by constructing the update with `Some(..)` for the counts a frame actually carries and leaving the rest `None`; a consumer folds one in with `CanonicalUsageUpdate::apply_to`.

### Added

- Enterprise-Managed Authorization is conformant on both the identity and the resource binding the extension makes normative. An ID-JAG's `sub` is linked to a local account through the existing federated-identity path — matched on `(iss, sub)`, falling back to the email, provisioning on first sight — so the access token, its session, and its audit trail name the employee instead of collapsing every employee onto the client's owner. The ID-JAG's `resource` claim pins the exchange: a request may omit `resource` or name the same one, and anything else is `invalid_target`. The email is treated as verified because an ID-JAG is RS256-signed by a `kid` from a configured trusted issuer's JWKS and that issuer is separately marked `can_issue_id_jag`.
- `grant_type=urn:ietf:params:oauth:grant-type:jwt-bearer` with an `assertion` redeems an ID-JAG, the shape the extension specifies and the one off-the-shelf enterprise clients send. It shares its validator with the equivalent token-exchange call, which stays available.
- Both authorization-server metadata documents advertise `authorization_grant_profiles_supported: ["urn:ietf:params:oauth:grant-profile:id-jag"]` and the jwt-bearer grant, so a conformant client can detect enterprise-managed support instead of inferring it from the RFC 8693 token-type lists.
- An MCP service declares the extension explicitly through `oauth.ema` rather than having it inferred from the shape of its audience string. The old inference fired on any audience outside the seven known names, which is a property of an infallible `FromStr`, not a statement about enterprise auth.
- RFC 9728 protected-resource metadata is one shape, `systemprompt_models::oauth::ProtectedResourceMetadata`, serialised by the three routes that advertise it and deserialised by the MCP client that reads it. It previously existed as two partial declarations on opposite sides of the same handshake, which could drift silently.
- A 401 from an MCP server surfaces as a typed `McpTransportError::AuthorizationRequired` carrying the resource, the metadata URL, the authorization servers, and whether the resource requires the enterprise-managed grant — so a caller can branch on the flow instead of matching on a message.
- Our outbound MCP client declares the extension in its client capabilities, and a 401 from an MCP server is now read rather than stringified: the `WWW-Authenticate` parameters are parsed, the RFC 9728 resource metadata they point at is fetched through the outbound-URL guard, and the resulting error says whether the resource wants an enterprise-managed ID-JAG or an interactive authorization flow.
- Gateway governance inspects the rendered outbound payload through `CanonicalRequest.forwarded_surface`, including keys and fields outside the canonical model. Adapters expose `build_body`. Traversal limits depth, leaves, total bytes and leaf size; limit exhaustion marks inspection as truncated.
- Response bodies are inspected the same way, against the bytes the client receives: `inspect::string_leaves` for a buffered reply and `inspect::sse_string_leaves` for a streamed one, which merges every frame under one shared budget and lets a single malformed frame contribute nothing rather than blinding the scanner to the rest.
- Inference and MCP use the same `GovernanceEngine` instance and rate-limit buckets. Governance precedes safety scanners, includes the deciding policy in denials and persists both allowed and denied evaluations.
- `ai_request_payloads` records SHA-256 digests of the client's request body, the body forwarded upstream, and the response body (migration `012_payload_digests`). The digest covers the full bytes, so a truncated capture is still provable, and the request and prepared digests legitimately differ after a model retarget, `max_tokens` clamp, or `user_id` strip.
...
- `Profile::validate` rejects a loopback `server.api_external_url` on a cloud profile. That URL is what a deployment advertises in OAuth token endpoints, MCP server URLs, and agent cards, so pointing it at loopback makes every remote client resolve it to itself. Local profiles are exempt — binding `0.0.0.0` while reaching the server on `localhost` is ordinary container-based development.
- `GatewayConfig::validate` requires billable route pricing or usable pricing for every matched registry model. Patterns must match at least one priced model.
- The default provider catalog carries cache read and write rates on every model, adds `claude-opus-5`, `claude-sonnet-5`, `claude-fable-5`, and `claude-haiku-4-5`, and gives the dated ids `aliases` so `claude-opus-4-5` and `claude-opus-4-5-20251101` resolve to one entry. Sonnet 4.6's context window and output ceiling are corrected to 1M/128k.

### Fixed

- A governance denial on `/v1/messages` renders as `400 invalid_request_error` rather than `403`. Claude Code and other Anthropic-SDK clients read any 403 on that route as an expired credential: they discard the body and tell the operator to re-login, so the actual reason never reached the person who needed it. A denial is a refusal on content — re-authenticating cannot change it — and 400 is the shape those clients surface verbatim.
...
- Bridge validation distinguishes mismatched loopback credentials from a configured base URL using an inactive port. Re-apply guidance requires a definite mismatch; remote or unparseable URLs are excluded from that check.
- Every gateway outbound path removes `metadata.user_id` and applies the resolved model and output-token ceiling, including fallback from byte passthrough to canonical rendering.
- `bridge login --no-browser` requests a displayed one-time code without a callback. Input accepts a bare code, the displayed login command or a callback URL and removes terminal bracketed-paste sequences. Exchange codes expire after ten minutes.
- The bridge proxy resolves its port through a sticky portfile with identity probing, so a restart reuses the port a sibling recorded rather than drifting to the next free one; a port answering without identifying itself is still treated as ours, and only a probe that positively identifies a foreign holder moves the proxy on.
- `infra logs request list` preserves complete request IDs in JSON and YAML. Terminal table rendering applies column widths; `infra logs show` prints executable trace commands with full IDs.
...
- `mime::extension_for("application/octet-stream")` yields `bin` rather than `exe`. Several concrete rows (exe/msi, appimage, sig) carry octet-stream as their essence, and the reverse lookup returned whichever sorted first, so an upload of unknown binary type was named with an executable extension.
- The gateway derives the OAuth `token_endpoint` it advertises from the host the client dialled, as `/.well-known/oauth-authorization-server` already did. Formatting `api_external_url` unconditionally handed a remote client the gateway's own loopback address, so every plugin hook failed with a connection error to `localhost`. A `Host` outside the `api_external_url` allowlist still falls back to the configured value, so a forged header cannot redirect the mint.
- Cache reads and cache writes are billed at their own rates instead of being ignored. For an agent loop resending a large cached prompt they are the bulk of the tokens, so costing on `input + output` alone understated the bill by an order of magnitude; `tokens_used` now counts all four buckets.
- A streamed Anthropic completion reports its token usage at all. The codec turned a `message_delta` frame into either a stop event or a usage event and preferred the stop, discarding the one frame that carries real counts — so every streamed request recorded zero tokens and zero cost. Both events are now emitted, usage first, so a consumer finalizing on the terminal event has already folded the counts in.
- Streaming usage tracks provider snapshots and field presence. Reported values replace earlier values, including explicit zero; absent fields retain their prior counts. Placeholder message-start values do not count as a terminal usage report.
- Streaming usage updates preserve input and cache counts when an Anthropic delta reports only output tokens. Explicitly reported zero values replace earlier estimates.
## [0.27.0] - 2026-07-29
...
- **Breaking:** `GovernanceChain` is removed from `systemprompt-security`. Migrate to `policy::GovernanceEngine::evaluate`, which returns `Evaluation { decision, chain }` carrying a per-entry `ChainEntryOutcome` trace rather than the first deny alone.
- **Breaking:** the MCP tool count is `Option<usize>` wherever it is reported — `McpConnectionResult.tools_count`, `HealthCheckDetails.tools_available`, `ServiceStatus.tools_count`, `McpServiceStatus.tools_count`, `StartupEvent::McpServerReady.tools`, and the `tools` argument of `StartupEventSenderExt::mcp_ready`. `None` means the tool list was never enumerated; `Some(0)` means a server was asked and answered with none. Migrate by matching on the option at each render site and showing "not enumerated" rather than a zero.
- **Breaking:** `CleanupRepository::delete_orphaned_mcp_executions` is removed, and `database_cleanup` no longer calls it. MCP tool executions are the governance audit record and deliberately outlive their context: `mcp_tool_executions.context_id` carries no foreign key, so a context id absent from `user_contexts` is a normal state, and the sweep hard-deleted same-day audit rows the moment their context disappeared. No migration — consumers aggregate executions by `user_id`.
- **Breaking:** `default_bootstrap_jobs()` no longer includes `database_cleanup`, so the sweep runs from its nightly cron entry rather than on every deploy, restart, and crash-loop. A deployment relying on the boot-time sweep must list `database_cleanup` in `scheduler.bootstrap_jobs` explicitly.
- **Breaking:** the deleting core jobs honour `JobConfig.enforce`: `database_cleanup`'s log deletes, `cleanup_empty_contexts`, `cleanup_anonymous_users`, and the new `backfill_session_geo` observe and log would-affect counts when `enforce` is `false`. `SchedulerConfig::with_system_admin()` opts the three retention jobs in, so the default config keeps deleting; a deployment authoring its own `scheduler.jobs` entries must add `enforce: true` to keep retention active. The OAuth/JAG expiry deletes and the status-only jobs (`cleanup_inactive_sessions`, `mcp_session_cleanup`) are exempt — their rows are expired by contract.
- **Breaking:** `JobExecutionService::new` takes the `SchedulerConfig` so manually triggered runs resolve the same `enforce` and `parameters` a scheduled run would; previously a manual run of an enforce-aware job always observed.
- **Breaking:** `AppPaths::from_profile` takes a `PathResolution` (`Profile::path_resolution()` derives it), and `SystemPaths::from_profile` likewise. A cloud profile resolves lexically — container paths such as `/app` are no longer canonicalized against the operator's local filesystem, which made `admin session login` (and with it `infra db query --profile <cloud>`) unusable against any cloud profile. `PathError` gains `NotAbsolute` for a relative path under lexical resolution.
- **Breaking:** `AppContext::load_geoip_database` returns `Result<Option<GeoIpReader>, RuntimeError>` and startup fails when an explicitly configured `paths.geoip_database` cannot be opened (`RuntimeError::GeoIpUnreadable`). "Not configured" keeps its startup warning; "configured but unreadable" is a misconfiguration the operator asked for and no longer degrades silently to NULL countries.
- **Breaking:** `users.email` is stored normalised (trimmed, lowercased) and the schema enforces it with a `CHECK`; migration `009_normalise_user_emails` lowercases existing rows and fails loudly when two rows differ only by case, naming `admin users merge` as the remedy. `normalise_email` is applied on every create and lookup, so `Ed@x.com` and `ed@x.com` resolve to one account.
- **Breaking:** `UserRepository::merge_users` runs as one transaction and rekeys every user-data table (sessions, tasks, messages, contexts, MCP executions/artifacts/sessions, governance decisions, logs, AI requests, engagement/analytics events, outbox, files, link clicks, fingerprint associations) instead of `user_sessions` + `agent_tasks` only and non-atomically. Security artifacts (OAuth tokens/codes, WebAuthn, API keys, device certs, bridge auth state, federated links) deliberately die with the source row via FK cascade. `MergeResult` gains `total_rows`.
- **Breaking:** the MCP SDK moves from `rmcp` `3.0.0-beta.1` to the released `3.0.0`, and `rmcp` is re-exported from `systemprompt::prelude`, so its removed deprecated aliases are removed from the facade's surface too. Migrate by renaming the singular request-parameter aliases to their plural forms (`CallToolRequestParam` → `CallToolRequestParams`, and likewise for `Initialize`, `GetPrompt`, `ReadResource`, `CreateMessage`, `Complete`, `Paginated`, `SetLevel`, `Subscribe`, `Unsubscribe`), `rmcp::Error` → `rmcp::ErrorData`, `Meta` → `MetaObject`, `CreateElicitationRequest`/`Result` → `ElicitRequest`/`ElicitResult`, `PrimitiveSchema` → `PrimitiveSchemaDefinition`, `ResourceReference` → `ResourceTemplateReference`, and `SamplingMessageContent` → `SamplingMessageContentBlock`.

- **Breaking:** `GatewayRequestGuard::check` and `run_gateway_guards` take a `GatewayGuardRequest` (user id, requested model, resolved route id, provider, streaming flag) instead of a bare user id, so an extension guard can enforce per-plan model and route entitlement. `GatewayDenyReason` gains `kind: GatewayDenyKind`: `Quota` (the default, preserving today's 429 + `retry-after`) or `Forbidden`, which the gateway renders as a 403 without `retry-after` — an entitlement denial is not a rate limit and retrying can never help.
- **Breaking:** quota buckets are keyed by an open-vocabulary subject: `ai_quota_buckets` gains `subject_kind` (default `'user'`) and a `cost_microdollars` counter, `user_id` becomes `subject_id`, and the unique key becomes `(subject_kind, subject_id, window_seconds, window_start)` (migration `011_subject_quota_buckets`; existing rows keep today's behaviour). `QuotaWindow` gains `subject` (serde default `"user"`) and `max_cost_microdollars`, and loses `Copy`; `IncrementParams` takes plain `subject_kind`/`subject_id` strings; `GatewayAudit::complete` returns the computed request cost. A window keyed on an extension dimension (e.g. `organization`) resolves its subject through the registered `SubjectAttributeProvider` — first value wins, unresolvable subject skips the window — and the cost ceiling is enforced one request late, since cost is known only after the response.

### Fixed

- Streaming gateway completions debit quota buckets (tokens and cost) and run the response-phase safety scan; both were buffered-only, so streamed traffic bypassed token counters, `max_cost_microdollars` ceilings, and response scanning. A failed stream debits only its precheck reservation.
- `cleanup_empty_contexts` no longer collects a context holding `mcp_tool_executions` or `governance_decisions` rows — a context carrying audit data is not empty regardless of age. Together with the removed orphan sweep this closes the data-loss chain that hard-deleted same-day MCP audit rows; its default window is 24 hours (was 1) and configurable via the `retention_hours` parameter.
- `JobConfig.parameters` reaches scheduled and bootstrap runs.
- `cloud profile create` preserves an existing profile's `paths.geoip_database` instead of authoring `None` over it, and GeoIP lookups log a structured `geoip_skip_reason` (`no_reader` vs `private_or_local_ip`) so a missing database is distinguishable from a proxy misconfiguration.
- Per-user aggregates undercounted: one human could hold several `users` rows (case-variant emails, federated duplicates, stranded anonymous history). Email normalisation, verified-email federated linking, and anonymous promotion close the paths that minted them; rows minted before this release remain split until merged with `admin users merge`.
- Startup marks OAuth-gated MCP tool counts as unmeasured when validation checks only port reachability. Logs omit `tools`, include `validation_type` and state that the tool list was not enumerated.
- MCP connection validation reports a name and version for a peer that sends no `Implementation`, which a server answering `server/discover` (SEP-2575) is permitted to do. Both fall back to the configured service name and `1.0.0`.
...
- Retention windows are job parameters with the previous constants as defaults: `retention_hours` (cleanup_empty_contexts, 24), `log_retention_days` (database_cleanup, 30), `inactive_hours` (cleanup_inactive_sessions, 1), `retention_days` (mcp_session_cleanup, 7; cleanup_anonymous_users, 30). `JobContext::get_parameter_parsed` parses them, failing the run on a malformed value.
- `backfill_session_geo`: an on-demand, enforce-gated scheduler job that backfills `country`/`region`/`city` on sessions written before GeoIP was enabled (`batch_size` parameter, default 1000). `country` was previously write-once at INSERT.
- `UserService::promote_anonymous` moves an anonymous visitor's history onto their registered account in one transaction; WebAuthn registration invokes it best-effort for a prior anonymous session.
...
- `policy::GovernanceEngine` evaluates a governed call against the configured policy chain first-deny-wins, returning a per-entry audit trace. The chain is instantiated from the inventory registry (`register_governance_policy!`) and a `GovernanceConfig` parsed from the `governance.policies` YAML shape.
- Connection-scope seam for pooled multi-tenancy row-level security: `systemprompt-database` gains `scope::ConnectionScopeProvider` (registered with `register_scope_provider!`), which translates the new `systemprompt-models` `RequestScope` into transaction-local custom GUCs applied via parameter-bound `set_config($1, $2, true)` on the new `begin_scoped` / `with_scoped_transaction{,_raw}` / `Database::begin_scoped` APIs. Settings die at COMMIT/ROLLBACK so pooled connections return clean; with no registered provider the scoped APIs are a plain `pool.begin()` and every existing pool and transaction surface is untouched.
- Four built-in governance policies ship in core — `secret_scan`, `scope_check`, `tool_blocklist`, and `rate_limit` — each emitting the typed `DenyReason` variant defined for it. Extensions register further policies against the same macro and enable them from the same YAML.
- `policy::DecisionAudit` and `record_decision` persist an evaluation trace into `governance_decisions.evaluated_rules`, deriving the flat columns from it.
- `policy::secrets` exposes the credential scanner as `detect_secrets` and `scan_str_for_secret`.
- Rate-limit state is scoped to the `GovernanceEngine` rather than the process. A deployment holding one engine per process is unaffected; two engines no longer share buckets.
...
- **Breaking:** `ChartArtifact` exposes and serializes chart type, title, axis labels and axis types. Schema properties replace `x-chart-hints`; older payloads deserialize with defaults.
...
- **Breaking:** charts render as inline SVG without Chart.js or CDN scripts. Update integrations that expect `<canvas>`, `window.CHART_CONFIG` or the Chart.js CSP exemption.
- **Breaking:** `PolicyContext` carries a `call_id`, and `GovernancePolicy::evaluate` is documented as idempotent per that id. One call is legitimately evaluated more than once — an enforcement point behind another still runs the chain, because callers that never passed the first can reach it — so a policy that accumulates state needs to tell "the same call again" from "another call". Migrate by populating the new field at each enforcement point; a policy that keeps no state is unaffected.
...
- Artifact UI is themeable. Every renderer's stylesheet now addresses colour, radius, shadow, and type through the `--mcpui-*` custom properties declared in `tokens.css`, and a deployment registers an `ArtifactTheme` — via `register_artifact_theme!`, the same compile-time shape the safety-scanner and route-selector registries use — to re-declare some or all of them. A partial theme overrides what it names and inherits the rest, and `extra_css` covers what a custom property cannot express, such as an `@font-face` or a rule aimed at one renderer. Restyling artifacts to match a product no longer means forking a renderer.
- `CallId` identifies one governed call across every evaluation of it.
...
- Gemini thought signatures are cached by conversation and tool-use ID and restored when clients omit them from replayed tool calls. Client-provided signatures take precedence. This release uses a process-local cache and requires sticky routing for multi-replica recovery.
...
- Gemini thought summaries parse and stream as thinking blocks; thinking-enabled requests set `includeThoughts` and replay signatures. **Breaking:** canonical thinking gains `id` and `encrypted_content`; `CanonicalEvent` gains `EncryptedContentDelta`.
- OpenAI Responses reasoning is replayable. Reasoning-enabled upstream requests send `store: false` so encrypted reasoning content comes back; reasoning items are emitted one per thinking part with their provider `id` and `encrypted_content`; and the Responses inbound adapter round-trips a client's reasoning ids and encrypted content instead of discarding them and inventing synthetic ids.
- An unsigned thinking block is never sent to the Anthropic upstream — Anthropic 400s on it; the block (and any message left empty) is omitted, trading reasoning continuity for a working request in cross-provider histories.
- The Anthropic-protocol inbound parser drops content blocks it does not model (`redacted_thinking`, `document`, `server_tool_use`, `web_search_tool_result`) instead of 400ing the request, mirroring the response-side parse that already strips them. History imported from a direct Anthropic session now degrades instead of failing.
...
- **Breaking:** `ai_requests.provider` and `ai_requests.model` are nullable, and `AiRequestRecord` carries `Option<String>` for both. A request refused before routing has no provider and no model; the columns previously held the literal string `"unknown"`, which no consumer could distinguish from a provider actually named that. The `NOT NULL` pair is replaced by a status-keyed `CHECK` — only a `rejected` row may omit them — so the invariant still holds everywhere it meant something. `AiRequestRecordBuilder::build` is infallible and returns `AiRequestRecord` rather than `Result`, and `AiRequestRecordError` is deleted along with its two missing-field variants. Migrate by dropping the `?`/`unwrap`, by matching on the option at any site that reads `.provider` or `.model`, and by filtering aggregates on `IS NOT NULL` instead of `!= 'unknown'`. Migration `010` backfills the historical sentinel to `NULL` and stamps those rows `rejected`.
- **Breaking:** `RequestStatus` gains a `Rejected` variant for a request refused before it reached a provider. Migrate by covering it in any exhaustive match.

### Added

- `SafetyConfig::history` selects how far back the request-phase scanners look: `off` (the default) judges only the newest user turn, `audit` also scans earlier turns and records their findings at the new `request_history` phase without denying the request, and `block` restores the pre-0.25.0 behaviour of letting a prior turn deny the current one. Policies written before the field existed keep working and get `off`.
- `SafetyScanner::scan_request_history` is the seam for judging turns the caller already sent. It defaults to scanning nothing and is called only when policy asks for it, so existing scanners need no change: whatever a scanner reports from `scan_request` still denies the request as before. The `PHASE_REQUEST`, `PHASE_REQUEST_HISTORY` and `PHASE_RESPONSE` constants name the phases a `Finding` can carry.
- `CanonicalRequest::latest_message_text` flattens the newest message of a role, and `CanonicalRequest::message_units` flattens the system prompt and each message into its own string. A detector that slides a window over `flatten_text` sees two unrelated messages as one; `message_units` is what lets it respect the boundary.
- The `null` scanner resolves from the gateway scanner registry. `NullScanner` was exported and documented as the scanner to name when scanning is disabled, but it was never registered, so a policy naming it hit the unknown-scanner warning and silently ran nothing.
...
- JavaScript is served as `text/javascript` rather than `application/javascript`, and YAML as `application/yaml` rather than `text/yaml`. Served `text/*` responses now carry `charset=utf-8`, including the HTML and CSS that previously omitted it; the parameter is confined to `text/*`, since RFC 8259 defines none for `application/json` and YAML and TOML are UTF-8 by definition. Anything matching on the previous header values needs updating.
...
- `ConfigLoader::load` caches merged services configuration by resolved path for the process lifetime. Restart after configuration edits. `load_from_path` and `validate_file` continue to read from disk on each call.
...
- Built-in safety scanning evaluates the system prompt and newest user turn. `safety.history` controls earlier-turn scanning; history findings block only in `block` mode. Category blocking accounts for the scan phase.
- Credit-card detection checks contiguous digit runs within each message, allowing spaces and hyphens as separators. It does not concatenate unrelated numbers or span messages.
- Duplicate `ai_safety_findings` rows are collapsed. A scanner emits one finding per match, so a message tripping two jailbreak phrases wrote two rows before any repetition was involved; findings are now deduplicated by phase, category and scanner before they are persisted, on both the request and response paths.
...
- A gateway request rejected before it authenticated no longer logs a warning about a skipped audit row. It has no principal by construction — `require_session_id` runs before `authenticate`, which needs the session id — so there is nothing to attribute an `ai_requests` row to, and forcing one would let anything probing `/v1/messages` write unbounded rows. The rejection is recorded by the access-log entry, which carries its status, and the message drops to `DEBUG`.
...
- `--json` output uses nested JSON values: card section content and table cells serialize as real nested JSON values instead of JSON-encoded strings (`CardSection.content` is now a `serde_json::Value`). The dead per-command `--format` argument on `infra db query|execute` and `cloud db query|execute` is removed — the global `--json`/`--yaml` flags are the one output switch.
...
- Source files now carry a Business Source License 1.1 header referencing <https://systemprompt.io>, and rustdoc/comment coverage was normalized across all crates to the project comment standard.
...
- `user_contexts` gains a `kind` discriminator (`user` | `cli_session`; additive migration `008_add_user_contexts_kind` backfills existing `CLI Session - ` rows). It is exposed as a typed `ContextKind` on `UserContext`/`UserContextWithStats`, required by `ContextRepository::create_context`, carried through cloud sync export/import, and a new optional `sqlx` feature on `systemprompt-models` derives `sqlx::Type` for DB-persisted enums so queries decode it directly. Downstream consumers (admin context listings, analytics joins) can now filter ephemeral CLI rows on the column instead of matching the display-name prefix.
...
- Extensions can override the framing policy for their routes: `ExtensionRouter::with_frame_options(FrameOptions)` (or applying `stamp_frame_options` to a route subtree) records a typed `FrameOptionsOverride` response extension, and the global security-headers middleware honours it — emitting `X-Frame-Options: SAMEORIGIN`/`DENY` or, for `FrameOptions::AllowAll`, removing the header entirely — always paired with the matching `Content-Security-Policy: frame-ancestors` directive. The profile default (`server.security_headers.frame_options`, `DENY`) still applies everywhere else, and a raw `x-frame-options` header set by a handler without the marker is still overwritten, so the override cannot be triggered accidentally. Unblocks third-party-embeddable pages (e.g. tool embed widgets) without weakening sitewide clickjacking protection.
...
- Crate-deploy process execution (`systemprompt-sync`) is routed through the existing `CommandRunner`/`CommandSpec` seam from `systemprompt-cloud` (the `DockerCli` precedent) instead of constructing `std::process::Command` inline — identical commands, arguments, stdin piping, and error mapping; tests substitute a stub runner.
...
- Full-codebase dead-code sweep: every pub item with no call site in this repo or any downstream consumer (template, extensions, MCP servers, bridge) is deleted, along with its characterising tests. Highlights: the never-wired provider-injection framework (`SessionAnalyticsProvider`, `FileUploadProvider`, `McpServiceProvider`, `ProcessCleanupProvider` traits, their domain impls, the `AgentState`/`TaskRepository` provider fields and setters, and the message-part file-upload offload path that could never execute); the A2A agent-token user re-verification seam (`AgentOAuthState::with_user_provider` and the DB user/permission re-check), unreachable because nothing ever installed a user provider; the interactive prompt/summary display stack in `systemprompt-logging` (`Prompts`, `PromptBuilder`, `QuickPrompts`, `ModuleDisplay`, `BatchModuleOperations`, `ValidationSummary`/`OperationResult`/`ProgressSummary`, and the `dialoguer` dependency); `CloudContext`/`ResolvedTenant` and five never-constructed `CloudError` variants; `DatabaseInfoRepository`; the unused `agui`/`jsonrpc`/`protocol` wire-constant modules; `SlackSurface` and the write-only `NormalizedInbound.surface` field; 20 never-constructed CLI output DTOs and the orphaned CLI `env.template` asset; the dead `ApiPaths`/`CliPaths` registry entries; and a tail of dead model constructors/builders across `systemprompt-models` (`AgentCardBuilder::with_mcp_skills`/`with_oauth2_security`, `AiMessage::user_with_parts`, `ToolUiConfig` builder methods, unused path/net constants). The facade README/doc example version pins are refreshed to 0.19.0.

### Fixed

- Every CLI invocation with a stale persisted session minted a new `CLI Session - {profile}` row in `user_contexts`, with no GC path — unbounded row churn that made admin context listings nondeterministic. The three CLI session paths now reuse one stable row per user/profile via `ContextRepository::get_or_create_cli_context` (re-pointing it at the new session), and `cleanup_empty_contexts` spares session-bound CLI contexts while collecting session-orphaned ones.
- Session counters update on task and message creation. Agent startup injects `AnalyticsAiSessionProvider` into `AiService` for AI usage totals. The redundant `increment_ai_request_count` mutation is removed; `increment_ai_usage` owns that counter.
...
- `systemprompt-events` to ~93% line coverage: the outbox bridge's listener connect-failure retry loop under paused tokio time, the prune tick deleting exactly the expired rows, relay double-install and closed-pool insert-failure arms (local delivery unaffected), and broadcaster serialize-failure/eviction contracts. Residuals documented as unreachable: LISTEN failure on a healthy connection, serde_json failure on plain event structs, NOTIFY failing after a successful INSERT on the same pool.
- `systemprompt-files` to 97.0% line coverage: the whole `files.yaml` validator matrix (urlPrefix rejection, size warnings, malformed YAML), upload service edges (base64 limits, path-traversal rejection, DB-failure disk cleanup, IO failure), the ingestion job across seven extensions with skip-on-rerun and per-failure arms, AI-provider error mapping, and repository query/stats contracts. Defense-in-depth re-checks and mid-walk race arms documented as unreachable.
- `systemprompt-scheduler` to 92.5% line coverage with live-process behavioral tests: dispatch panic-capture/failure/overlap-guard and advisory-lock contention arms, POSIX cleanup against test-spawned children (SIGTERM→SIGKILL fallback, unmarked-live-PID refusal, port-owner mismatch), reconciler restart/orphan-sweep/stop arms, service stop lifecycles, and closed-pool error propagation across the maintenance jobs. Zombie-safe `try_wait` deadline polls throughout; command-missing and NOT-NULL-column warn arms documented as unreachable.
- `systemprompt-runtime` startup/builder gaps closed with an in-process full-bootstrap fixture and inventory-registered fixture extensions: the three `SystemAdmin` build-failure variants, marketplace filter-factory resolution, verbose/quiet startup validation across malformed content/web/metadata configs, missing internal-MCP manifest gating, storage-root validation failure, GeoIP/config context loaders (100%), and context-plane accessors. Own-suite coverage now exceeds the prior full-suite figure; defensive validator arms documented as unreachable.
- `systemprompt-config` to 97.0% line coverage: the full secrets bootstrap (file/env/Fly/subprocess sources, strict/warn/skip missing-file modes, seed generate-and-persist plus read-only-dir ephemeral arm, rotation, every accessor error), profile bootstrap resolution and rejection arms, `build_from_profile` path/signing-key/instance-id derivation with database-config validation, gateway-section route-id synthesis, variable-flattening including the unresolved-after-5-passes error, and validator/writer/skill edge arms. Compile-time-constant regex failures documented as unreachable.
- `systemprompt-sync` to 93.3% line coverage: full crate-deploy flow via a stub runner + wiremock (image tagging, docker login stdin, git-sha and every command-failure arm), pre-sync apply-from-cloud with backup/diff/apply verification, real DB push/pull round-trips including the partial-import mapping, content/access-control sync jobs against seeded disk+DB divergence, API-client 503-retry/401-remint arms, and tar traversal/symlink rejection with raw GNU headers. Extension-asset loop and cross-DB conflict arms documented as class-c for this suite.
- `systemprompt-generator` to 94.5% line coverage: the full prerender pipeline against real Handlebars templates and seeded content (list/item/locale routes, TOC, template-not-found and closed-pool arms), inventory-registered fixture extensions driving the component/extender/content-provider/page-prerenderer and copy-assets loops end-to-end, the >50k-URL sitemap chunk/index path via a single `generate_series` insert, RSS branding fallbacks and provider-failure arms, and web-config load/path fallbacks.
- `systemprompt-oauth` to 96.8% line coverage with real WebAuthn ceremonies (softtoken registration/authentication/link flows including tampered-challenge, replayed-token, and pending-challenge age-cap arms), decoded-claim JWT assertions across generation/authentication/authorization, CIMD dispatch and fetch-failure arms, auth-code replay revoking the refresh-token family, PKCE `plain` rejection, JTI single-use enforcement, and session-lookup fallback arms under a paused clock. TLS-only CIMD fetch arms and non-injectable `Instant` expiries documented as residuals.
- Expanded tests for database-error propagation, GeoIP enrichment, Slack transport and URL validation, header/cookie extraction, token rejection, marketplace filtering, template lifecycle, extension registration and database conversions. Shared fixtures include `closed_db_pool()` and the canonical web configuration.
- Tests assert returned fields, exact collection membership and CLI stdout content. `just lint-test-value`, included in the quality checks, rejects discarded unasserted results; the standards audit reports zero-assertion functions.
...
- Coverage uses continuous-mode profiling for the full instrumented suite and CLI subprocesses, excludes the separately measured bridge, uploads LCOV to Codecov and checks a 0.5-point regression threshold. Added `codecov.yml` and CI/coverage badges.
...
- `systemprompt-files`: `File.metadata` is `sqlx::types::Json<FileMetadata>` (was `serde_json::Value`) and `ContentFile.role` is `FileRole` (was `String`); the fallible `File::metadata()` and `ContentFile::parsed_role()` accessors are removed — use the typed fields directly. `FileRole` serde casing is now `snake_case`, so the OG-image role serializes as `"og_image"`.
...
- Gateway reasoning-model requests use the configured model output limit for the upstream completion budget. Non-reasoning models retain the caller’s requested token limit.
...
- Agent analytics now count completed and failed tasks correctly. The queries behind agent stats, leaderboards, and the overview filtered `agent_tasks.status` against `'completed'` / `'failed'`, but that column stores the canonical A2A `TaskState` value (`TASK_STATE_COMPLETED` / `TASK_STATE_FAILED`) enforced by the `agent_tasks_status_check` constraint, so completion counts and success rates always read zero. The filters now match the stored values.

## [0.15.0] - 2026-06-03

### Breaking

- A `gateway.providers` entry renames its `protocol` field to `wire` and adds a required `surface` field. Existing profiles must be updated: set `wire` to the codec the gateway speaks (`anthropic`, `openai-chat`, `openai-responses`, `gemini`) and `surface` to the client API family the provider's models are advertised under (`anthropic`, `openai`, `gemini`, or `backend`). See the API-surface entry below.

### Added

- A provider declares its client **API surface** independently of the **wire protocol** it speaks. `ProviderEntry` carries `wire` (the outbound codec the gateway speaks — `anthropic`, `openai-chat`, `openai-responses`, `gemini`) and a required `surface` (the client API family its models are advertised under — `anthropic`, `openai`, `gemini`, or `backend`). A `surface: backend` provider is routable through gateway routes but advertised to no client API, so a vendor can speak the Anthropic wire — reusing that codec — without its models masquerading as Anthropic models in any catalog. The advertisement rule has a single definition (`ApiSurface::is_advertised`, applied once via `ProviderRegistry::advertised_providers`), so a backend provider cannot leak into a client catalog through a hand-rolled flatten. `/v1/models` and the bridge profile scope their advertised set by surface through the `x-inference-protocol` header, and an unknown value (or `backend`, which is never a client surface) is rejected with `400` rather than widening the advertised set. The default catalog YAML and `admin config catalog` carry the new `surface` field alongside the renamed `wire`.
- A closed-allowlist posture for the gateway. `gateway.allow_unlisted_models` (default `false`) controls whether a model that matches no explicit route and is absent from the provider registry may be dispatched to `default_provider`: when off, such a request is denied (`403`) before dispatch rather than billed; when on, it is forwarded and a warning is logged. Route ids are synthesised deterministically from `(model_pattern, provider)` via FNV-1a so `access_control_rules` can address a route by a name that survives reordering and toolchain bumps.
- The audit trail records the client-requested model alongside the served model. `ai_requests` gains a `requested_model` column (migration 006): `model` holds the model actually served (after route rewrite and any upstream substitution — a request for `gpt-5` may be served by MiniMax as `MiniMax-M3`), while `requested_model` preserves what the client asked for, so an audit shows both. `NULL` for non-gateway requests and for rows written before the migration; surfaced through `infra logs audit` and the logging trace.
- A manual `infra jobs run` now records the run into `scheduled_jobs` so it appears in `infra jobs history`, mirroring the scheduler's own ticks. The record preserves the job's existing `next_run` and is non-fatal — recording is bookkeeping and never masks the job's own outcome.
- AI request latency is recorded when a request completes. The `ai_requests` completion update now derives `latency_ms` from the elapsed time between the row's `created_at` and completion, so request duration is available without joining against separate timing rows.
...
- The gateway maps an upstream failure to the status the client should see: a malformed request (`400`/`404`/`422`) or rate limit (`429`) passes through as the caller's to fix, while auth, `5xx`, timeout, and transport failures collapse to `502`/`504` without leaking the upstream's credential-level detail.
- The `analytics requests` and `infra logs request` command descriptions now state how the two overlapping surfaces differ and cross-reference each other — `analytics requests` for dashboard metrics (time range, model filter, CSV export), `infra logs request` for a quick operational view — and the `infra logs audit` help reflects that it reconstructs a full chain by request, task, or trace id.

### Fixed

- `infra logs` request, summary and audit commands render table and machine output to stdout through `render_result`. Empty results use message artifacts; nested aggregates render as compact JSON in cards. Removed per-command `--json` and `--full` flags on audit and `--full` on request show; use global `--output json`.
- JSON/YAML command output includes levelled notices in one structured `CliArtifact` on stdout, including notice-only commands invoked through MCP. Interactive notices continue to use stderr.
- Shutdown polls child processes for exit, treats zombies as terminated, signals agent and MCP children concurrently and aborts the event-relay task during drain.
- `systemprompt_models::wire::sse::frame_end` recognizes LF, CRLF, CR and mixed blank-line separators and returns the complete frame length. All provider codecs share the helper.
- The committed offline SQLx cache for `systemprompt-ai` is complete. Only a subset of the crate's query metadata was checked in, so an offline (`SQLX_OFFLINE`) build — including the one crates.io performs — could not verify the remaining queries; the full cache is now present.
- The bridge parses a Cowork `settings.json` that carries a UTF-8 byte-order mark. A file written by PowerShell's `Set-Content -Encoding utf8` is BOM-prefixed, which the JSON parser rejected with "expected value at line 1 column 1"; the leading BOM is now stripped before parsing.
- `GatewayConfig::candidate_routes` supplies both dispatch and authorization catalogs, including a synthetic default-provider catch-all when needed. Explicit catch-all routes are deduplicated.
...
- **Bridge 0.10.7:** Windows policy installation requests UAC elevation to write `HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Claude`. Activity messages identify the prompt and provide remedies for cancellation or access denial.
- Gateway stream finalization records upstream errors and missing stop events as failures, distinguishing empty from truncated streams. Content with no reported usage emits a cost-capture warning. End-of-stream and dropped-stream paths share this decision.
- Console and database logging share secret-field redaction and control-character escaping through `sanitize`. Secret markers, credential suffixes and exact auth/cert/pem field names render as `[REDACTED]`.

## [0.14.3] - 2026-06-02

### Fixed

- MCP session recovery reads persisted non-null `initialize_params` regardless of worker status. Recovered sessions return to `active` on activity; client deletion clears initialization parameters.
...
- Trace status derives from associated agent tasks, AI requests, MCP executions and error logs. It is non-null and uses `running`, `failed`, `canceled` or `completed`; status filtering applies consistently.
- Registry-based process signaling verifies spawn identity through `live_pid_is_subprocess`. Foreign or recycled PIDs are not signaled and their registry records are cleared. The check covers agent and MCP reconciliation, lifecycle operations and cleanup.
- The OpenAI Chat Completions wire codec emits the output-token limit as `max_completion_tokens` rather than the deprecated `max_tokens`. The gpt-5 and o-series models reject `max_tokens`, while `max_completion_tokens` is accepted by every Chat Completions model, so the field rename unblocks those models without affecting the others.
...
- The gateway credential extractor prefers the `Authorization` header over `x-api-key` when both are present.
...
- Manifest assembly scopes skills, agents, MCP servers and plugins through the active marketplace’s `PluginComponentRef` includes before RBAC filtering. Empty includes retain the global fallback. In this release, zero marketplaces leaves the catalog unscoped, one selects that marketplace, and multiple select one with a warning.
...
- Route context middleware uses four typed layers: `PublicContextMiddleware`, `UserOnlyContextMiddleware`, `A2AContextMiddleware` and `McpContextMiddleware`. Each implements its caller-admission and context-construction contract.
...
- **`SharedAuthzHook` re-exported from `systemprompt_security::authz::hook`**. The `authz` facade re-export is unchanged, so `systemprompt_security::authz::SharedAuthzHook` continues to resolve.
...
- Updated non-test crates for the workspace lint profile: narrowed unused public visibility, exposed public argument types, normalized owned-string and clone operations, documented scoped lint exceptions and removed unfulfilled expectations.
...
- `web validate`, `web sitemap show` and runtime landing-page loading use `parse_content_config`, accepting both direct content configuration and a top-level `content:` wrapper.
- `web validate` resolves configured logo and favicon `/files/` URLs against `profile.paths.storage_resolved()`, matching runtime asset serving.
...
- `admin agents task` uses the active CLI session unless `--token` is supplied explicitly. Task and message commands share the A2A helper and validate agent existence before dispatch.
...
- `admin keys issue-plugin-token` issues a service principal with `hook:govern`, `hook:track` and `aud=hook`. Administrative authorization is required for issuance. Re-issue existing plugin tokens to adopt the narrower scope.
...
- Bridge PAT, device-certificate and session-code exchanges persist the JWT’s backing session with request analytics at issuance. Migration `006_user_sessions_source_bridge_mcp.sql` permits `bridge` and `mcp` session sources.
- Bridge credential exchange sends the process-stable `x-session-id`; the gateway binds the JWT and persisted session to it. Session creation is idempotent for token renewal. Cached tokens with a different session ID are discarded.
...
- Removed `jwt_secret`, its accessors, validation and environment propagation. Delete `jwt_secret` from `secrets.json` and configure `oauth_at_rest_pepper` with at least 32 characters. Subprocesses receive `OAUTH_AT_REST_PEPPER`.
...
- Removed redundant per-item Rustdoc from library crates while retaining documentation of public contracts and required implementation constraints.
...
- RS256 decoding keys use PKCS#1 `RSAPublicKey` DER through `EncodeRsaPublicKey::to_pkcs1_der()`. Removed the unused SPKI error variant and import.
...
- `BootstrapLockGuard` retains one pooled PostgreSQL connection for schema installation so advisory lock acquisition and release use the same session.
...
- Trace listing includes agent-task-only traces and resolves agent and status fields from the most recently updated task, excluding empty trace IDs.
- **`ai_requests.trace_id` propagation failures surface in the operator log.** `systemprompt-ai`'s record builder now emits a `WARN` carrying `request_id` when the supplied `RequestContext` carries an empty `trace_id`, instead of omitting the column and breaking the trace ↔ `ai_requests` join in `infra logs trace list`.
...
- **Breaking:** `RequestStorage::store` is async and returns `Result<(), AiError>`. AI service paths await and propagate the primary request insert; secondary message, tool, session and analytics writes log failures. The internal write module is `request_storage::writes`.

### Docs

- Security documentation describes RS256 validation, token exchange, the signing authority, JWKS discovery, trusted issuers, Ed25519 manifest signatures and the OAuth at-rest pepper.
...
- Schema installation runs structural DDL, pending migrations and dependent DDL in that order. Migration-added columns are available before index and view creation; migrations can alter declared cross-extension tables.
...
- **Breaking:** `SqlExecutor::parse_sql_statements` returns `DatabaseResult<Vec<String>>` and parses PostgreSQL syntax with `sqlparser`. Named dollar quotes and apostrophe-quoted function bodies are supported. Parse failures propagate through repository and installation errors; callers must handle the result.
...
- SQL parser tests cover named dollar-quoted bodies, apostrophe-quoted function bodies and malformed SQL returning an error.

## [0.9.2] - 2026-05-12

### Fixed

- CLI credential bootstrap handles `CloudError::CredentialsFileNotFound` by initializing empty credentials. Other credential errors propagate; the wrapper matches the typed error directly.
- Schema statement parsing recognizes function bodies separately from `CREATE TRIGGER` statements, preserving subsequent statement boundaries.
...
- Analytics repository tests serialize database fixtures through a shared async mutex to bound concurrent pool usage.
...
- Gateway context IDs are derived for requests across supported inbound protocols, including clients that omit `x-context-id`.
...
- Removed `just check-bans`, `just check-bans-crate` and the corresponding quality job. SQL query restrictions use the SQLx checks; identifier and naming conventions remain defined by contributor standards for this release.
...
- Fly.io credential validation propagates API validation failures. `SYSTEMPROMPT_ALLOW_UNVALIDATED_CREDS=1` explicitly permits unvalidated credentials in this release.
...
- **Breaking:** removed `systemprompt::prelude::{Entity, EntityId, GenericRepository, RepositoryExt}`. Migrate to per-entity repositories using `sqlx::query!` or `query_as!`.
...
- Static content lookup includes `source_id` through `ContentPageRequest` and `get_by_source_and_slug`. A slug in another content source does not satisfy the route.
- Process-spawn and file-open errors include the binary or path and relevant domain identifiers across MCP, scheduler, sync, agent and CLI paths.
...
- Gateway `/v1/messages` requests persist audit records and apply configured policy, quotas and safety scanning around provider dispatch.
...
### Fixed
- `admin agents registry` resolves its gateway from explicit `--url`, then the active profile’s `api_external_url`, then `http://localhost:8080` when no profile is loaded.

## [0.2.3] - 2026-04-20

### Fixed
- Local-trial session creation uses `CloudConfig::is_local_trial` and `Profile::is_local_trial`. Local profiles can execute local commands without cloud credentials; missing local email hints use `admin@localhost.dev` in this release. Cloud operations retain credential requirements.
...
### Fixed
- Agent migrations support repeat execution after partial application. Task-state migration includes `pending` normalization; AI request foreign-key creation checks for an existing constraint.
...
- Agent cost breakdowns include an `unattributed` aggregate for requests without an agent association. The top-N limit applies only to attributed agents. Tests cover mixed attribution, limit behavior and empty windows.
- Registered `003_a2a_v1_task_states.sql` in `AgentExtension::migrations` to normalize existing task states and update their CHECK constraint during migration.
...
- **Breaking:** removed `CreateAiRequest` and `AiRequestRepository::create`. Use `AiRequestRecord` with `AiRequestRepository::insert`, including its optional `task_id`.
...
- Migrated internal ID fields to typed identifiers and added `JsonSchema` for `LogId`. WebAuthn registration methods return `UserId`. External protocol values and partial CLI lookups retain string representations under their boundary rules.
