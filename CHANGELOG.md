...
- A model the gateway can serve only through its synthetic catch-all route is no longer denied as an unknown authorization entity. When a profile sets `gateway.default_provider` but declares no explicit `"*"` route, the dispatcher synthesises a catch-all route at request time, but that route's id was absent from the authz entity catalog, so requests matching only the default route failed authorization. `GatewayState::resolved_route_ids` — the single source the catalog is materialised from — now appends the synthetic default route's id (deduplicated against an operator-authored `"*"` route), keeping the catalog in lock-step with the dispatcher's reachable set.
