...
ways that cancelled out just often enough to hide: reasoning tokens were
counted at the wire and lost before the audit row, cached prompt tokens were
billed twice on three of four wires, and a reply the wire could not parse
became a free empty turn. All of that now runs through one usage type and one
cost function. Second, a marketplace is no longer a singleton: the bridge
manifest unions every enabled marketplace, records which one owns each plugin,
and the Claude Code CLI mirrors one host marketplace per gateway marketplace.
Third, a retired migration slot can be declared spent, which closes a class of
"edited since it was applied" errors that named files nobody could find.

### Added

- **Bridge manifest:** assembly unions every enabled marketplace instead of resolving one active marketplace, and `MarketplaceMembership` records which enabled marketplaces own each plugin, agent and MCP server, so an entry several marketplaces carry is admitted when any one of them admits the subject and a deny on the entry or its plugin still closes the cascade first. `marketplaces` names each enabled marketplace with its display name and the plugin ids it still carries after the per-user filter; a marketplace denied at its own level, or left with no surviving plugin, is not listed. Two enabled marketplaces without a `settings.default_marketplace_id` now assemble instead of failing closed. The field is additive: an older manifest deserialises unchanged and a bridge reading an empty list falls back to the single `org-provisioned` marketplace it wrote before.
...
- **Gateway:** a transient upstream `429` or `503` is retried up to four times with exponential backoff from 1s, a 30s ceiling, 25% jitter and `retry-after` honoured when it asks for longer than the curve. Vertex MaaS sheds load with 429 and the gateway relayed it straight through, so `deepseek.v3.2` failed on every capacity blip and OpenCode stopped dead. Retrying is safe only before a response byte has been relayed; every other status, and the final 429/503 once the budget is spent, still relays verbatim. Retries increment `gateway_upstream_retries_total`.
- **Gateway:** a URL image in a Gemini request is fetched and inlined by the dispatch pipeline rather than downgraded to a text part by the codec, which had the model answering about a prompt that had lost one of its inputs. The fetch is bounded by a whole-operation timeout, a 5 MiB cap enforced while the body streams, a content-type allowlist of what Gemini decodes, and manual redirect following so every hop is re-checked. The SSRF guard reuses `validate_outbound_url_with_trust` and the shared block list, tightened for a caller-supplied URL: loopback is not an implicit allow, and the hostname is resolved so the DNS answer is checked too. Only Gemini pays the latency; Anthropic and OpenAI accept a URL natively.
- **Gateway:** a Google service-account secret is exchanged via RFC 7523 for an OAuth access token, cached per secret and retired early so one cannot expire in flight. Vertex rejects API-key auth as a class, so nine Vertex MaaS models looked configured and failed on first use. Detection is by content — Google's own `type: service_account` — and any other secret is sent verbatim exactly as before. The resolver carries how a credential is presented alongside its value, so a minted token goes out as a bearer and an API key on `x-goog-api-key`, on the buffered and streaming lanes alike.
- **AI:** reasoning tokens are counted and billed on every wire, buffered and streamed — Gemini `thoughtsTokenCount`, OpenAI Responses `output_tokens_details.reasoning_tokens`, OpenAI Chat `completion_tokens_details.reasoning_tokens` — and persisted on `ai_requests.reasoning_tokens`. They were dropped by construction: a `gemini-2.5-flash` turn that burned a 200-token thinking budget recorded 6 output tokens and billed for 6. Reasoning is a breakdown *of* `output_tokens`, never an addition, and `CanonicalUsage::normalise_reasoning` enforces that at runtime: a breakdown exceeding its parent, or a wire total overshooting `input + output` by exactly the reasoning count, is folded into output and warned about with the provider named, so an OpenAI-compatible third party that reports additively announces itself in the log rather than in the bill. The internal AI path records the count too. `infra logs audit <id>` reports it, `infra logs request list` renders it as a share of the output figure (`27/200(194r)`), and the analytics cost and request summaries report reasoning, cache-read and cache-creation tokens, which were stored per request and dropped on the way into every rollup.
- **Database:** a migration slot the tree has retired can declare itself spent. `NNN_<name>.tombstone` — or `NNN-MMM_<name>.tombstone` for a retired chain — records the number with no SQL and a prose body; `build.rs` treats it as occupied, so reusing the number fails the build rather than a deployment, and the runner never executes, records or checksums it. A shipped migration keeps its tracking row forever, so deleting the file does not give the number back, and nothing said so: the runner keyed on version alone, refilling a spent number looked exactly like editing the migration that used to live there, and the operator got "Migration 34 has been edited since it was applied" naming a file nobody could find, with both offered remedies wrong. The recorded name is now asserted against the file in the slot — a mismatch is `MigrationSlotReused`, naming both files — and applied versions no file claims are reported as `orphaned` by `migrate-status` and warned about at boot, never fatal, because every established database carries such rows.
...
- **Loader (operator-visible):** a provider whose `api_key_secret` does not resolve in the secret store is demoted to `surface: backend` before validation, so its models leave `/v1/models` and every client picker rather than being offered and 502-ing on first use — nine Vertex MaaS models were advertised by an instance that could not dispatch one of them. Deliberately not a boot failure: an instance serving Anthropic must still start when an unrelated credential is absent, and an uninitialised secret store means "unknown", never "absent". The warning names the provider only; it had also carried the secret's name, which the logging layer redacts on any field called `secret`, so it rendered `[REDACTED]`. `cloud doctor` names the secret in full, and falls back to the repository's services tree off-container instead of reporting success with credentials unchecked.
- **CLI (operator-visible):** both `migrate-repair` paths refuse outright when a slot collision is present, on the dry run as well as under `--apply`, and point at the tombstone mechanism. Repair matched recorded rows on `(extension_id, version)` alone, so where the file in a slot carried a different name than the row, repairing stamped one migration's checksum onto a row describing another and permanently silenced that row's drift detector. `migrate-status` labels collision rows; `--apply` stops reporting `0 migration(s) re-applied` for work it has just done, and the reconcile-only path states plainly that no SQL was executed.
...
- **Gateway (accounting):** Anthropic adaptive-thinking tokens reach `ai_requests.reasoning_tokens`. Claude 5 models report the thinking share as `usage.output_tokens_details.thinking_tokens`, on the buffered reply and on the trailing `message_delta` of a stream, and the Anthropic codec read neither: a live `claude-opus-5` turn with `thinking: {type: adaptive}` audited 4302 output tokens and `reasoning_tokens = 0`. The count is copied across as the same breakdown-of-output every other wire records, so `output_tokens`, `total_tokens` and cost are unchanged; the passthrough lane shares the codec and is covered too. A model reporting no details still yields 0.
...
- **Security (governance):** the high-entropy secret backstop exempts provider-signed reasoning blobs. Gemini's `thoughtSignature`, Anthropic's `signature` and `redacted_thinking` `data`, and OpenAI's `encrypted_content` are opaque base64 the client must echo back verbatim, so an enforcing `secret_scan` stage denied every multi-turn thinking continuation. The exemption is the entropy detector only, at those JSON paths only, with the generically named keys granted it only when the enclosing object declares the reasoning block type that owns them; every vendor pattern still runs there.
...
- **Gateway:** an upstream 2xx whose body carries no turn is a 502 rather than a successful empty answer. A buffered `/v1/messages` call to a Vertex MaaS model returned 200 with `content: []`, zero tokens and no error, twice in three calls; the buffered parsers are total by design, so it became a well-formed canonical response relayed as a turn in which the model said nothing, audited as completed. `buffered_body_defect` separates "nothing came back" from "the model legitimately produced no text"; the raw body is relayed as a genuine non-2xx would be and the audit row is failed with the excerpt.
...
- **Gateway:** every terminal signal that silently discarded a tool call is closed through one rule: a generic end-of-turn beside a tool call becomes `ToolUse`, and truncation always wins, because a call cut mid-arguments carries JSON the client cannot run. Applied at openai_chat buffered and streamed (Cerebras, vLLM builds and Moonshot all send a plain `stop` beside a complete `tool_calls` array), anthropic buffered and streamed, gemini, openai_responses, the stream tap's accumulator, and the same-wire Anthropic passthrough, which relays the body unparsed and now rewrites exactly the `stop_reason` token when the body contradicts itself. The Anthropic streaming codec carries block state, so the audit and the client agree on `tool_use`.
- **Gateway:** streamed `reasoning_content` on the openai_chat wire reaches the client as canonical thinking; the buffered parse mapped it and the streaming half dropped it. A reasoning model is detected from the catalog's `max_thinking_budget` rather than a hardcoded prefix list. Gemini's implicit thinking path sends no `thinkingConfig` at all — sending an implicit budget switched thinking *on* for models Google ships with it off, and `gemini-2.5-flash-lite` answered "say hello in one word" with 4,373 thought tokens — and only `maxOutputTokens` is raised.
- **Gateway:** model limits resolve by the requested catalog id rather than the upstream name, and the upstream model name comes from the catalog entry the route resolved to; `ProviderModel::upstream_model` was parsed, validated and read by nobody. Both Vertex entries had lost their output cap and thinking budget, which is why the MaaS thinking models still returned empty text at `max_tokens: 128` after the wire-side fixes landed. A truncated buffered Responses turn is marked incomplete rather than `completed`, and neither Responses lane emits the `stop_reason` key the contract does not define.
...
- **Database:** a fresh install's baseline stamp commits in the same transaction as the structural DDL it describes. An install that failed part-way left earlier extensions holding their tables with no baseline, so the next install called them established and executed migration SQL against a schema shape the declarative baseline had already moved past, failing identically on every retry. A tombstoned slot is also no longer put through slot-identity verification, which refused to boot on precisely the databases tombstones exist for. `PendingMigration.no_tx` was hardcoded `false` at both construction sites.
...
Two threads run through this release. A Mac install of the bridge on 2026-09-03
failed three separate ways at once, and all three are fixed here along with the
architectural reason two of them were invisible to every test we had. Separately,
governance gained a third posture: a policy, a safety scanner or a quota window
can now record its finding and let the call through, so a deployment can see what
enforcement *would* have cost before it pays it.

### Added

- **Governance:** `mode: enforce | warn`, per policy with a `governance.mode` default. `Decision::Warn` carries the same `DenyReason` the enforcing form would have, so a warn row and a deny row are directly comparable; every enforcement point treats it as an allow and `Decision::permits` is the single predicate. Warn does not halt the chain, so the audit row carries *every* finding on the call rather than only the first. An unknown mode value is a parse error — reading it either way silently changes what is enforced.
- **Gateway:** `safety.mode: warn` keeps every scanner running and every finding persisted and drops only the refusal; `ai_safety_findings` gains a `blocked` column stamped by the same predicate that decides the refusal, so the report cannot disagree with what the gateway did. `quota_mode: warn` still reserves against every window and evaluates every ceiling, then writes the breach to `governance_decisions` under policy `quota` and proceeds — the plane most likely to be hit by ordinary development no longer answers 429 in a deployment that blocks nothing.
...
- **Bridge:** `install --host <id>` (repeatable, `--hosts all`) enrols a host app from the CLI, and `uninstall --host <id>` un-enrols one while leaving the bridge installed. `HostApp::install_profile` had been reachable only from the GUI's Re-apply button and from `reapply_stale_profiles`, which by design touches only hosts that already carry a profile — so on Linux, whose only setup path is `install`, no host profile was ever written and every request to the loopback proxy came back `403 no loopback credential presented`. Enrolment shares `build_profile_inputs` with the re-apply path, so an enrolled profile and a repaired one come from the same live port, secret and models; the 403 body and the re-apply hint now name `install --host`, which is the actual remedy when a client was never enrolled.
...
- **Users:** anonymous visitors no longer appear in user listings. `create_anonymous` stores every anonymous visitor as an ordinary `users` row (`roles = ARRAY['anonymous']`, `<fingerprint>@anonymous.local`), so `list`, `list_all`, `search` and `count` were presenting traffic as people — a page of 50 in the admin directory spent its budget on visitors before it reached an account. Exclusion is now the default and the opt-in is explicit (`list_including_anonymous` and siblings). `--role anonymous` is unaffected; it routes through `find_by_role`.
- **Bridge:** sign-in survives another process holding the browser-callback port. `LoopbackServer::bind` bound a single hardcoded `127.0.0.1:8767` with no fallback, so anything that claimed it first — Docker Desktop's `com.docker.backend.exe` does — made every sign-in fail at bind with a raw `os error 10048`, and the wedge was unrecoverable: the token cache latched and asked for re-authentication, which could not start. `bind` walks `LOOPBACK_PORTS` (8767 first, so firewall rules naming it are unchanged), stepping over a candidate only on `AddrInUse` so a genuine bind fault is still reported against the port that had it. The failure also never reached `bridge.<date>.log` — the only artefact available when nobody is at the machine — because it went straight to a GUI toast.
...
- **Bridge:** a trust-on-first-use pin that never landed is no longer silent. A failed config write was downgraded to a warn line and the fetched key returned anyway: the sync succeeded, nothing was pinned, and every later sync re-trusted whatever the gateway served. `ConfigWriteError::Malformed` makes that permanent, so the failure now reaches the activity log with its underlying error, and `BridgeContext` carries the session fact so validate can tell "fetched but not written" from "never provisioned".
- **Bridge (macOS):** the managed-preferences store read only `CFString` and returned `None` for every other property-list type. The two keys Cowork depends on most are arrays, so a correctly-provisioned Mac reported `allowedWorkspaceFolders: missing` and `managedMcpServers: not written` while `defaults read` showed both present — and the remedy each named could never work, because sync had already written them and the reader could not see them. Values serialise to JSON now, with a bare string kept bare so callers comparing against plain values still match.
...
- **Bridge:** a white-label build printed the hardcoded systemprompt host as the MDM snippet's fallback gateway, which an admin could paste verbatim; it falls back to the brand's own `default_gateway_url`. `docs_url` points at `/documentation`, a page that exists on its own, because both footers link it verbatim rather than appending a platform. `render_index_from`'s `__CORE_VERSION__` placeholder carried `COMPAT_VERSION` — the bridge's compatibility version, not core's — and is renamed `__BRIDGE_VERSION__` to match the value it holds.
...
- **Bridge:** the Claude Code enterprise MCP policy is never written again, and any copy an older bridge left behind is removed on every sync. `managed-mcp.json` put the CLI into exclusive mode and `allowManagedMcpServersOnly` in `managed-settings.json` denied every server not on the allowlist; Claude Desktop 1.44121 applies both to Cowork sessions, so Cowork's built-in workspace server (the sandbox bash tool) was refused by permission rule and every skill that shells out failed while the bridge showed green. MCP servers reach the CLI through per-plugin `.mcp.json` files, and Claude tools are never blocked by bridge policy. A regression test holds the settings document free of the lock keys.
- **Bridge:** sync re-asserts the whole Claude Desktop managed policy on Windows (`allowedWorkspaceFolders`, the telemetry, auto-update and deployment-mode hardening flags, the org UUID, the manifest pubkey pin, the egress allowlist and the managed MCP list) and creates the pre-trusted workspace directory, asking for one administrator approval only when a value has actually drifted. Previously only `managedMcpServers` was refreshed, so a fresh or wiped machine left Cowork prompting on `request_cowork_directory` with auto-updates back on. `install --apply` no longer writes a per-user `HKCU` copy when unelevated (Cowork ignores it once the machine key exists); it stages the same single elevated pass. The probe now reads `HKLM` as authoritative over `HKCU`, matching what Cowork reads.
...
- **Bridge:** `disableNonessentialServices` is enforced as `false`. Claude Desktop 1.44121 reads that flag to block the claudemcpcontent.com renderer that MCP display extensions load from, so `true` left every dashboard reporting "There was a problem displaying content". `validate` fails when the value is `true`, when `disabledBuiltinTools` or `builtinToolPolicy` name `Bash` (the only policy keys that deny Cowork's shell), or when a `managed-mcp*`/`managed-settings*` file sits in the Claude Code policy directory.
- **Bridge:** sync stages the manifest's dashboard bundle (`manifest.json` plus one page per record) into the pre-trusted workspace under `systemprompt/artifacts/`. That folder is connected to every Cowork session, so the setup skills install dashboards with file tools alone; Claude Desktop 1.44121 denies Cowork's own shell tool (it passes `--disallowedTools Bash` and the engine now applies a `Bash` deny to `mcp__workspace__bash`), which had made the shell-based staging script fail on every install.
...
- **CLI:** `admin setup` and the `admin config catalog|gateway` editors write an `includes:` entry that actually resolves. The catalog and routes are written to `<services>/ai/providers.yaml` and `<services>/ai/gateway.yaml`, but the include was registered against the root aggregator as `ai/providers.yaml` — and an include resolves relative to the file that lists it, `<services>/config/config.yaml`. The next boot died with `include file not found: <services>/config/ai/providers.yaml`, so a freshly set-up deployment could not start. The include text is now `../ai/…`, held in its own constant so the filesystem path and the include path can no longer be confused, and a test pins the two together.
...
- **MCP:** a failed service start reports itself again. `LifecycleHandler` treated `ServiceStartRequested` as a command and started the server from inside `EventBus::publish`, so `start_services` started every server twice on the success path, and on failure the handler's error propagated out of `publish` before the orchestrator's own error arm — no `ServiceFailed` event was ever emitted and the aggregation branch was unreachable. `ServiceStartRequested` and `ServiceRestartRequested` are notifications now; the orchestrator owns the start path and publishes `ServiceFailed`, and the daemon starts its servers explicitly. `restart_services` no longer touches processes, which is what it always claimed to do.
...
- **Security:** `--help` no longer prints the live value of an environment-bound argument. With clap's `env` feature the help text interpolates whatever the variable currently holds, and `Secrets::to_subprocess_env` populates `GEMINI_API_KEY`, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GITHUB_TOKEN` and `DATABASE_URL` in every spawned MCP server — so an agent that ran `systemprompt admin setup --help` was handed a live provider key, and a production gateway scan caught one reaching a model. Every secret- and PII-bearing argument now renders `[env: GEMINI_API_KEY]` with no value; the harmless ones (`--db-host`, `--db-port`, `--db-name`, `--default-provider`, `--tenant-id`) still show theirs. The widest exposure was `--database-url`, which is `global = true` and so leaked its credentialed URL into the help of every subcommand.
...
- `admin bootstrap` no longer synthesizes a default owner email that the typed `Email` identifier rejects, which left fresh profiles unable to run `admin session login`. The owner email is explicit configuration: the profile's new `system_admin.email` (typed, validated at parse; `SYSTEM_ADMIN_EMAIL` for env-built profiles) or a validated `--email`; creating the owner row without one is refused. Generated profiles write `system_admin.email` explicitly; already-bootstrapped installs are unaffected.
...
- Governance inspects the exact bytes the gateway forwards upstream, not a reconstruction of them. The outbound adapters gain `build_body`, which renders the upstream payload without sending it, and every string in that payload — including object keys and content the canonical model cannot represent — is attached to the request as `CanonicalRequest.forwarded_surface` before any scanner runs. Requests relayed as the caller's own bytes were previously scanned only through the lossy canonical form, so a content block the inbound parser does not model, an image, `structuredContent`, or `_meta` reached the provider unscanned. The walk is iterative and bounded on depth, leaf count, total bytes, and per-leaf size; exhausting a budget marks the surface truncated and logs it rather than reporting a clean scan.
...
- Inference requests run through the same four-stage `GovernanceEngine` that governs MCP tool calls, on the same instance — the rate limiter's buckets are instance-scoped, so a second engine would have given inference its own budget and silently doubled every operator limit. Denials render with the deciding policy id, and every evaluation is persisted on allow as well as deny, since the chain trace is the audit product. Governance runs before the scanner plane, so a denied request produces exactly one audit row and one response.
...
- `GatewayConfig::validate` rejects a route that cannot be costed — a route-level `pricing:` override must be billable, and otherwise every registry model the pattern reaches must carry usable rates and the pattern must reach at least one. Uncosted inference is a configuration bug rather than a runtime warning: the request bills at zero and the gap stays invisible until someone reads the ledger. This is what catches a glob route aimed at a catalog that has fallen behind the models actually in use.
...
- The bridge distinguishes the two ways a complete host profile can still fail. Both produce an identical 403 "bad loopback secret" and share a fix, but the message now names the cause: a baked secret the live proxy does not verify against, or a baked base URL pointing at a port the proxy no longer holds. Staleness is asserted only on a definite mismatch — a remote gateway or an unparseable URL is not this check's business — because a false "re-apply required" trains operators to ignore the real one.
- The gateway strips `metadata.user_id` on every path. It was removed only when the byte-passthrough lane was rejected up front, but an adapter can also decline that lane at render time — a request body that is not a JSON object falls back to the canonical build — and on that path neither strip ran, so the caller's end-user identifier was forwarded. The passthrough lane now re-applies the same three policy transforms in place (route model, output-token ceiling, identifier strip), so which lane a request takes never changes what identity leaves the gateway.
- `bridge login --no-browser` accepts what the consent page actually shows. The browserless path sent a loopback `redirect` no process was listening on, told the user to wait for a failed navigation, and then parsed the address bar; it now requests no callback, so the page displays a one-time code. The paste handler takes a bare code, the full `login --code … --gateway …` line the page displays, or a callback URL, and strips the bracketed-paste escape sequences a terminal wraps around a paste — nothing removes those from a raw stdin read, so a code the user could see was correct was rejected. The exchange code's lifetime is 10 minutes rather than 2, sized for a person reading it off one screen and typing it into another.
...
- Streamed usage is no longer summed across frames, taken from a placeholder, or partially erased. Providers report a complete cumulative snapshot, so the wrapper replaces counts rather than adding them; the stream tap takes the last snapshot outright rather than keeping the largest value per field (which let a stale `message_start` estimate survive a real later zero and left `total_tokens` permanently stale); a request counts as having reported usage only when a real usage event arrived, since `message_start` carries small non-zero placeholders; and a zero count is no longer forwarded as if it were reported, because `CanonicalUsage` has no presence bit and doing so let a frame reporting only `output_tokens` erase an earlier `input_tokens`.
- A streamed request bills for its input and cache tokens. An Anthropic `message_delta` states `output_tokens` alone, and every absent count parsed as a zero, so folding that frame in as though it were a complete snapshot overwrote the input and cache totals `message_start` had established — an agent loop resending a large cached prompt lost almost its entire basis. An update now states only what its frame reported, and applying one overwrites those counts and leaves the rest. A stated zero still overwrites, so a stale `message_start` estimate cannot outlive a real later report.
## [0.27.0] - 2026-07-29
...
- `JobConfig.parameters` reaches scheduled and bootstrap runs; previously only manual/API runs received a populated map, so no cron-scheduled job could read a parameter and every retention window was a Rust constant.
...
- Startup no longer reports `tools=0` for an OAuth-gated MCP server. Such a server is validated by port reachability alone — no token exists to call `tools/list` with — and the fabricated zero was indistinguishable from a server that genuinely exposes no tools, on the startup log, `ServiceStatus`, and the `McpServerReady` event alike. The count is now unmeasured in that case: the startup line omits the `tools` field, carries `validation_type`, and reads "MCP service validated; tool list not enumerated".
...
- **Breaking:** charts render as inline SVG rather than through Chart.js. The chart and dashboard renderers no longer load a script from `cdn.jsdelivr.net`, so both now emit a plain strict CSP instead of one carrying a script-src exemption for that host, and `assets/js/chart.js` is gone. A rendered artifact is now genuinely self-contained: it draws identically with no network, which is what a strict CSP always claimed. Anything asserting on the old `<canvas>`, on `window.CHART_CONFIG`, or on the CDN in a CSP header needs updating.
...
- Gemini thought parts round-trip end to end. Thought summaries (`"thought": true` parts) previously deserialized as plain text — the model's reasoning leaked to clients as answer text and its `thoughtSignature` was silently dropped. They now parse and stream as real thinking blocks, `thinkingConfig.includeThoughts` is requested whenever thinking is enabled, and replayed history returns thought parts with their signatures verbatim, per Gemini's documented contract. (`CanonicalContent::Thinking` gained `id`/`encrypted_content` fields and a `CanonicalEvent::EncryptedContentDelta` variant in `systemprompt-models` — a breaking change for canonical-model consumers.)
- OpenAI Responses reasoning is replayable. Reasoning-enabled upstream requests send `store: false` so encrypted reasoning content comes back; reasoning items are emitted one per thinking part with their provider `id` and `encrypted_content` (previously a single id-less summary item, which the API rejects); and the Responses inbound adapter round-trips a client's reasoning ids and encrypted content instead of discarding them and inventing synthetic ids.
...
- One safety finding no longer denies the rest of a conversation. The request-phase scan read `CanonicalRequest::flatten_text`, which concatenates the system prompt and every message of every role, so each turn re-judged everything the caller had already sent: once any finding landed in a blocked category, every subsequent turn in that conversation returned `403` with no new offending input, which is indistinguishable from a broken client. It is worse than a plain replay, because a tool call the policy layer *correctly refused* stays in the assistant message and is rendered back into the scan surface as `[tool_use:…]` on every later turn — the deployment did the right thing and was punished for it for the rest of the session. The built-in scanner now judges the system prompt and the newest user turn; earlier turns are scanned only when `safety.history` asks for them, and their findings deny nothing unless it is set to `block`. Blocking is phase-aware rather than matching `block_categories` against a bare category string.
- The credit-card detector no longer splices unrelated numbers into a card. It stripped every non-digit from the whole flattened conversation and slid a 16-digit window over the result, so a version string, a timestamp and an identifier — in one message or across several — could concatenate into a sequence that passes Luhn while nothing in the conversation is a card number. Digits are now collected per message into runs that only a space or a hyphen may interrupt, which is how a card is actually written. Combined with the replay above, a single accidental splice used to poison every remaining turn.
...
- Gateway requests are attributed to their caller in the `logs` table. The gateway router is nested without the context middleware — it authenticates inside the handler — so the access-log middleware ran before any principal existed and recorded every request as the platform owner, with `SessionId::system()` and `TraceId::system()`. Read back through `infra logs trace show`, a single rejected request appeared as two users acting at the same instant: one `ai_requests` row under the real caller and a sibling trace under the owner. The handler now hands the resolved principal back on the response and the access log uses it, falling back to the platform actor only when the request was rejected before it authenticated. These rows also carry `"kind": "access_log"` so they can be excluded from per-user activity regardless of attribution.
...
- The per-session usage counters on `user_sessions` (`task_count`, `message_count`, `ai_request_count`, `total_tokens_used`, `total_ai_cost_microdollars`) were read by analytics queries but never written — the only writer chain hung off provider seams that no composition root ever injected. They are now wired directly: `TaskRepository` increments `task_count` on task creation and `message_count` for both messages persisted by `update_task_and_save_messages`, and the agent subprocess boot injects `AnalyticsAiSessionProvider` into `AiService` so AI usage totals accumulate. The redundant, never-called `increment_ai_request_count` mutation is removed (`increment_ai_usage` already owns that column).
...
- The `infra logs` request/summary/audit commands now print their data to stdout in interactive table mode. `infra logs request list`, `request show`, `request stats`, `summary`, and `audit` built their text output from `CliService::section`/`info`, which write to stderr, so `systemprompt infra logs request list > out.txt` produced an empty file while the table appeared on the terminal. These commands now route both interactive and machine output through `render_result`, rendering a box-table (list) or a presentation card (show/stats/summary/audit) to stdout, with the empty/not-found case emitting a `message` artifact. This is the table-mode analogue of the JSON-mode stderr fix below. The redundant per-command `--json`/`--full` flags on `audit` (and `--full` on `request show`) are removed; use the global `--output json` for machine output. Nested aggregate fields on `stats`/`summary` render as compact JSON within their card sections.
...
- Streaming SSE frame detection is now line-ending agnostic across every provider codec. The four codecs (Anthropic, Gemini, OpenAI Chat, OpenAI Responses) shared a copy of a frame splitter that only recognised `\n\n` and drained just those two bytes; an upstream that terminated events with `\r\n\r\n` or `\r\r` was never re-framed, and a `\n\n` frame left a stray carriage return in the buffer. Frame boundary detection now lives in one `systemprompt_models::wire::sse::frame_end` helper that recognises `\n\n`, `\r\n\r\n`, `\r\r`, and mixed pairings and reports the full event length including its blank-line terminator.
...
- A model the gateway can serve only through its synthetic catch-all route is no longer denied as an unknown authorization entity. When a profile sets `gateway.default_provider` but declares no explicit `"*"` route, the dispatcher synthesises a catch-all route at request time, but that route's id was absent from the authz entity catalog, so requests matching only the default route failed authorization. The dispatcher and the catalog now derive their routes from one registry-aware `GatewayConfig::candidate_routes` set — `resolve_route` finds within it and `dispatchable_route_ids` materialises it (deduplicated against an operator-authored `"*"` route) — so the reachable set and the authorization catalog cannot drift.
...
- A streamed gateway response that ends without an upstream stop event is recorded as failed rather than completed. The end-of-stream and dropped-stream finalization paths now share one decision: an upstream error fails the audit, a missing stop event fails it (distinguishing an empty upstream stream from one truncated mid-content), and a completion that carried content but reported zero usage logs a cost-capture-miss warning. Previously the end-of-stream path completed the audit regardless of whether the stream had terminated cleanly.
...
- A recorded child PID that the kernel has recycled to an unrelated process is no longer signalled. The server stores each spawned agent and MCP server's PID in the `services` table, and a row can outlive the process — a crash or hard kill never clears it — after which the kernel reuses the number for something else. Shutdown already confirmed a recorded PID still carried its spawn markers in `/proc/<pid>/environ` before signalling, but the startup and runtime reap paths — agent reconciliation, the agent `disable`/`restart`/unresponsive-cleanup operations, and the MCP orchestrator's reconcile, stale-binary, port-orphan, lifecycle-stop, and disabled-service cleanup — still trusted bare liveness and could SIGTERM/SIGKILL (or, via a process group, `kill(-pid)`) whatever process now holds the reused PID. The `/proc` identity check is now a single `systemprompt_models::subprocess::live_pid_is_subprocess` primitive applied at every site that signals a registry PID; a PID that no longer carries our markers is left untouched and its registry row cleared, and startup reconciliation now drops a `running` row whose PID is alive but foreign instead of adopting it. Port-discovered kills, which establish identity by port and process name, are unchanged.
