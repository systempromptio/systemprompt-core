...
- `scope: cluster | node` on scheduler jobs. Cluster jobs run once under the advisory lock as before; node jobs run on every replica, and the 900 ms tick dedupe is keyed by the instance that last ran them so replicas no longer starve each other. `content_prerender`, `page_prerender` and `copy_extension_assets` declare `node`, so every replica renders its own `web/dist`.
...
- User-keyed HTTP rate limits are counted in `user_rate_limit_buckets` and hold across replicas; address-keyed throttles stay per process and are documented as scaling with N.
...
- **Breaking:** `ServiceRepository::new`, `AgentServiceRepository::new` and `A2ARepositories::new` take an `InstanceId`; `PostgresEventBridge::new` and `EventRouter::install_relay` take the origin `InstanceId`; `FileUploadService::new` and `ImageStorage::new` take an `Arc<dyn FileStorage>`; `ProxyEngine::new` takes the proxy identity repository; `install_recorder` takes the instance id; `ThoughtSignatureCache::global()` is gone in favour of a per-context cache over the repository.
