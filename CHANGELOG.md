...
- **Runtime:** `AppContext` owns the process's `AiService`: `ai_service()` / `ai_service_arc()` return `Option<Arc<AiService>>`, assembled at boot from the services catalog (`None`, with a warning, when no default provider is usable). Every extension router receives it as `Extension<Option<Arc<AiService>>>` beside the governance engine.
