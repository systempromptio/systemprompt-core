...
- **Events:** opt-in transactional facts and durable consumer acknowledgement in the existing SSE outbox. Pending facts survive cleanup; existing routing and SSE payloads retain their contracts. Apply events migration 004 and upgrade all relays before enabling durable producers.
- **Analytics:** reporting queries use analytics-owned projections fed by source transactions through the existing outbox. Runtime initializes the baseline and processes pending facts; `analytics projection status`, `sync`, and `rebuild` expose maintenance operations. Rebuild blocks source writes while taking its snapshot. Session lifecycle and usage move to users, event ingestion to logging, and authoritative public-page counts to content. Rust callers must inject `SessionStore`, `AnalyticsEventStore`, and `ContentCatalogStats` into analytics repositories and supply `SessionProvider` to OAuth session creation; reporting is eventually consistent.
