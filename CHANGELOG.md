...
- **Bridge manifest:** `marketplaces` names each enabled marketplace with its display name and the plugin ids it still carries after the per-user filter, so a bridge client can mirror one host marketplace per gateway marketplace. A marketplace denied at its own level, or left with no surviving plugin, is not listed. The field is additive: an older manifest deserialises unchanged and a bridge reading an empty list falls back to the single `org-provisioned` marketplace it wrote before. The Claude Code CLI emitter uses it to write one marketplace directory per gateway marketplace with `<plugin>@<marketplace-id>` keys, tracks the marketplaces it owns in a sidecar so a user's own marketplaces are never pruned, and purges the legacy layout on the first marketplace-aware sync.
