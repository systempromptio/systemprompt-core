...
- The governance runtime moves into `systemprompt-security`, ending the copy of it each downstream carried. `policy::GovernanceEngine` instantiates the configured chain from the inventory registry (`register_governance_policy!`) and a `GovernanceConfig` parsed from the `governance.policies` YAML shape, and evaluates it first-deny-wins with a full per-entry audit trace. The four built-in policies — `secret_scan`, `scope_check`, `tool_blocklist`, `rate_limit` — ship in core and emit the typed `DenyReason` variants core already defined for them; extensions register additional policies and enable them from the same YAML. `policy::DecisionAudit` + `record_decision` persist the trace into `governance_decisions.evaluated_rules` with the flat columns derived, and `policy::secrets` exposes the shared credential scanner (`detect_secrets`, `scan_str_for_secret`). Enforcement points — webhook handlers, in-process seams, identity and scope resolution — stay extension-side; they build a `PolicyContext` and call the engine. Rate-limit state is engine-scoped rather than process-global: deployments holding one engine per process (all of them today) are unaffected, but two engines no longer share buckets.
