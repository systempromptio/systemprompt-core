...
pub(super) fn count_skills_across_plugins(root: &Path) -> Option<usize> {
    sum_across_plugins(root, |dir| count_dir_children(&dir.join("skills")))
}

pub(super) fn count_agents_across_plugins(root: &Path) -> Option<usize> {
    sum_across_plugins(root, |dir| count_md_files(&dir.join("agents")))
}

fn sum_across_plugins(root: &Path, per_plugin: impl Fn(&Path) -> Option<usize>) -> Option<usize> {
    let mut n = 0usize;
    for entry in std::fs::read_dir(root).ok()?.flatten() {
        let name = entry.file_name();
        let Some(name) = name.to_str() else { continue };
        ...
        if entry.file_type().ok().is_some_and(|t| t.is_dir()) {
            n += per_plugin(&entry.path()).unwrap_or(0);
        }
    }
    Some(n)
}
