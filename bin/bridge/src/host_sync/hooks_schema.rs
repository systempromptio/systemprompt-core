    ...
    pub(crate) fn append_comms_hooks(&mut self, command: &str) {
        for (event, is_async) in [("UserPromptSubmit", false), ("Stop", true)] {
            self.hooks
                ...
                .or_default()
                .push(HookMatcher::wildcard(HookEntry::user_command(
                    command.to_owned(),
                    event,
                    is_async,
                )));
...
const HOST_HEADER: &str = "x-systemprompt-host";
const DEVICE_CREDENTIAL_HEADER: &str = "x-systemprompt-device-credential";
