...
- macOS: settings window no longer opens with `with_maximized(true)`. Under winit 0.31.0-beta.2 the appkit backend routes an initial-maximized request through `WindowDelegate::set_fullscreen`, and a related unwrap on the delegate's fullscreen bookkeeping panics (via `abort()` — `panic = "abort"`) the first time the window resigns key. Symbolicated `bin/bridge/target/release-debug/astound-bridge` against the crash frames pointed at `winit-appkit-0.31.0-beta.2/src/window_delegate.rs:1495`/`:1578`. Dropping the maximized attribute keeps the same default window size (1100x760 min 800x600) and lets the user zoom manually.
