...
- Every sync derives `modelPicker` rows from the bridge profile's provider health for the non-Claude families and writes them into the settings the bridge owns: always the standalone fragment, and `~/.claude/settings.json` only where its keys are already merged. Claude Code's own gateway discovery keeps only ids containing `claude` or `anthropic`, so Gemini and Vertex models never reached its picker. The written ids are recorded in a metadata sidecar so removal takes back exactly those rows, and Claude ids stay with discovery, which carries their pricing.
...
- **macOS:** `install --apply` routes Claude Code through the gateway. It wrote the Claude Desktop managed preferences and reported Claude Code as governed, but the terminal CLI reads none of those preferences, so every `claude` model call went straight to Anthropic and never reached the audit trail; only Linux wrote the settings file. The settings writer is now unix-wide, the machine policy file is resolved through `claude_code_policy_dir()` rather than a hard-coded `/etc/claude-code`, and default-model seeding follows it to macOS. Claude Code hands `apiKeyHelper` to `/bin/sh` verbatim, so the macOS helper path was split on its spaces and failed with exit 127; the value is shell-quoted when it contains whitespace and left bare otherwise.
...
- `sync` no longer writes its own signing pin into the administrator channel. It stamped `manifestTrust` with source `policy` into the machine policy hive on every sync, so a pin no administrator had placed outranked the operator's own gateway URL and survived a full state reset: pointing the bridge at a second gateway failed with "pinned for <first gateway>" and the only remedy offered was an `install --apply --pubkey` the operator had no reason to run. A pin the bridge derives for itself is operator trust and is persisted to the config file; `install --apply --pubkey` still writes the policy key.
- Setup-screen defects seen on a first run: every `<details>` folded moments after being opened, because the reconciler stripped the browser-set `open` attribute on each snapshot tick; the gateway field arrived pre-filled with the brand default with nothing saying so, and the form now names the host it will sign in to and flags a default that was never chosen; and the "no longer accepts this bridge's credentials" toast outlived a purge and a fresh sign-in, and now hides as soon as the snapshot carries a verified identity.
...
- `allowedWorkspaceFolders` lists the home directory (`~`, not default-selected) after the brand workspace. The Claude Desktop Code tab enforces that list as the only permitted workspace roots, so the brand-only value refused every other folder with "Directory C:\Users\<user> is not within the allowed workspace roots configured by your administrator" while Cowork, which only reads the default-selected entry, kept working. `doctor` reports the roots in force as `policy workspace roots` and warns when home is absent.
...
- **Operator-visible:** the Claude Code CLI emitter mirrors one marketplace per marketplace the gateway manifest lists — `~/.claude/plugins/marketplaces/<marketplace-id>/`, cache under `plugins/cache/<marketplace-id>/`, and `<plugin>@<marketplace-id>` keys in `installed_plugins.json`, `known_marketplaces.json`, and the settings `enabledPlugins` / `extraKnownMarketplaces` maps — with `marketplace.json`'s `name` set to the marketplace id and its `description` to the marketplace name. A plugin two marketplaces carry is mirrored under each. Every plugin used to land in a single fixed `org-provisioned` marketplace, so `claude plugin list` could not say which marketplace granted a plugin and two marketplaces were indistinguishable on the client. The marketplaces the emitter owns are recorded in `~/.claude/plugins/.systemprompt-marketplaces.json`; a later sync prunes only those, so a marketplace the user registered by hand survives, and the legacy `org-provisioned` layout is purged one-way on the first sync against a marketplace-aware gateway. A manifest that lists no marketplaces — an older gateway — is still mirrored as `org-provisioned` holding every plugin, so behaviour there is unchanged. `doctor`'s marketplace check inspects every owned marketplace and reports the worst status with each detail kept, and the hook-URL check walks every owned marketplace's plugins.
...
- The bridge binary's non-terminal helpers are reachable from the separate test workspace through `#[doc(hidden)] pub mod test_api` blocks over `cli/proxy`, `cli/update`, `cli/login`, `install/managed_file`, `install/mdm/linux/settings` and `sync/apply`. Modules widen no further than `pub(crate)`, the seams delegate and add nothing, and no logic, signature or behaviour changes. The black-box suite behind them had been reporting eleven passes in 0.009s while executing nothing, because `SP_BRIDGE_BIN` was referenced and set nowhere.
...
- **Operator-visible:** OpenCode is offered every model the gateway can serve it. `accepted_surfaces()` declared `[ApiSurface::OpenAi]`, so `host_model_view` kept only providers whose *native* wire family was OpenAI: against a live instance the provider block offered nine models and hid all fourteen `claude-*` and all three `gemini-*` the same gateway was serving happily. The filter was answering the wrong question — `surface` governs advertisement, not dispatch; `resolve_upstream` keys entirely off `provider.wire`, and the gateway normalises any inbound wire to canonical before rendering any provider wire outbound, so a Gemini- or Anthropic-native provider is exactly as servable over the one OpenAI-compatible wire OpenCode speaks. Verified end to end, tool calls included. `Backend` stays excluded, listed by omission of an explicit surface rather than by passing an empty slice, because the empty-slice "no filter" convention would silently drop the one guarantee that matters. The sibling hosts have the same shape and are deliberately unchanged: `codex-cli` and `hermes` also declare `[OpenAi]` and `claude-desktop` declares `[Anthropic]`, and each changes what a different picker shows, so each wants its own verification.
...
- `install --host <id>` enrols a host app from the CLI — repeatable, comma-separated, or `--hosts all`, resolved through the host registry. `HostApp::install_profile` had been reachable only from the GUI's Re-apply button and from `reapply_stale_profiles`, which by design touches only hosts that already carry a profile. On Linux the only setup path is `install`, which installed no host profile at all: `sync` wrote OpenCode's MCP connectors while the provider block and the `auth.json` key were never written, so every request the client made to the loopback proxy came back `403 no loopback credential presented`. An unknown or suppressed id fails the whole request rather than being skipped; a sync-only agent such as `claude-code` resolves and reports there is nothing local to install; a host the last synced manifest does not enable is skipped with that reason. Enrolment shares `build_profile_inputs` with the re-apply path, so an enrolled profile and a repaired one come from the same live port, secret and models. The 403 body and `reapply_hint` now name `install --host`, which is the actual remedy when the client was never enrolled and has no profile to re-apply, and `proxy` prints the OpenCode provider snippet beside the Anthropic exports.
...
- `validate` no longer races the sync it describes. `on_login_finished` sent `ValidateRequested` before dispatching either first-run provisioning or a sync, on the same UI queue, so validate ran first and unconditionally, inspecting the machine policy and host files the sync was about to write — reporting the state before provisioning as if it were the state after. Both branches end in a sync and `on_sync_finished` re-validates on success and failure alike, so the eager call is removed rather than reordered. A validate landing mid-provisioning degrades to a warning instead of a red line, since nothing about it is actionable then.
- A trust-on-first-use pin that never landed no longer looks like a success. A failed config write was downgraded to a warn line and the fetched key returned anyway: the sync succeeded, nothing was pinned, and every later sync re-trusted whatever the gateway served — the pin protected nothing from the second run on, with a rolling-log warning as the only trace. `ConfigWriteError::Malformed` makes that permanent, the failure reaches the activity log with its underlying error, and `BridgeContext` carries the session fact so validate can tell "fetched but not written" from "never provisioned" — the two used to render the same unactionable line.
- The macOS managed-preferences store understood only `CFString` and returned `None` for every other property-list type. The two keys Cowork depends on most are arrays, so a correctly-provisioned Mac reported `allowedWorkspaceFolders: missing` and `managedMcpServers: not written` while `defaults read` showed both present — and the remedy each failure named could never work, because sync had already written them and the reader could not see them. Only the three string-valued keys beside them passed, which is the tell. Values serialise to JSON now, with a bare string kept bare so callers comparing against plain values still match.
- The Claude Code policy-dir check decided on filenames alone, so any file named `managed-settings.json` failed it — while `managed_mcp::clear_policy`, which decides on content, deliberately leaves an empty `{}` alone rather than prompting for elevation it does not need. The two disagreed and the check named a remedy that could never clear it: the failure was permanent and the advice was wrong. The check now asks the remover's own question — would sync act on this? — so `managed-mcp.json` offends by existing and `managed-settings.json` only when it carries a bridge key.
...
- The entropy backstop no longer denies macOS paths. It scored an absolute path as one token (`/` is a token character, not a delimiter), so a macOS `$TMPDIR` cleared every check — its random segment supplied the entropy, its `/T/` segment the uppercase. Claude Code carries those paths in its system prompt, so every request from an affected Mac was denied with "secret detected: High-entropy token" while Claude Desktop was unaffected. Path-shaped tokens are exempt, scored per segment so key material cannot hide inside one.
...
- The web tree is within the JavaScript standard: every file ≤150 lines and every function ≤30. The eleven oversize components were split by responsibility into sibling render modules and `utils/` helpers — `sp-agent-drawer` → `agent-drawer-{add,detail,health,parts}.js` + `utils/agent-drawer-actions.js`; `sp-profile` → `profile-{identity,usage,conversations}.js` + `utils/profile-format.js` (which also owns `sp-rail-profile`'s initials and subtitle, previously a second copy); `sp-rail-profile` → `utils/{rail-profile-menu,update-check}.js`; `sp-element` → `reconcile.js` (the keyed patcher) + `utils/escape.js` (`escapeHtml`/`attr`, imported directly by every user rather than re-exported); `sp-settings` → `settings-sections.js`, `settings-prefs-row.js`, `utils/settings-prefs.js`; `sp-marketplace`/`sp-marketplace-detail` → `marketplace-shell.js`, `marketplace-detail-{mcp,sections}.js`, `utils/marketplace-kinds.js` (one kind table — the two files carried different copies, and the detail's lacked `artifacts`, so an artifact's chip showed its raw id); `sp-setup` → `setup-sections.js`, `utils/setup-{steps,settle}.js` (`stepsFromSnapshot` is pure); `sp-setup-gateway` → `utils/gateway-signin.js`; `sp-activity-log` → `activity-{parts,actions}.js`, `utils/log-{format,stats}.js`; `sp-cloud-status` → `cloud-status-sections.js`, `utils/cloud-views.js`; `sp-hosts-list` → `utils/host-diff.js`.
