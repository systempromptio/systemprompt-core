...
- The 0.56.0 bridge, re-cut. `bridge-v0.56.0` was created as an empty published release before its assets uploaded — the repository now publishes immutable releases, which refuse every later upload — and an immutable release's tag name cannot be reused, so the same binaries ship as `bridge-v0.56.1`. No code change beyond the version.

## [0.56.0] - 2026-09-18

### Fixed

- A file the installed plugin tree already holds at the manifest's digest is copied into staging instead of downloaded (`sync/apply/fetch.rs`): an unchanged plugin re-syncs with only the files the host rewrites after install (`.claude-plugin/plugin.json`, the stripped `.mcp.json`) fetched again.
- **Every GUI request since 0.53.0 was refused.** `IpcRequest` gained a required `mount` field in 0.53.0 (the nonce that tells a reply for a reloaded page from one for the current page) and nothing on the page ever sent it: every `window.ipc.postMessage` was logged as `ipc: bad request: missing field \`mount\`` and never answered, so every control that waited on a reply — the wizard's Install and Finish, the Account panes, Repair, Sync — dimmed or sat on "loading…" for the life of the window; the reply script already passed the mount the page's `reply()` did not accept. The IPC bus is now one file, `web/js/ipc-bootstrap.js`, injected by the webview and loaded by `index.html`, so the preview and the node tests run the bytes the app runs; `invoke` sends `{id, mount, cmd, args}`; a request the bridge cannot parse is still answered (`invalid_format`) on the id it can recover; read-style commands carry a 30 s deadline. `web/dev/tests/ipc-envelope.test.mjs` holds the posted envelope to the committed `IpcRequest` binding, which `check-bridge-bindings` holds to the Rust struct.
- **The welcome wizard re-opened on a configured machine after an upgrade.** 0.53.0 (Claude Desktop) and 0.54.0 (Hermes, OpenCode) changed the credential a profile carries to a per-host token, so an upgraded machine's profiles probed `Stale` and, with no host `Installed`, the wizard came back on "Step 2 of 2" with every card offering to install what was already there. The wizard now honours the on-disk completion record (`agents_onboarded` / a finished first run, both already on the wire): a machine that has been through setup stays in the app and sees Repair in the Agents tab. At launch the GUI re-applies a profile it installed that now reads `Stale` — the repair `install --apply` and `login` already ran — without raising a prompt: a host whose install needs one (the machine policy on Windows, the profile approval on macOS) answers `PermissionDenied` to `HostApp::install_profile_unattended` and is logged as needing the user's Repair. `reapply_stale_profiles` takes an `Attendance`.
- Windows: the Claude Desktop profile that Repair, the wizard and the launch-time re-apply install carried only the six inference keys, so a machine policy that differed elsewhere (`managedMcpServers` after a connector change, the workspace roots, the hardening keys) was reported as "already holds this policy", nothing was written and the next sync failed with the same difference. The Windows profile now renders the whole `claude_desktop_policy`, as the macOS profile always did, so the one elevation Repair raises replaces the machine hive with the values the sync enforces. `reg_profile::profile_entries` / `render_reg` return `Result<_, MdmError>` (an organisation id that is not a UUID is refused, not dropped) and `policy::reg_values` is available on every target.
- The GUI refused to open a `http://localhost` URL, so on a self-hosted gateway (`http://localhost:8080`) the Connectors "Connect" button and every "open in browser" link did nothing but log "refusing to open non-https url". Plain `http://` is now allowed when the host is the machine itself (`localhost`, `127.0.0.1`, `[::1]`); `https://` anywhere and every other scheme are unchanged.
- A sync was requested every 30 s for the life of the window on a machine whose last sync left any warning under Claude Desktop. The Cowork tick keyed on "some warning for this host" and could not tell "open Cowork once" from an installation-evidence timeout, so the header sat on "syncing" and the gateway was polled continuously. `HostWarning` carries a `kind` (`cowork_session_missing`, `plugin_dependencies`, `evidence_unacknowledged`, `permission_rules`, `tool_catalog`, `node_packages`, `manifest`) and the tick re-requests a sync only for `cowork_session_missing`. **Breaking:** `HostWarnings::push` takes the kind first; the wire `HostWarning` gains `kind`.
- Installation-evidence capture no longer serialises the hosts under one 30 s wall each (three hosts ≈ 110 s per sync) nor reports a slow gateway as "feedback operation timed out: deadline has elapsed". The hosts are captured together, each with its own budget; a pass stops between items when the budget would be exceeded and reports "N of M installation receipts still pending; retried on the next sync"; a publication whose receipt the gateway already acknowledged is not re-planned on every sync. `recover_pending` takes a deadline and returns `RecoveryProgress`; `capture_host` returns `CaptureOutcome`.
- The MCP probe sent `tools/list` with `"params": null`; Google's Discovery Engine MCP answers that with `-32602 Invalid params` at HTTP 200, which the probe reported as "response did not contain a result.tools array" for a server that was in fact authenticated. `params` is now `{}`, and a JSON-RPC `error` on `tools/list` is reported by its code and message.
- The wizard panel scrolls (it never did; four cards pushed Finish below the window) and its cards no longer crush the description to one word per line; a card whose host has not been probed shows "Checking…" instead of an Install button.
- Disconnect, Reset and Remove application live in the tray only. The window's account menu offers Sign out; `device.action.open` is no longer an IPC command.
...
- macOS: the Claude Desktop Code tab refused to add any folder ("Your administrator has disabled adding folders") and so could not start a session. `allowedWorkspaceFolders` was rendered with `isDefaultSelected` as `<string>true</string>`; Claude Desktop types the field boolean, drops each malformed entry, and an empty resulting list blocks every folder. Every Mac since the key was first written on macOS (0.45.0) was affected; Cowork survived only because it falls back to its own directory prompt. Nested policy values are now native plist types (`<true/>`, `<integer>`), the top-level `disable*` keys stay string booleans. `validate` reports the entries Claude Desktop would drop and an empty resulting list as `policy workspace roots` failures, and `state`/the host profile print the real value of an array-of-objects key instead of an empty string.
...
- **Breaking:** `ProfileGenInputs::api_key: LoopbackSecret` is replaced by `host_token: HostToken`, derived per host by `reapply::build_profile_inputs`; no renderer sees the loopback secret. `feedback::sessions::native_session` and `NativeSessionLedger::observe` take the verified host (`Option<&HostId>`) the proxy resolved from the presented credential (`LoopbackCredential::verified_host`). `proxy::forward::hook` owns the hook-track helpers.
...
- The proxy stamps `x-systemprompt-client-attestation` on inference traffic (`host-token` with the canonical `x-systemprompt-client` for a verified host, `bridge-secret` for the raw secret, leaving any caller declaration in place) and always strips the inbound header.
