...
fn main() {
    ...
    // A white-label binary embeds its own branded Windows resource; linking a
    // second one from this crate produces duplicate .rsrc leaves. The brand
    // repo opts out here (see SYSTEMPROMPT_BRIDGE_WEB_OVERLAY for the pattern).
    println!("cargo:rerun-if-env-changed=SYSTEMPROMPT_BRIDGE_WINRES");
    let winres_off = std::env::var("SYSTEMPROMPT_BRIDGE_WINRES").as_deref() == Ok("off");
    if target_os == "windows" && !winres_off {
