...
function classify(snap) {
  ...
  if (cloudState === "probing" || cloudState === "unknown") {
    return { text: "checking…", cls: "badge-muted" };
  } else if (cloudState === "unreachable") {
    return { text: "cloud unreachable", cls: "badge-err" };
  } else {
...
function classifyHosts(snap) {
  ...
  if (hosts.length === 0) {
    return { text: "no host apps", cls: "badge-muted" };
  }
  ...
  if (anyAbsent) {
    ...
    return { text: "profile partial", cls: "badge-warn" };
