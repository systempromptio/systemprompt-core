...
const createMeta = (host) => {
  ...
  meta.className = "sp-setup-agent__meta";
  const name = document.createElement("div");
  name.className = "sp-setup-agent__name";
  const suffix = host.kind === "cli_tool" ? " · CLI" : " · Desktop";
  name.textContent = host.display_name + suffix;
  const desc = document.createElement("div");
  desc.className = "sp-setup-agent__desc";
...
const createButton = (host, installed) => {
  ...
  btn.type = "button";
  btn.className = installed ? "sp-btn-ghost" : "sp-btn-primary";
