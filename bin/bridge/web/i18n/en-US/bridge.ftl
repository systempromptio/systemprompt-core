...
sync-partial = Sync finished, but { $count } agent(s) did not update: { $hosts }. Open Status for the reason.
sync-elevation-required = Synced everywhere except { $hosts } — its connector list is written by an administrator. Press Update as administrator to finish.
login-config-foreign-owner = The configuration folder { $path } was created by another Windows account ({ $owner }). Repair it as administrator, or delete the folder and sign in again.
config-dir-repair-started = Repairing the configuration folder — approve the Windows prompt.
config-dir-repaired = Configuration folder { $path } now belongs to this account.
config-dir-repair-failed = Could not repair the configuration folder: { $error }
toast-action-details = Details
toast-action-repair-admin = Repair as administrator
toast-action-update-admin = Update as administrator
toast-config-dir-repaired = Configuration folder repaired. Sign in again.
toast-agent-repaired-short = { $name } repaired.
...
disconnect-running = Disconnecting this computer…
disconnect-success = This computer is disconnected. Your sign-in and settings were kept.
disconnect-failure = Disconnect failed: { $error }
removal-incomplete = Cleanup finished with items that could not be removed: { $error }
...
marketplace-empty-rules = No rules yet
...
marketplace-refresh-error = Could not refresh. Showing the previous list.
marketplace-signin-required = Sign in to view your marketplace.
marketplace-verifying = Checking your connection…
marketplace-unreachable = Waiting for the gateway connection.
...
agent-update-explainer = Update rewrites this agent's connector list from the gateway and re-applies it. Quit and relaunch the agent afterwards.
...
marketplace-badge-degraded = synced with failures
marketplace-badge-needs-approval = Claude Desktop needs approval
...
tray-restart-update = Restart to finish updating
tray-disconnect = Disconnect this computer…
tray-purge = Reset Bridge completely…
tray-remove-application = Remove Bridge application…
...
device-disconnect-title = Disconnect this computer?
device-disconnect-body = Removes managed integrations and scheduled synchronization. Your sign-in and settings are kept so you can reconnect later.
device-disconnect-confirm = Disconnect computer
device-purge-title = Reset Bridge completely?
device-purge-body = Removes integrations, credentials, identity, configuration, managed files and saved state. The app returns to fresh setup.
device-purge-confirm = Reset everything
device-remove-title = Remove Bridge application?
device-remove-body = First removes all Bridge integrations and local data, then shows the installed application so you can remove it safely.
device-remove-confirm = Clean up and continue
device-remove-guide-title = Finish removing Bridge
device-remove-guide-scoop = Quit Bridge, then run “scoop uninstall bridge” in PowerShell.
device-remove-guide-macos = Show the application in Finder, quit Bridge, then move it to Trash.
device-remove-guide-standalone = Show the application, quit Bridge, then delete it using the same method you used to install it.
device-remove-reveal = Show application
device-remove-quit = Quit Bridge
device-action-working = Working…
device-action-cancel = Cancel
device-action-done = Done
tray-quit = Quit
...
overall-degraded = synced with failures
overall-needs-approval = Claude Desktop needs approval
overall-ready = ready
