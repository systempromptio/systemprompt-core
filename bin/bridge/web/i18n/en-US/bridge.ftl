...
requests-backfill-failed = Could not read the request history from the proxy.
requests-caption = Every request the proxy forwarded, and what your gateway decided about it.
requests-empty = No requests yet — start a governed agent and they appear here.
requests-filter-all = All
requests-filter-denied = Denied only
requests-heading = Governed requests
requests-search-placeholder = Search requests…
requests-verdict-allow = allowed
requests-verdict-deny = denied
requests-verdict-unknown = —
