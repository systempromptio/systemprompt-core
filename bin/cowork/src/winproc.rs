...
pub(crate) fn is_elevated() -> bool {
    ...
    // SAFETY: GetCurrentProcess returns a pseudo-handle that requires no close. OpenProcessToken
    // writes a real handle into `token` only on success; we close it via CloseHandle below.
    // GetTokenInformation only reads `elevation.assume_init()` after the call returned ok != 0.
    unsafe {
...
pub(crate) fn attach_parent_console_if_present() {
    ...
    // SAFETY: AttachConsole is safe to call from any thread; failure (no parent console) is
...
pub(crate) fn detach_console() {
    ...
    // SAFETY: FreeConsole detaches the calling process; safe regardless of attached state.
    unsafe {
        FreeConsole();
    }
}
