...
/**
 * Vite plugin that injects metadata from $SYSTEMPROMPT_WEB_METADATA_PATH
...
export function metadataInjector(): Plugin {
  ...
  return {
    ...
    configResolved() {
      const metadataPath = process.env.SYSTEMPROMPT_WEB_METADATA_PATH;
      if (!metadataPath) {
        throw new Error('SYSTEMPROMPT_WEB_METADATA_PATH environment variable must be set');
      }
      try {
