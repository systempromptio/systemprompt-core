...
export function useTokenExpiryMonitor() {
  ...
  useEffect(() => {
    ...
    const checkTokenExpiry = async () => {
      ...
      } else if (timeUntilExpiry <= TIMING.TOKEN_REFRESH_THRESHOLD_MS && userType !== 'anon' && refreshToken) {
    ...
    intervalRef.current = setInterval(checkTokenExpiry, TIMING.TOKEN_CHECK_INTERVAL_MS)
