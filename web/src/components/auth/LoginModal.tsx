...
export function LoginModal({ isOpen, onClose, onSuccess, onSwitchToRegister, agentName }: LoginModalProps) {
  ...
  const { setAuth, isWebAuthnSupported: checkWebAuthnSupported, authenticateWithPasskey } = useAuthStore()

  useEffect(() => {
    setIsWebAuthnSupported(checkWebAuthnSupported())
  ...
  const handleAuthenticate = async () => {
    ...
    if (result.success && result.accessToken) {
      try {
        ...
        setAuth(userEmail, userId, result.accessToken, result.refreshToken || undefined, result.expiresIn || 3600)
