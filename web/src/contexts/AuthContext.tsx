...
export function AuthProvider({
  ...
  const parseToken = useCallback((token: string): User | undefined => {
    try {
  ...
  const logout = useCallback(async () => {
    try {
      ...
      /* Logout should proceed regardless of API failure */
    } finally {
      setToken(undefined)
      setState({
        user: undefined,
  ...
  const refreshToken = useCallback(async () => {
    ...
    try {
      ...
      const message = caughtError instanceof Error ? caughtError.message : 'Token refresh failed'
      setError(message)
      await logout()
    }
  ...
  const updateUser = useCallback((updates: Partial<User>) => {
    setState((prev) => ({
      ...prev,
      user: prev.user ? { ...prev.user, ...updates } : undefined,
    }))
  }, [])
