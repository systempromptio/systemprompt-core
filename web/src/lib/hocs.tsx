  ...
  return class WithPerformance extends Component<P> {
    ...
    componentDidMount() {
      ...
      if (options.logThreshold && renderTime > options.logThreshold) {
        logger.warn(`${componentName} took ${renderTime.toFixed(2)}ms to mount`, undefined, 'PerformanceMonitor')
      }
    }
    ...
    componentWillUnmount() {
      if (this.mountTime !== null) {
        ...
        if (options.logThreshold && lifeTime < options.logThreshold) {
          ...
          logger.warn(`${componentName} was unmounted quickly (${lifeTime.toFixed(2)}ms)`, undefined, 'PerformanceMonitor')
        }
      }
    }
