...
// Core artifact types - extensions can define additional types (validated as strings)
...
export function validateArtifactMetadata(value: unknown, artifactId: string): ArtifactMetadata {
  ...
  if (!isKnownType) {
    logger.warn(
      `Unknown artifact_type "${value.artifact_type}" for ${artifactId}`,
      { knownTypes: VALID_ARTIFACT_TYPES },
      'MetadataValidator'
    )
  }
...
export function extractTaskMetadataSafe(
  ...
  try {
    ...
    logger.warn(`Using default metadata for ${entityId}`, e, 'MetadataValidator')
    return {
