...
so it runs **sharded** under `cargo-nextest` — the same 10-shard split CI uses.
...
# Groups: shared infra domain app-runtime app-rest entry-api entry-cli bridge integration edge
just test-shard shared

# Run all 10 shards sequentially (each against its own fresh DB)
