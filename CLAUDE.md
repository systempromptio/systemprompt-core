...
- **Repository pattern**: services never run SQL directly; all queries via compile-time macros (`sqlx::query!` family). Runtime `sqlx::query(_)` only where the SQL text is necessarily dynamic: `infra/database/src/admin/**`, `infra/database/src/services/postgres/**`, `infra/database/src/repository/entity.rs`, the table-driven analytics projector (`domain/analytics/src/projection/{mod,snapshot}.rs`), `entry/cli/src/commands/admin/setup/**` (bootstrap DDL) and `entry/cli/src/commands/infrastructure/jobs/cleanup_logs.rs`. The list is `scripts/check-sqlx.sh`'s allowlist; a turbofish (`query_scalar::<_, T>(`) is a runtime query too.
...
- **No env-var readers outside the allowlist**: `std::env::var`/`var_os`/`vars` in a production crate is only for the sanctioned boot/subprocess variables, `${VAR}` interpolation, the secrets `env` source, build-script env, and OS directory/identity probes — every such file is listed with a reason in `scripts/env-var-allowlist.txt`. A runtime kill switch or feature flag is a profile field. Gate: `just lint-env-vars`.
- **Fail-closed policy expansion**: a policy projected over an inventory (tool names, plugin ids, hosts) that may be empty or stale withholds the subject (`Option`/`Result`) — never a partial map a client resolves to a more permissive default. `just lint-fail-open` flags a `known_*`/`*_catalog`/`inventory` walk in a guard fn (`partial-projection`) as a tripwire; the review is the gate. See `internal/guides/rust.md` §6.
- **Native-matrix test crates are DB-free**: a crate in `scripts/bridge-native-crates.txt` must not depend, even transitively within `crates/tests`, on `systemprompt-test-fixtures` or a `sqlx::query*!` user. Gates: `just lint-native-test-deps`, `just lint-bridge-native-tests`.
- **Workspace `clippy`/`check` always `--keep-going`**: the first broken crate must not hide the findings behind it. `just lint`, `just check` and the bridge recipes carry it; read the whole list before fixing.
...
The CLI records where the profile came from (`ProfileSource`: `--profile`, `SYSTEMPROMPT_PROFILE`, the stored session, or directory discovery). An explicit `--profile` is a one-shot target: it never rewrites the active session (`admin session switch|login` do). `infra db migrate`, the destructive `infra db` subcommands and `infra jobs run` refuse a cloud profile that arrived implicitly — pass `--profile <name>` to target it.
