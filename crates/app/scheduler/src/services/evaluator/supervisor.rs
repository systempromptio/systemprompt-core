...
use systemprompt_evaluation::repository::experiments::{
    AssignmentRepository, DeterministicMeasurement, EvaluationLifecycleRepository,
    EvaluationTrafficClass, EvidenceRepository, ExecutionCapabilityRepository, ExecutionCompletion,
    ExecutionEvent, ExecutionEventRepository, ExecutionLease, ExecutionStage, ExperimentRepository,
    GatewayEvaluationRepository, GeneratedSuggestion, TerminalOutcome, WorkerRepository,
};
...
impl EvaluatorSupervisor {
    ...
    pub async fn run_once(
        ...
        let variant = assignment
            ...
            .variants
            .get(
                usize::try_from(record.variant_index)
                ...
                if let Err(error) = self.experiments.heartbeat(owner, &lease).await {
                    ...
                    self.lifecycle
                        .record_cleanup(
                            ...
                            confirmed,
                            (!confirmed)
                                .then_some("Cancellation cleanup was not fully acknowledged"),
                        )
                        .await
                        .map_err(internal)?;
        ...
        let judgment = if status.success() {
            ...
            self.gateway
                .set_traffic_class(owner, &lease, EvaluationTrafficClass::Judge)
                .await
                .map_err(internal)?;
            let judge_name = format!("eval-judge-{suffix}");
            let judge_launch =
                ContainerLaunch::builder(self.config.docker.clone(), directory.clone())
                    .image(self.config.client_image.clone())
                    .network(network.name().to_owned())
            ...
            artifacts.insert(
                ...
                ArtifactFile {
                    bytes: std::fs::read(judge_stderr)?,
                    executable: false,
                },
            );
            self.append_event(
        ...
        let retained_requests = self
            .evidence
            .list_request_ids(owner, &record.id)
            .await
            .map_err(internal)?;
        let preliminary_references = artifacts
            .keys()
            .cloned()
            .chain(
                retained_requests
            ...
            .collect::<BTreeSet<_>>();
        let preliminary_archive = EvidenceArchive {
            files: artifacts.clone(),
        };
        let deterministic = verification::evaluate(VerificationInput {
            case: &case,
            evidence: &preliminary_archive,
        });
        let semantic_passed = judgment
            .as_ref()
            .and_then(|value| scoring::score(&rubric, value, &preliminary_references).ok())
            .is_some_and(|outcome| outcome.passed);
        let should_suggest = status.success()
            && (!deterministic.hard_failures.is_empty()
            ...
            && assignment
                ...
                .frozen
                .as_ref()
                .is_some_and(|frozen| frozen.cost_envelope.suggestion_calls > 0)
            && self
                .lifecycle
        ...
        if should_suggest {
            self.gateway
                .set_traffic_class(owner, &lease, EvaluationTrafficClass::Suggestion)
                .await
                .map_err(internal)?;
            let suggestion_name = format!("eval-suggestion-{suffix}");
            let suggestion_launch =
                ContainerLaunch::builder(self.config.docker.clone(), directory.clone())
                    .image(self.config.client_image.clone())
                    .network(network.name().to_owned())
                    .name(suggestion_name.clone())
            ...
            let suggestion_status = loop {
                if let Some(status) = suggestion_run.poll()? {
                    break status;
                }
                if last_heartbeat.elapsed() >= Duration::from_secs(20) {
                    if let Err(error) = self.experiments.heartbeat(owner, &lease).await {
            ...
            artifacts.insert(
                ...
                ArtifactFile {
                    bytes: std::fs::read(suggestion_stderr)?,
                    executable: false,
                },
            );
            if suggestion_status.success() {
                if let Ok(generated) = parse_suggestion(&suggestion_bytes) {
                    ...
                    if let Some(request) = after
                        .iter()
                        .find(|request| !retained_requests.contains(request))
                    {
                        self.lifecycle
                            .record_generated_suggestion(owner, &record.id, request, &generated)
                            .await
                            .map_err(internal)?;
                    }
                }
        ...
        let evidence = ExecutionEvidence::builder()
            ...
            .installed_bundle_digest(assignment.skill_bundle.digest.clone())
            .candidate_bundle_digest(variant.skill_bundle_digest.clone())
            .workspace_digest(assignment.configuration.digest.clone())
            .requests(requests)
        ...
        self.evidence
            .submit(
                ...
                &lease,
                &evidence,
                &EvidenceArchive { files: artifacts },
            )
            .await
            .map_err(internal)?;
        let outcome = if status.success() && cleanup_confirmed {
        ...
        if outcome == TerminalOutcome::Completed {
            ...
            let scored = judgment.as_ref().and_then(|value| {
                scoring::score(&rubric, value, &references)
                    .ok()
                    .map(|outcome| (value.clone(), outcome))
            });
            let accounting = self
                .lifecycle
                .execution_accounting(owner, &record.id)
                .await
                .map_err(internal)?;
            let deterministic_passed = deterministic.hard_failures.is_empty()
                && deterministic.checks.values().all(|value| *value);
            let measurement = DeterministicMeasurement {
                ...
                verified_success: deterministic_passed
                    && scored.as_ref().is_some_and(|(_, outcome)| outcome.passed),
            };
            self.lifecycle
                .record_measurement(owner, &lease, &measurement)
                .await
                .map_err(internal)?;
        }
        Ok(true)
    ...
    async fn append_event(
        &self,
        worker: &systemprompt_evaluation::repository::experiments::WorkerRecord,
...
fn safe_suffix(execution: &EvalExecutionId) -> String {
    execution
        .as_str()
        .bytes()
...
fn image_digest(image: &str) -> SchedulerResult<String> {
    image
        .rsplit_once("@sha256:")
        .map(|(_, digest)| digest.to_owned())
        .ok_or_else(|| SchedulerError::config_error("Pinned image digest missing"))
}
fn internal(error: impl std::fmt::Display) -> SchedulerError {
