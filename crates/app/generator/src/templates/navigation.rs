...
fn build_social_link(platform_type: &str, href: &str) -> Result<String> {
    let icon = get_social_icon(platform_type);
    if icon.is_empty() {
        return Ok(String::new());
    }

    let first_char = platform_type
        .chars()
        .next()
        .ok_or_else(|| ContentError::missing_field("platform_type"))?;

    let platform_label = first_char.to_uppercase().collect::<String>() + &platform_type[1..];
...
pub fn generate_social_action_bar_html(
    ...
    let label = action_bar_config
        ...
        .and_then(|l| l.as_str())
        .ok_or_else(|| ContentError::missing_branding_config("social_action_bar.label"))?;
    ...
    let enable_share = action_bar_config
        ...
        .ok_or_else(|| ContentError::missing_branding_config("social_action_bar.enable_share"))?;
    ...
    if let (Some(platforms), Some(social_links)) = (platforms, social_links) {
        for platform in platforms {
            let platform_type = platform
                .get("type")
                .and_then(|t| t.as_str())
                .ok_or_else(|| ContentError::missing_field("platform.type"))?;
                ...
                .find(|link| link.get("type").and_then(|t| t.as_str()) == Some(platform_type))
            {
                let href = social_link
                    .get("href")
                    .and_then(|h| h.as_str())
                    .ok_or_else(|| ContentError::missing_field("social_link.href"))?;
                let rendered = build_social_link(platform_type, href)?;
                if !rendered.is_empty() {
                    links_html.push(rendered);
                }
            }
        }
    }
...
fn build_footer_section(section_name: &str, links: &serde_yaml::Value) -> Result<Option<String>> {
    let first_char = section_name
        .chars()
        .next()
        .ok_or_else(|| ContentError::missing_field("section_name"))?;

    let section_title = first_char.to_uppercase().collect::<String>() + &section_name[1..];

    let Some(links_seq) = links.as_sequence() else {
        return Ok(None);
    };
...
pub fn generate_footer_html(web_config: &serde_yaml::Value) -> Result<String> {
        ...
        if let Some(mapping) = footer_config.as_mapping() {
            for (section_name, links) in mapping {
                let section_name_str = section_name
                    .as_str()
                    .ok_or_else(|| ContentError::missing_field("footer.section_name"))?;
