...
pub fn generate_social_action_bar_html(web_config: &FullWebConfig, is_hero: bool) -> String {
    let action_bar = &web_config.social_action_bar;
    let social_links = &web_config.navigation.social;
    ...
    for platform in &action_bar.platforms {
        if let Some(social_link) = social_links
            .iter()
            .find(|link| link.link_type == platform.platform_type)
        {
            let rendered = build_social_link(&platform.platform_type, &social_link.href);
...
fn build_footer_section(
    section_name: &str,
    links: &[systemprompt_models::NavLink],
) -> Option<String> {
    if links.is_empty() {
        return None;
    }
    ...
    let link_items: Vec<String> = links
        .iter()
        .map(|link| format!(r#"<li><a href="{}">{}</a></li>"#, link.path, link.label))
        .collect();
...
fn build_footer_social_html(social_links: &[systemprompt_models::SocialLink]) -> String {
    let social_items: Vec<String> = social_links
        .iter()
        .filter_map(|link| {
            let icon = match link.link_type.as_str() {
...
pub fn generate_footer_html(web_config: &FullWebConfig) -> Result<String> {
    let footer = &web_config.navigation.footer;
    let social = &web_config.navigation.social;

    let mut sections_html = Vec::new();

    if let Some(section_html) = build_footer_section("legal", &footer.legal) {
        sections_html.push(section_html);
    }
    if let Some(section_html) = build_footer_section("resources", &footer.resources) {
        sections_html.push(section_html);
    }

    let social_html = build_footer_social_html(social);
    ...
    let copyright = &web_config.branding.copyright;
