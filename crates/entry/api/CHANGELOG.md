...
- `routes::admin::services::ServicesRefresh::run(actor, restart)`: the in-process handle to the fetch-verify-compose-reconcile pipeline behind `POST /admin/services/refresh`, handed to every extension router as `Extension<ServicesRefresh>`; one process-wide `RefreshLock` guards the route and every handle.
- `services::gateway::artifact_scanner::ArtifactScanner` runs the gateway safety scanners over an ingested tool result; `audit::tool_results` replays gateway `tool_result` blocks through `ArtifactIngest`. The proxy tap (`services::proxy::audit::tap`) ingests every proxied tool result and stamps the execution id into the response `_meta`; **Breaking:** `McpAudit::new` takes `Option<Arc<ArtifactIngest>>` as its second argument.
- The `/api/v1/mcp/registry` `port` field is `Option<u16>` (`null` for an `external` server).
- Every failed `/v1/bridge/latest` and `/v1/bridge/download/{platform}` resolution logs its status and reason.

### Changed

- The server builder takes the gateway-policy repository from `AppContext::ai_repositories().gateway_policies` instead of constructing one.
...
- The reasoning-token invariant is enforced rather than assumed. `reasoning_tokens` is a breakdown of `output_tokens`, never an addition, but the OpenAI-compatible third parties (Cerebras, Moonshot, Qwen) were only assumed to follow it because they copy the field name, and one reporting additively under-bills silently. `CanonicalUsage::normalise_reasoning` folds the count into `output_tokens` and warns with the provider named when a breakdown exceeds its parent or a wire total overshoots by exactly the reasoning count.
- Model limits resolve by the requested catalog id, not the upstream name. `find_model` matches ids and aliases, so passing the upstream name returned `None` for every provider whose ids differ from what it sends upstream — both Vertex entries lost their output cap and thinking budget, which is why the MaaS thinking models still returned empty text at `max_tokens: 128`.
- The upstream model name comes from the catalog entry the route resolved to. `ProviderModel::upstream_model` was parsed, validated and read by nobody: all three dispatch sites resolved from the route alone, which is invisible only while the two names agree. One `qwen.*` route cannot carry a different upstream name for each of the nine models under it. `effective_upstream_model_for` falls back through a route-level override, then the catalog's per-model name, then the requested id; the governance check was comparing requirements against a name that could differ from the one actually sent.
- A truncated buffered Responses turn is marked incomplete. The renderer hardcoded `"status": "completed"` and emitted no `incomplete_details`, so a turn cut at `max_output_tokens` reached the client as a finished one; it now mirrors the streaming terminal exactly.
- The Responses body no longer emits a `stop_reason` key the contract does not define. Both lanes express the verdict as `status` plus `incomplete_details`, and an item cut mid-arguments is `status: "incomplete"`.
- A provider whose `api_key_secret` does not resolve is demoted to `surface: backend`, so its models leave `/v1/models` and every client picker instead of 502-ing when someone picks one. An uninitialised secret store means "unknown", never "absent" — several entry points load services with no secrets at all, and demoting there would empty the catalog.
- The MCP proxy resolver's restart retry is bounded. It recursed on `attempt_restart` returning `Ok`, but starting nothing — an unregistered name filters to an empty target list — also reports `Ok`, so a crashed row that never recovered recursed until the stack was exhausted, killing the API process on a single proxied request. It re-reads the row and returns it only when the status is actually running.
- `validate_self_issued` takes the JWT header its only caller already decoded, instead of re-decoding it into an error arm that could not execute. `resolve_context_id`'s empty-context branch and the session-lookup fallback behind it are deleted: `ContextId` is UUID-validated on every construction path. `monitor_all_agents`' `process_exists` re-check on a `Running` row is deleted: `list_all_agents` already downgrades a dead pid to `Failed`.

### Changed

- **Breaking:** `CanonicalUsage` is the only usage type. `CapturedUsage` and `CostTokens` are deleted along with the gateway's `CapturedUsage → CanonicalUsage → CapturedUsage` round trip, `ModelPricing::cost_microdollars(&CanonicalUsage)` prices all four billable classes on both paths, and `CanonicalUsage::billable_total` is the single definition of `tokens_used`.
- `upstream_model_for` moves from `GatewayRoute` to `ProviderEntry`: the mapping is provider knowledge, not routing knowledge.
- The stream tap's abort and tail rendering move to `stream_tap/abort.rs`; both answer what the caller is told when a tapped stream ends. No behaviour change.
