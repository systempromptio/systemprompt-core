...
- `routes::admin::services::ServicesRefresh::run(actor)`: the in-process handle to the fetch-verify-compose-reconcile pipeline behind `POST /admin/services/refresh`, handed to every extension router as `Extension<ServicesRefresh>`; one process-wide `RefreshLock` guards the route and every handle.
