...
impl GatewayAudit {
    ...
    /// Settles a failure with whatever the provider streamed before it broke.
    /// `usage` is `None` when nothing was observed; otherwise it is priced the
    /// same way a completion is, so a truncated stream is not free.
