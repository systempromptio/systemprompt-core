...
pub(super) fn key(headers: &HeaderMap) -> Result<TaskId, ManagedHttpError> {
    let key = headers
        .get("idempotency-key")
...
pub(super) async fn begin<T: Serialize + Sync>(
    ...
    kind: &str,
    input: &T,
) -> Result<ApiOperationClaim, ManagedHttpError> {
    Ok(ctx
        .managed_repository()
...
) -> Result<OperationResponse<T>, ManagedHttpError> {
...
pub(super) async fn finish<T: Serialize + serde::de::DeserializeOwned + Sync>(
    ctx: &AppContext,
    operation: &ApiOperation,
    result: Result<T, ManagedHttpError>,
) -> Result<OperationResponse<T>, ManagedHttpError> {
    match result {
        Ok(result) => {
...
async fn status(
    ...
    Path(id): Path<TaskId>,
) -> Result<Json<OperationResponse<OperationResult>>, ManagedHttpError> {
    let operation = ctx
        .managed_repository()
