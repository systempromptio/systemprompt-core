...
) -> Result<Json<InventoryEntry>, ManagedHttpError> {
    Ok(Json(
        ctx.managed_repository()
...
) -> Result<Json<super::collections::Page<BaselineCapture>>, ManagedHttpError> {
    let items =
...
async fn publish_latest(
    State(ctx): State<AppContext>,
    Extension(actor): Extension<RequestContext>,
) -> Result<Json<Vec<LatestPublication>>, ManagedHttpError> {
    Ok(Json(
...
) -> Result<Json<ObservedMembership>, ManagedHttpError> {
    Ok(Json(
        ctx.managed_repository()
...
) -> Result<Json<BaselineCapture>, ManagedHttpError> {
    Ok(Json(
        ctx.managed_repository()
