...
) -> Result<Json<Option<FeedbackSnapshot>>, OptimizationHttpError> {
    Ok(Json(
...
async fn health(
    State(ctx): State<AppContext>,
) -> Result<Json<SnapshotHealth>, OptimizationHttpError> {
    Ok(Json(
        ctx.feedback_snapshots_repository()
...
) -> Result<(StatusCode, Json<SnapshotRangeJob>), OptimizationHttpError> {
    Ok((
        StatusCode::ACCEPTED,
        Json(
            ctx.feedback_snapshots_repository()
