...
use super::types::{
    ColumnInfo, DbCountOutput, DbDescribeOutput, DbIndexesOutput, DbInfoOutput, DbSizeOutput,
    DbTablesOutput, DbValidateOutput, IndexInfo, TableInfo, TableSizeInfo,
};
...
pub async fn execute_indexes(
    ...
    for table in &filtered_tables {
        if let Ok(indexes) = admin.get_table_indexes(&table.name).await {
            for idx in indexes {
                all_indexes.push(super::types::TableIndexInfo {
