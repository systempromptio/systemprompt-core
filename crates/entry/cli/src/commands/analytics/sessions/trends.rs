//! `analytics sessions trends` command with chart output.
//!
...
fn build_chart(output: &SessionTrendsOutput) -> ChartArtifact {
    let labels: Vec<String> = output.points.iter().map(|p| p.timestamp.clone()).collect();
    let sessions: Vec<f64> = output
        .points
        .iter()
        .map(|p| p.session_count as f64)
        .collect();
    let users: Vec<f64> = output
        .points
        .iter()
        .map(|p| p.active_users as f64)
        .collect();
    ChartArtifact::new("Session Trends", ChartType::Line)
        .with_labels(labels)
        .with_datasets(vec![
            ChartDataset::new("sessions", sessions),
            ChartDataset::new("active_users", users),
        ])
}
