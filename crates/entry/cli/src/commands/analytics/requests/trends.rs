//! `analytics requests trends` command with chart output.
//!
...
fn build_chart(output: &RequestTrendsOutput) -> ChartArtifact {
    ...
    let requests: Vec<f64> = output
        ...
        .collect();
    let tokens: Vec<f64> = output
        .points
        .iter()
        .map(|p| p.total_tokens as f64)
        .collect();
    ChartArtifact::new("AI Request Trends", ChartType::Line)
        .with_labels(labels)
        .with_datasets(vec![
            ChartDataset::new("requests", requests),
            ChartDataset::new("tokens", tokens),
        ])
}
