...
pub async fn execute(args: TrendsArgs, config: &CliConfig) -> Result<()> {
    ...
    if config.is_json_output() {
        let result =
            CommandResult::chart(output, ChartType::Line).with_title("Agent Usage Trends");
...
async fn fetch_trends(
    ...
    for row in rows {
        let period_key = format_period_label(truncate_to_period(row.started_at, group_by), group_by);
