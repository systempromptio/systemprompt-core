...
pub(super) fn is_docker_available() -> bool {
    Command::new("docker").arg("--version").output().is_ok()
}

pub(super) fn is_compose_available() -> bool {
    Command::new("docker")
...
pub(super) fn create_compose_files_if_missing(
    compose_dir: &Path,
    container_name: &str,
    port: u16,
) -> Result<()> {
...
pub(super) fn wait_for_postgres_ready(config: &PostgresConfig, container_name: &str) {
