...
pub(crate) fn apply_multiplier(base: u64, multiplier: f64) -> u64 {
    (base as f64 * multiplier).round() as u64
}

pub(crate) fn get_tier_multiplier(tiers: &TierMultipliers, tier: &str) -> Result<f64> {
    match tier {
...
pub(crate) fn get_endpoint_rate(limits: &RateLimitsConfig, endpoint: &str) -> Result<u64> {
    match endpoint {
...
pub(crate) fn load_profile_for_edit(path: &str) -> Result<Profile> {
    let content =
...
pub(crate) fn collect_endpoint_changes(
...
pub(crate) fn collect_tier_changes(
