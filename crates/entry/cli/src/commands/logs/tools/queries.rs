...
pub async fn query_tools(
    ...
    let rows = match (since, name, server, status) {
        ...
        (Some(s), Some(n), Some(sv), None) => {
            query_since_name_server(pool, s, n, sv, limit).await?
        },
        (Some(s), Some(n), None, Some(st)) => {
            query_since_name_status(pool, s, n, st, limit).await?
        },
        (Some(s), None, Some(sv), Some(st)) => {
            query_since_server_status(pool, s, sv, st, limit).await?
        },
...
async fn query_since_name_server(
    pool: &Arc<PgPool>,
    since: DateTime<Utc>,
    name: &str,
    server: &str,
    ...
    Ok(sqlx::query_as!(
        ...
        r#"SELECT started_at as "timestamp!", trace_id as "trace_id!", tool_name as "tool_name!",
           server_name, status as "status!", execution_time_ms
           FROM mcp_tool_executions
           WHERE started_at >= $1 AND tool_name ILIKE $2 AND server_name ILIKE $3
...
async fn query_since_name_status(
    pool: &Arc<PgPool>,
    since: DateTime<Utc>,
    name: &str,
    status: &str,
    ...
    Ok(sqlx::query_as!(
        ...
        r#"SELECT started_at as "timestamp!", trace_id as "trace_id!", tool_name as "tool_name!",
           server_name, status as "status!", execution_time_ms
           FROM mcp_tool_executions
           WHERE started_at >= $1 AND tool_name ILIKE $2 AND status = $3
           ORDER BY started_at DESC LIMIT $4"#,
        since,
        name,
        status,
...
async fn query_since_server_status(
    pool: &Arc<PgPool>,
    since: DateTime<Utc>,
    server: &str,
    status: &str,
    ...
    Ok(sqlx::query_as!(
        ...
        r#"SELECT started_at as "timestamp!", trace_id as "trace_id!", tool_name as "tool_name!",
           server_name, status as "status!", execution_time_ms
           FROM mcp_tool_executions
           WHERE started_at >= $1 AND server_name ILIKE $2 AND status = $3
           ORDER BY started_at DESC LIMIT $4"#,
        since,
        server,
        status,
