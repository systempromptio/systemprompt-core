...
- `FilesAiPersistenceProvider::{find_by_id, list_by_user}` return `AiProviderError::Internal` when a stored file's metadata cannot be serialised instead of substituting `null`.
