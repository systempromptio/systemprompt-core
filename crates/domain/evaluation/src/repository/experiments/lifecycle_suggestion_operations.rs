//! Atomic suggestion operations retain exact retry identity and experiment budget.
...
use crate::experiments::{content_digest, conflict, invalid, missing};
use crate::Result;
...
impl EvaluationLifecycleRepository {
    pub async fn create_suggestion(&self, owner: &UserId, request: &SuggestionRequest) -> Result<EvalSuggestionId> {
        if request.hypothesis.trim().is_empty() || request.hypothesis.len()>4000
            || request.operation_key.trim().is_empty() || request.operation_key.len()>200
            || request.supporting_execution_ids.is_empty() || request.supporting_execution_ids.len()>1000
            || serde_jcs::to_vec(request)?.len()>262144 {
            return Err(invalid("Suggestion requires bounded evidence, hypothesis and operation key"));
        }
        ...
        let experiment=sqlx::query!("SELECT budget_id,spec FROM eval_experiments WHERE owner_id=$1 AND id=$2 FOR UPDATE",owner.as_str(),request.experiment_id.as_str()).fetch_optional(&mut *tx).await?.ok_or_else(|| missing("Experiment unavailable in this scope"))?;
        if experiment.budget_id!=request.budget_id.as_str() { return Err(conflict("Suggestion must use the experiment budget")); }
        let spec = serde_json::from_value(experiment.spec)?;
        self.admission.admit(&spec)?;
        let executions:Vec<_>=request.supporting_execution_ids.iter().map(|id|id.as_str().to_owned()).collect();
        ...
        if usize::try_from(valid).ok()!=Some(executions.len()) { return Err(invalid("Suggestions may use development evidence only")); }
        let reservation=match BudgetRepository::reserve_in(&mut tx,owner,&request.budget_id,&format!("suggestion:{}",request.operation_key),request.maximum_cost_microdollars).await? {
            ReservationAdmission::Admitted(id)=>id,
            ReservationAdmission::AlreadyReserved(_)=>return Err(conflict("Suggestion operation has historical reservation without retained retry identity")),
        };
    ...
    pub async fn suggestion(&self,owner:&UserId,id:&EvalSuggestionId)->Result<crate::campaigns::suggestions::RetainedSuggestion> {
        ...
        Ok(crate::campaigns::suggestions::RetainedSuggestion{id:EvalSuggestionId::new(row.id),hypothesis:row.hypothesis,status:row.status,proposed_changes:row.proposed_changes,originating_evidence:row.originating_evidence})
    }
    pub async fn approval(&self,owner:&UserId,id:&systemprompt_identifiers::EvalApprovalId)->Result<super::ExecutionApproval> {
        ...
        Ok(super::ExecutionApproval{id:systemprompt_identifiers::EvalApprovalId::new(row.id),execution_id:systemprompt_identifiers::EvalExecutionId::new(row.execution_id),operation:row.operation,precondition_digest:row.precondition_digest,status:row.status})
    }
}
