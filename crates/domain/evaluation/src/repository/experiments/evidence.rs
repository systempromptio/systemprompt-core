...
impl EvidenceRepository {
    ...
    pub async fn register_managed_workspace(
        ...
        if content_digest(manifest)? != expected_digest || file_count > 256 || byte_count > 8 * 1024 * 1024 {
            return Err(invalid("Managed workspace projection failed digest or size verification"));
        }
        let assets = managed_assets(manifest)?;
        let expanded_bytes = assets.iter().try_fold(0usize, |total, (_, _, bytes, _)| total.checked_add(bytes.len()))
            .ok_or_else(|| invalid("Managed workspace asset size overflow"))?;
        if assets.len() != file_count || expanded_bytes != byte_count {
            return Err(invalid("Managed workspace asset counts differ from the verified projection"));
        }
        let file_count = i32::try_from(file_count).map_err(|_| invalid("Managed file count overflow"))?;
        let byte_count = i64::try_from(byte_count).map_err(|_| invalid("Managed byte count overflow"))?;
        ...
        if !verified { return Err(conflict("Managed workspace registration conflicts with retained content")); }
        tx.commit().await?;
        Ok(())
    }

    pub async fn get_managed_workspace(&self, owner: &UserId, digest: &str) -> Result<super::ManagedWorkspaceReference> {
        ...
        if managed_revision_id.starts_with("legacy-archive:") {
            return Err(missing("Legacy workspace was retained as a verified archive and must be replaced by a managed bundle before execution"));
        }
        ...
        let actual = stored.into_iter().map(|asset| (asset.path, asset.asset_digest, asset.content, asset.executable)).collect::<Vec<_>>();
        let expected_bytes = expected.iter().try_fold(0usize, |total, (_, _, content, _)| total.checked_add(content.len()))
            .ok_or_else(|| invalid("Managed workspace asset size overflow"))?;
        if actual != expected
            || usize::try_from(row.verified_file_count).ok() != Some(expected.len())
            || usize::try_from(row.verified_byte_count).ok() != Some(expected_bytes)
        {
            return Err(invalid("Managed workspace retained assets failed integrity verification"));
        }
    ...
    pub async fn list_request_ids(&self, owner: &UserId, execution: &EvalExecutionId) -> Result<Vec<AiRequestId>> {
    ...
    pub async fn list_request_ids_by_traffic(&self, owner: &UserId, execution: &EvalExecutionId, traffic_class: &str) -> Result<Vec<AiRequestId>> {
        if !matches!(traffic_class, "fixture" | "live_evaluation" | "suggestion" | "judge") { return Err(invalid("Unknown evaluation traffic class")); }
...
fn managed_assets(manifest: &serde_json::Value) -> Result<Vec<(String, String, Vec<u8>, bool)>> {
    let revisions = manifest.get("revisions").and_then(serde_json::Value::as_object)
        .ok_or_else(|| invalid("Managed workspace requires revision manifests"))?;
    let assets = manifest.get("assets").and_then(serde_json::Value::as_object)
        .ok_or_else(|| invalid("Managed workspace requires exact assets"))?;
    let mut rows = Vec::new();
    for (revision, value) in revisions {
        let files = value.get("files").and_then(serde_json::Value::as_object)
            .ok_or_else(|| invalid("Managed revision files are missing"))?;
        for (path, entry) in files {
            if path.starts_with('/') || path.contains(['\\', ':']) || path.split('/').any(|part| matches!(part, "" | "." | "..")) {
                return Err(invalid("Managed workspace contains a non-portable path"));
            }
            let digest = entry.get("digest").and_then(serde_json::Value::as_str)
                .ok_or_else(|| invalid("Managed file digest is missing"))?;
            let bytes: Vec<u8> = serde_json::from_value(assets.get(digest).cloned().ok_or_else(|| invalid("Managed file asset is missing"))?)?;
            let declared = entry.get("bytes").and_then(serde_json::Value::as_u64)
                .ok_or_else(|| invalid("Managed file byte count is missing"))?;
            let executable = entry.get("executable").and_then(serde_json::Value::as_bool)
                .ok_or_else(|| invalid("Managed file mode is missing"))?;
            if bytes.len() as u64 != declared || hex::encode(Sha256::digest(&bytes)) != digest {
                return Err(invalid("Managed file bytes differ from their digest or length"));
            }
            rows.push((format!("{revision}/{path}"), digest.to_owned(), bytes, executable));
        }
    }
    Ok(rows)
}
