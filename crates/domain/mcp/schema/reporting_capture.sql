...
FROM (SELECT mcp_execution_id,tool_name,server_name,started_at,completed_at,execution_time_ms,status,error_message,user_id,session_id,context_id,task_id,trace_id,request_method,request_source,actor_kind,actor_id,ai_tool_call_id,source,correlation,payload_sha256,created_at FROM mcp_tool_executions) fact;
