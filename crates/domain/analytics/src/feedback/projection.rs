...
impl FeedbackFactsRepository {
    ...
    async fn replace_in(
        ...
        sqlx::query!("INSERT INTO analytics_fact_deltas(owner_id,generation,fact_kind,source,fact_id,before_fact,after_fact) VALUES($1,$2,$3,$4,$5,$6,$7)", owner.as_str(), generation, kind, &change.key.source, change.key.id.as_str(), before, after).execute(&mut **tx).await?;
        Ok(())
    }
}
