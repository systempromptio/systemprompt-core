-- Seed default anomaly detection thresholds. Idempotent via ON CONFLICT.
-- Creates the table guard-style so the migration is order-independent
-- relative to the declarative schema (which also CREATE TABLE IF NOT EXISTS).
