...
//! `gemini` uses [`GeminiProvider`]. Connectivity is the provider's
//! [`UpstreamTarget`] — endpoint, credential and hosting resolved from the
//! registry entry — so every client reaches whatever platform the catalog
//! names; the per-provider AI policy supplies resilience and the web-search
//! toggle.
//!
    ...
    pub fn create(
        ...
        let inner: Arc<dyn AiProvider> = match params.target.wire() {
            ...
            WireProtocol::OpenAiChat | WireProtocol::OpenAiResponses => {
                let provider = OpenAiProvider::with_target(params.target.clone())
                    .with_models(params.models.to_vec())
                    .with_default_model(params.default_model.map(str::to_owned));
                let provider = if params.google_search_enabled {
                    provider.with_web_search()
            ...
            WireProtocol::Gemini => {
                let mut provider = GeminiProvider::with_target(params.target.clone())?
                    .with_models(params.models.to_vec())
                    .with_default_model(params.default_model.map(str::to_owned));
