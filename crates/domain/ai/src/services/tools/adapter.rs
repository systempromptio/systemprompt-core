...
pub fn mcp_tool_to_definition(tool: &McpTool) -> ToolDefinition {
    ToolDefinition {
        ...
        // Why: model_config is metadata; if it cannot be serialized we log and omit it rather than
        // failing the conversion of the entire tool.
...
pub fn definition_to_mcp_tool(def: &ToolDefinition) -> McpTool {
    McpTool {
        ...
        // Why: see `mcp_tool_to_definition`; model_config is metadata, drop on parse failure rather
        // than failing the whole tool conversion.
...
pub fn rmcp_result_to_trait_result(result: &CallToolResult) -> TraitToolCallResult {
    ...
    TraitToolCallResult {
        ...
        // Why: meta is opaque advisory data; preserve the rest of the result if serialization fails
        // after logging.
