...
- `managed_invocation_attributions` is no longer created by core; it was never written. Migration 016 drops it together with `managed_api_operations` and the consumer invocation evidence, attribution projection and attribution history tables, except one a consumer view still reads, which the consumer drops after redefining the view.
...
- `managed_api_operations`, `managed_consumer_invocation_evidence`, `managed_consumer_attribution_projection` and `managed_consumer_attribution_history` are no longer created by core. Migration `016` drops them with the never-read `managed_git_verifications`, `managed_dependency_verifications`, `managed_resource_git_bindings` and `managed_inventory_captures`.
