...
impl ManagedRepository {
    pub async fn record_consumer_invocation(
        ...
        let projection = sqlx::query!("SELECT receipt_id,version FROM managed_consumer_attribution_projection WHERE evidence_id=$1", stored.id)
            .fetch_one(&mut *tx).await?;
        tx.commit().await?;
...
pub(super) async fn correct_session(
    ...
    loop {
        let rows = sqlx::query!("SELECT id FROM managed_consumer_invocation_evidence WHERE consumer_id=$1 AND device_id=$2 AND host=$3 AND native_session_id=$4 AND id>$5 ORDER BY id LIMIT 128", identity.consumer_id.as_str(), identity.device_id.as_str(), host, session, after)
...
async fn correct_one(tx: &mut Transaction<'_, Postgres>, evidence_id: &str) -> Result<()> {
    ...
    let changed = sqlx::query!("INSERT INTO managed_consumer_attribution_projection(evidence_id,receipt_id) VALUES($1,$2) ON CONFLICT(evidence_id) DO UPDATE SET receipt_id=EXCLUDED.receipt_id,version=managed_consumer_attribution_projection.version+1,updated_at=now() WHERE managed_consumer_attribution_projection.receipt_id IS DISTINCT FROM EXCLUDED.receipt_id RETURNING version", evidence_id, receipt)
    ...
    if let Some(changed) = changed {
        ...
        sqlx::query!("INSERT INTO managed_consumer_attribution_history(evidence_id,version,receipt_id,reason) VALUES($1,$2,$3,$4)", evidence_id, changed.version, receipt, reason)
            .execute(&mut **tx).await?;
    }
    Ok(())
}
