...
fn render_skill(bundle: &RevisionBundle, host: EvaluatorClient, key: &str) -> Result<String> {
    ...
    let (name, description, instructions) = if let Some(config) = root.0.get("config.yaml") {
        let config: systemprompt_models::DiskSkillConfig =
            serde_yaml::from_slice(&config.bytes).map_err(|_| ManagedError::Integrity)?;
        let content = root
            .0
    ...
    let rendered_name =
        serde_json::to_string(&rendered_name).map_err(|_| ManagedError::Integrity)?;
    let description = serde_json::to_string(&description).map_err(|_| ManagedError::Integrity)?;
