...
use systemprompt_identifiers::{ManagedReconciliationId, ManagedResourceId, ResourceRevisionId, UserId};
...
pub enum ConflictResolution { Candidate, Incoming, Manual, Delete }

impl ConflictResolution {
    const fn as_str(self) -> &'static str {
        match self { Self::Candidate => "candidate", Self::Incoming => "incoming", Self::Manual => "manual", Self::Delete => "delete" }
    }
}
...
impl ManagedRepository {
    pub async fn begin_reconciliation(&self, owner: &UserId, request: &ReconciliationRequest) -> Result<ReconciliationRecord> {
        let revisions = [
        ...
        for revision in revisions {
            let found = sqlx::query_scalar!("SELECT id FROM managed_revisions WHERE owner_id=$1 AND resource_id=$2 AND id=$3",
                owner.as_str(), request.resource_id.as_str(), revision.as_str())
                .fetch_optional(&self.pool).await?;
            if found.is_none() { return Err(ManagedError::Unavailable); }
        }
        let base = self.get_revision_files(owner, &request.upstream_base_revision_id).await?;
        let candidate = self.get_revision_files(owner, &request.managed_candidate_revision_id).await?;
        let incoming = self.get_revision_files(owner, &request.incoming_revision_id).await?;
        let mut paths = std::collections::BTreeSet::new();
        paths.extend(base.0.keys().cloned()); paths.extend(candidate.0.keys().cloned()); paths.extend(incoming.0.keys().cloned());
        let mut conflicts = Vec::new();
        for path in paths {
            let digest = |files: &super::RevisionFiles| files.0.get(&path).map(|file| super::AssetDigest::of(&file.bytes).as_str().to_owned());
            let b = digest(&base); let c = digest(&candidate); let i = digest(&incoming);
            if c != b && i != b && c != i {
                conflicts.push(ReconciliationConflict { path, base_digest: b, candidate_digest: c, incoming_digest: i, resolution: None });
            }
        }
        ...
        tx.commit().await?;
        Ok(ReconciliationRecord { id: ManagedReconciliationId::new(stored), status: "open".to_owned(), conflicts, resolved_revision_id: None })
    }

    pub async fn resolve_reconciliation_conflict(&self, owner: &UserId, id: &ManagedReconciliationId, path: &str, resolution: ConflictResolution, resolved_digest: Option<&str>) -> Result<()> {
        ...
        match (resolution, resolved_digest) {
            ...
            (ConflictResolution::Manual, _) => return Err(super::error::invalid("Manual reconciliation requires an exact lowercase SHA-256 digest")),
            (_, None) => {},
            (_, Some(_)) => return Err(super::error::invalid("Only manual reconciliation accepts a resolved digest")),
        }
        ...
        if changed.rows_affected() != 1 { return Err(ManagedError::Unavailable); }
        Ok(())
    }

    pub async fn complete_reconciliation(&self, owner: &UserId, actor: &UserId, id: &ManagedReconciliationId, revision: &ResourceRevisionId) -> Result<()> {
        ...
        if conflicts.iter().any(|conflict| conflict.resolution.is_none()) {
            return Err(ManagedError::Conflict("Reconciliation has unresolved conflicts".to_owned()));
        }
        let base = self.get_revision_files(owner, &ResourceRevisionId::new(row.upstream_base_revision_id)).await?;
        let candidate = self.get_revision_files(owner, &ResourceRevisionId::new(row.managed_candidate_revision_id)).await?;
        let incoming = self.get_revision_files(owner, &ResourceRevisionId::new(row.incoming_revision_id)).await?;
        ...
        if !belongs { return Err(ManagedError::Unavailable); }
        ...
        for path in paths {
            ...
            let expected = if same_file(candidate_file, incoming_file) {
                ...
                let conflict = conflicts.iter().find(|conflict| conflict.path == path)
                ...
                match conflict.resolution.as_deref() {
                    ...
                    Some("delete") => None,
                    Some("manual") => {
                        let actual = resolved.0.get(&path).ok_or_else(|| ManagedError::Conflict("Manual resolution content is missing".to_owned()))?;
                        let digest = super::AssetDigest::of(&actual.bytes);
                        if conflict.resolved_digest.as_deref() != Some(digest.as_str()) {
                            return Err(ManagedError::Conflict("Manual resolution digest does not match supplied content".to_owned()));
                        }
                        Some(actual)
                    },
                    _ => return Err(ManagedError::Conflict("Invalid reconciliation resolution".to_owned())),
                }
            };
            if !same_file(resolved.0.get(&path), expected) {
                return Err(ManagedError::Conflict(format!("Resolved revision differs from the recorded merge at {path}")));
            }
        }
        ...
        if changed.rows_affected() != 1 { return Err(ManagedError::Conflict("Reconciliation has unresolved conflicts".to_owned())); }
        Ok(())
    }
}
