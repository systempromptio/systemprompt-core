//! Idempotent distribution, exact installation receipts, and verified attribution.
//!
...
use systemprompt_identifiers::{DistributionId, InstallationReceiptId, InvocationAttributionId, ManagedResourceId, PublicationId, ResourceRevisionId, UserId};
...
pub enum TrafficClass { Production, Fixture, LiveEvaluation, Suggestion, Judge }

impl TrafficClass {
    const fn as_str(self) -> &'static str { match self { Self::Production => "production", Self::Fixture => "fixture", Self::LiveEvaluation => "live_evaluation", Self::Suggestion => "suggestion", Self::Judge => "judge" } }
}
...
impl ManagedRepository {
    pub async fn list_distribution_status(&self, owner: &UserId) -> Result<Vec<DistributionStatus>> {
    ...
    pub async fn list_installation_receipts(&self, owner: &UserId, resource: Option<&ManagedResourceId>) -> Result<Vec<InstallationReceipt>> {
        ...
        rows.into_iter().map(|row| Ok(InstallationReceipt { id: InstallationReceiptId::new(row.id), installation_id: row.installation_id, publication_id: PublicationId::new(row.publication_id), resource_id: ManagedResourceId::new(row.resource_id), generation: row.generation, bundle_digest: AssetDigest::try_from(row.bundle_digest)?, installed_manifest: serde_json::from_value(row.installed_manifest)?, client_evidence: row.client_evidence, verified_at: row.verified_at })).collect()
    }

    pub async fn claim_distribution(&self, owner: &UserId, claim_token: &str) -> Result<Option<DistributionClaim>> {
        if claim_token.trim().is_empty() || claim_token.len() > 200 { return Err(super::error::invalid("Invalid distribution claim token")); }
        ...
        tx.commit().await?;
        Ok(Some(DistributionClaim { id, outbox_id, publication_id: PublicationId::new(publication_id), generation, payload: row.payload, claim_token: claim_token.to_owned() }))
    }

    pub async fn complete_distribution(&self, owner: &UserId, claim: &DistributionClaim, delivered: bool, error: Option<&str>) -> Result<()> {
        let mut tx = self.pool.begin().await?;
        ...
        if existing.status == status && existing.error.as_deref() == error { tx.commit().await?; return Ok(()); }
        ...
        if changed.rows_affected() != 1 { return Err(ManagedError::Conflict("Distribution claim is stale or already completed".to_owned())); }
        if delivered {
    ...
    pub async fn record_installation(&self, owner: &UserId, request: &InstallationReceiptRequest) -> Result<InstallationReceipt> {
        let evidence_session = request.client_evidence.get("session_id").and_then(serde_json::Value::as_str);
        let evidence_owner = request.client_evidence.get("owner_id").and_then(serde_json::Value::as_str);
        if request.installation_id.trim().is_empty() || request.installation_id.len() > 200 || request.generation < 1 || request.files.len() > 256 || evidence_session.is_none_or(str::is_empty) || evidence_owner != Some(owner.as_str()) || serde_jcs::to_vec(&request.client_evidence)?.len() > 65_536 { return Err(super::error::invalid("Invalid authenticated installation receipt")); }
        let bundle = self.get_publication_bundle(owner, &request.resource_id, request.generation, &request.bundle_digest).await?;
        ...
        if publication != request.publication_id.as_str() { return Err(ManagedError::Integrity); }
        let mut expected = BTreeMap::new();
        for (revision_id, manifest) in &bundle.revisions {
            for (path, file) in &manifest.files {
                expected.insert((revision_id.as_str(), path.as_str()), (file.digest.as_str(), file.bytes, file.executable));
            }
        }
        let supplied: BTreeMap<_, _> = request.files.iter().map(|file| ((file.revision_id.as_str(), file.path.as_str()), (file.digest.as_str(), file.bytes, file.executable))).collect();
        if supplied != expected || supplied.len() != request.files.len() { return Err(ManagedError::Integrity); }
        ...
        Ok(InstallationReceipt { id: InstallationReceiptId::new(row.id), installation_id: request.installation_id.clone(), publication_id: request.publication_id.clone(), resource_id: request.resource_id.clone(), generation: request.generation, bundle_digest: request.bundle_digest.clone(), installed_manifest: request.files.clone(), client_evidence: request.client_evidence.clone(), verified_at: row.verified_at })
    }

    pub async fn attribute_invocation(&self, owner: &UserId, request: &InvocationAttributionRequest) -> Result<InvocationAttribution> {
        let session = request.authenticated_evidence.get("session_id").and_then(serde_json::Value::as_str);
        let attested_owner = request.authenticated_evidence.get("owner_id").and_then(serde_json::Value::as_str);
        if request.invocation_id.trim().is_empty() || request.invocation_id.len() > 200 || session.is_none_or(str::is_empty) || attested_owner.is_some_and(|value| value != owner.as_str()) || serde_jcs::to_vec(&request.authenticated_evidence)?.len() > 65_536 { return Err(super::error::invalid("Invalid authenticated invocation attribution")); }
        let verified = if let (Some(installation), Some(key), Some(revision), Some(generation)) = (
            request.installation_id.as_deref(), request.resource_key.as_deref(), request.resource_revision_id.as_ref(), request.publication_generation,
        ) {
        ...
        let (status, receipt_id, resource_id, revision_id, generation) = if let Some(row) = verified {
            ("verified", Some(row.receipt_id), Some(row.resource_id), row.revision_id, Some(row.generation))
        } else { ("revision_unknown", None, None, None, None) };
        ...
        {
            return Err(ManagedError::Conflict("Invocation attribution replay conflicts with immutable evidence".to_owned()));
        }
        Ok(InvocationAttribution { id: InvocationAttributionId::new(stored.id), invocation_id: request.invocation_id.clone(), installation_id: stored.installation_id, resource_id: stored.resource_id.map(ManagedResourceId::new), revision_id: stored.revision_id.map(ResourceRevisionId::new), publication_generation: stored.publication_generation, traffic_class: request.traffic_class, status: stored.status })
    }
}
