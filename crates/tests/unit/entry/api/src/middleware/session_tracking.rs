...
use systemprompt_api::services::middleware::should_skip_session_tracking;
...
fn health_endpoint_is_skipped() {
    assert!(should_skip_session_tracking("/health"));
}
...
fn content_page_is_not_skipped() {
    assert!(!should_skip_session_tracking("/about"));
}
