...
async fn dead_repo() -> Arc<SessionRepository> {
    ...
    Arc::new(systemprompt_test_fixtures::fixture_analytics_repositories(&pool).map(|repositories| repositories.sessions).expect("repository construction is not a query"))
}
...
async fn live_repo() -> Arc<SessionRepository> {
    ...
    Arc::new(systemprompt_test_fixtures::fixture_analytics_repositories(&pool).map(|repositories| repositories.sessions).expect("repository"))
}
