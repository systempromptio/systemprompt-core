...
fn acknowledge_installation(outbox: &Outbox, key: &str, receipt: &str) {
    outbox.delivery(key, Ok(ConsumerReceiptResponse {
...
fn completed_sessions_release_both_capacity_bounds_without_changing_receipt() {
    ...
    for index in 0..1025 {
        ...
        outbox.queue_session(EvaluatorClient::Codex, &session).unwrap();
        outbox.acknowledge_session(&key, &InstallationReceiptId::new("receipt"), &session).unwrap();
        assert!(outbox.entries().unwrap()[0].1.session_bindings.is_empty());
    }
    let persisted: serde_json::Value = serde_json::from_slice(
        &std::fs::read(dir.path().join("outbox.json")).unwrap()).unwrap();
    ...
    assert!(matches!(outbox.enqueue(conflict), Err(FeedbackError::Readback)));
}
...
fn pending_capacity_rejects_atomically_and_restart_preserves_mixed_delivery() {
    ...
    acknowledge_installation(&outbox, &key, "receipt");
    for index in 0..256 {
        outbox.queue_session(EvaluatorClient::Codex, &format!("pending-{index}")).unwrap();
    }
    let before = std::fs::read(&path).unwrap();
    assert!(matches!(outbox.queue_session(EvaluatorClient::Codex, "overflow"), Err(FeedbackError::Full)));
    assert_eq!(std::fs::read(&path).unwrap(), before);
    outbox.acknowledge_session(&key, &InstallationReceiptId::new("receipt"), "pending-0").unwrap();
    let recovered = Outbox::new(path, scope("device"));
    recovered.queue_session(EvaluatorClient::Codex, "replacement").unwrap();
    ...
    for index in 1..256 {
        assert_eq!(entries[0].1.session_bindings.get(&format!("pending-{index}")), Some(&false));
    }
}
...
fn acknowledged_retry_remains_frozen_across_restart_and_upgrade() {
    ...
    let key = outbox.enqueue(receipt.clone()).unwrap();
    acknowledge_installation(&outbox, &key, "receipt");
    outbox.queue_session(EvaluatorClient::Codex, "original").unwrap();
    outbox.acknowledge_session(&key, &InstallationReceiptId::new("receipt"), "original").unwrap();
    let recovered = Outbox::new(path, scope("device"));
    recovered.acknowledge_session(&key, &InstallationReceiptId::new("receipt"), "original").unwrap();
    assert!(matches!(recovered.acknowledge_session(&key, &InstallationReceiptId::new("forged"), "original"), Err(FeedbackError::Scope)));
    ...
    recovered.queue_session(EvaluatorClient::Codex, "original").unwrap();
    assert!(recovered.entries().unwrap().iter().all(|(_, entry)| entry.session_bindings.is_empty()));
}
...
fn one_acknowledged_resource_cannot_discard_another_resources_pending_binding() {
    ...
    outbox.queue_session(EvaluatorClient::Codex, "shared").unwrap();
    outbox.acknowledge_session(&first, &InstallationReceiptId::new("first"), "shared").unwrap();
    let recovered = Outbox::new(path, scope("device"));
    for (key, entry) in recovered.entries().unwrap() {
        assert_eq!(entry.session_bindings.get("shared"), Some(&(key == first)));
    }
    recovered.acknowledge_session(&second, &InstallationReceiptId::new("second"), "shared").unwrap();
    assert!(recovered.entries().unwrap().iter().all(|(_, entry)| entry.session_bindings.is_empty()));
}
...
fn legacy_completed_bindings_compact_without_losing_pending_installation_sessions() {
    ...
    let key = outbox.enqueue(receipt).unwrap();
    acknowledge_installation(&outbox, &key, "receipt");
    outbox.queue_session(EvaluatorClient::Codex, "legacy-completed").unwrap();
    ...
    recovered.reserve_installation(systemprompt_bridge::feedback::outbox::PendingInstallation::new(
        pending, EvaluatorClient::Codex, vec![dir.path().join("pending")],
    )).unwrap();
    recovered.queue_session(EvaluatorClient::Codex, "needs-installation").unwrap();
    recovered.acknowledge_session(&key, &InstallationReceiptId::new("receipt"), "needs-installation").unwrap();
    let entries = recovered.entries().unwrap();
    assert!(!entries[0].1.session_bindings.contains_key("legacy-completed"));
    assert_eq!(entries[0].1.session_bindings.get("needs-installation"), Some(&true));
