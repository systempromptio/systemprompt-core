use systemprompt_bridge::proxy::session::derive_gateway_conversation_id;

#[test]
fn anthropic_messages_shape_hashes() {
    let body = br#"{"system":"helpful","messages":[{"role":"user","content":"hi"}]}"#;
    let h = derive_gateway_conversation_id(body).expect("anthropic body should hash");
    assert_ne!(h, 0);
}
...
fn openai_chat_shape_hashes() {
    ...
    let h = derive_gateway_conversation_id(body).expect("openai chat body should hash");
    assert_ne!(h, 0);
}
...
fn openai_responses_shape_hashes() {
    ...
    let h = derive_gateway_conversation_id(body).expect("openai responses body should hash");
    assert_ne!(h, 0);
}
...
fn anthropic_array_content_hashes() {
    ...
    let h = derive_gateway_conversation_id(body).expect("anthropic array content should hash");
    assert_ne!(h, 0);
}

#[test]
fn malformed_body_returns_none() {
    assert!(derive_gateway_conversation_id(b"not json").is_none());
}

#[test]
fn empty_messages_returns_none() {
    assert!(derive_gateway_conversation_id(br#"{"messages":[]}"#).is_none());
}

#[test]
fn no_first_user_returns_none() {
    let body = br#"{"messages":[{"role":"system","content":"only system"}]}"#;
    assert!(derive_gateway_conversation_id(body).is_none());
}
...
fn anthropic_and_openai_chat_with_same_system_hash_equal() {
    ...
    assert_eq!(
        derive_gateway_conversation_id(anthropic),
        derive_gateway_conversation_id(openai_chat),
...
fn openai_responses_instructions_align_with_anthropic_system() {
    ...
    assert_eq!(
        derive_gateway_conversation_id(anthropic),
        derive_gateway_conversation_id(openai_responses),
...
fn role_changes_hash() {
    ...
    assert_ne!(
        derive_gateway_conversation_id(user_first),
        derive_gateway_conversation_id(assistant_first)
    );
}
...
fn unicode_content_hashes() {
    ...
    let h = derive_gateway_conversation_id(body).expect("unicode body should hash");
    assert_ne!(h, 0);
}
...
fn long_content_hashes_stably() {
    ...
    let h1 = derive_gateway_conversation_id(body.as_bytes()).expect("long body should hash");
    let h2 = derive_gateway_conversation_id(body.as_bytes()).expect("long body should hash");
    assert_eq!(h1, h2);
}
...
fn array_content_concatenates_text_parts() {
    ...
    assert_eq!(
        derive_gateway_conversation_id(split),
        derive_gateway_conversation_id(joined),
...
fn multiple_system_messages_concatenate() {
    ...
    assert_eq!(
        derive_gateway_conversation_id(one),
        derive_gateway_conversation_id(two),
...
fn anthropic_array_system_value_serializes_consistently() {
    ...
    let h1 = derive_gateway_conversation_id(body).expect("array system should hash");
    let h2 = derive_gateway_conversation_id(body).expect("array system should hash");
    assert_eq!(h1, h2);
}
...
fn empty_string_content_still_hashes() {
    ...
    let h = derive_gateway_conversation_id(body).expect("empty content should still hash");
    assert_ne!(h, 0);
}
...
fn missing_role_defaults_to_user() {
    ...
    assert_eq!(
        derive_gateway_conversation_id(no_role),
        derive_gateway_conversation_id(user_role),
