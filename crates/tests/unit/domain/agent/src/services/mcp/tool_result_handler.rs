...
fn base_ctx() -> RequestContext {
    RequestContext::new(
        ...
        ContextId::try_new(CTX_UUID).expect("valid ContextId"),
        AgentName::try_new("trh-agent").expect("valid AgentName"),
    )
}
...
fn rejects_unauthenticated_caller() {
    ...
    let context_id = ContextId::try_new(CTX_UUID).expect("valid ContextId");
    let args = json!({});
