...
fn is_bot_ip_returns_true_for_microsoft_157_ip() {
    ...
    let analytics = SessionAnalytics::builder(&headers).with_caller_ip("157.55.39.1".parse().unwrap()).build();
...
fn is_bot_ip_returns_true_for_microsoft_207_ip() {
    let headers = HeaderMap::new();
    let analytics = SessionAnalytics::builder(&headers).with_caller_ip("207.46.13.1".parse().unwrap()).build();
    assert!(analytics.is_bot_ip());
}
...
fn is_bot_ip_returns_true_for_facebook_69_ip() {
    let headers = HeaderMap::new();
    let analytics = SessionAnalytics::builder(&headers).with_caller_ip("69.171.250.1".parse().unwrap()).build();
    assert!(analytics.is_bot_ip());
}
...
fn is_bot_ip_returns_true_for_facebook_173_ip() {
    let headers = HeaderMap::new();
    let analytics = SessionAnalytics::builder(&headers).with_caller_ip("173.252.88.1".parse().unwrap()).build();
    assert!(analytics.is_bot_ip());
}
...
fn is_bot_ip_returns_true_for_facebook_31_ip() {
    let headers = HeaderMap::new();
    let analytics = SessionAnalytics::builder(&headers).with_caller_ip("31.13.24.1".parse().unwrap()).build();
    assert!(analytics.is_bot_ip());
}
