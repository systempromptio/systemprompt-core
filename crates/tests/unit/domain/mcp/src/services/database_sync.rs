...
async fn cleanup_stale_services_empty_table_returns_ok() {
    ...
    let svc_repo = ServiceRepository::new(&db, systemprompt_identifiers::InstanceId::new("test-instance")).unwrap();
...
async fn delete_disabled_services_removes_only_the_disabled_service() {
    ...
    let deleted = delete_disabled_services(&ServiceRepository::new(&db, systemprompt_identifiers::InstanceId::new("test-instance")).unwrap(), &enabled)
        .await
        .unwrap();
