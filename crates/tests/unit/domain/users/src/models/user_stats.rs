...
fn user_export_creation() {
    let e = create_test_user_export();
    assert_eq!(e.id, "user-export-123");
    assert_eq!(e.name, "exportuser");
    assert_eq!(e.email, "export@example.com");
}
#[test]
fn user_export_debug() {
    assert!(format!("{:?}", create_test_user_export()).contains("UserExport"));
}
