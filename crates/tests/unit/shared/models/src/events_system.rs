...
fn context_updated_omits_absent_name() {
    let event = SystemEventBuilder::context_updated(
        ContextId::try_new(CTX).expect("valid ContextId"),
        None,
    );
    ...
    let named = SystemEventBuilder::context_updated(
        ContextId::try_new(CTX).expect("valid ContextId"),
        Some("N".to_owned()),
    );
...
fn context_deleted_and_heartbeat_round_trip() {
    let deleted =
        SystemEventBuilder::context_deleted(ContextId::try_new(CTX).expect("valid ContextId"));
...
fn contexts_snapshot_serializes_summaries() {
    let stats = ContextWithStats {
        context_id: ContextId::try_new(CTX).expect("valid ContextId"),
