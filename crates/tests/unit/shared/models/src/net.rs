...
mod ssrf_adversarial_tests {
    ...
    // -- IETF-reserved and multicast IPv4 ------------------------------------

    #[test]
    fn rejects_ipv4_zero_network_multicast_and_reserved() {
        for host in [
            "0.1.2.3",
            "192.0.0.8",
            "198.18.0.1",
            ...
            assert!(
                matches!(
                    validate_outbound_url(&format!("https://{host}/")),
                    Err(OutboundUrlError::BlockedHost(_))
                ),
    ...
    #[test]
    fn rejects_nat64_and_ipv4_compatible_embeddings_of_blocked_v4() {
        for host in ["[64:ff9b::a9fe:a9fe]", "[::a9fe:a9fe]", "[64:ff9b::7f00:1]"] {
            assert!(
                matches!(
                    validate_outbound_url(&format!("https://{host}/")),
                    Err(OutboundUrlError::BlockedHost(_))
    ...
    fn rejects_ipv6_multicast() {
        assert!(matches!(
            validate_outbound_url("https://[ff02::1]/"),
            Err(OutboundUrlError::BlockedHost(_))
        ));
    }

    #[test]
    fn nat64_embedding_of_a_public_v4_is_allowed() {
        assert!(validate_outbound_url("https://[64:ff9b::808:808]/").is_ok());
    }
