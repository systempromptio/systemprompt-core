...
async fn test_broadcaster_broadcast_only_to_target_user() {
    ...
    broadcaster
        .register(&user1, &ConnectionId::new("conn-1"), sender1)
        .await;
    broadcaster
        .register(&user2, &ConnectionId::new("conn-2"), sender2)
        .await;
    ...
    let _event = rx1
        .recv()
        .await
        .expect("Should receive on user1 connection");
...
async fn test_broadcaster_total_connections_multiple_users() {
    ...
    broadcaster
        .register(&user1, &ConnectionId::new("conn-1"), s1)
        .await;
    broadcaster
        .register(&user1, &ConnectionId::new("conn-2"), s2)
        .await;
    broadcaster
...
async fn test_broadcaster_connected_users_multiple() {
    ...
    broadcaster
        .register(&first_user, &ConnectionId::new("conn-1"), s1)
        .await;
    broadcaster
