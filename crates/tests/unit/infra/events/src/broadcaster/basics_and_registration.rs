...
async fn test_broadcaster_register_multiple_connections_same_user() {
    ...
    let (sender3, _rx3) = tokio::sync::mpsc::channel(systemprompt_events::SSE_BUFFER);
...
async fn test_broadcaster_register_replaces_same_connection_id() {
    ...
    broadcaster.register(&user_id, &ConnectionId::new("conn-1"), sender1).await;
    broadcaster.register(&user_id, &ConnectionId::new("conn-1"), sender2).await;
...
async fn test_broadcaster_unregister_connection() {
    ...
    broadcaster.unregister(&user_id, &ConnectionId::new("conn-1")).await;
...
async fn test_broadcaster_unregister_nonexistent_connection() {
    ...
    broadcaster.unregister(&user_id, &ConnectionId::new("non-existent")).await;
    assert_eq!(broadcaster.connection_count(&user_id).await, 0);
}
