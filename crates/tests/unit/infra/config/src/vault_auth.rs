...
#[tokio::test]
async fn a_blank_environment_token_with_no_file_names_the_variable() {
    let server = MockServer::start().await;
    let cfg = fx::config(&server.uri(), fx::token_auth());
    let provider = VaultKvProvider::from_config(&cfg, |_name| Some("   ".to_owned())).unwrap();

    let err = vault_error(provider.fetch().await.unwrap_err());

    assert!(matches!(
        err,
        VaultError::MissingCredential { ref name } if name == "VAULT_TOKEN"
    ));
    assert!(err.to_string().contains("never write a token into profile"));
}

#[tokio::test]
async fn a_blank_environment_token_falls_through_to_the_token_file() {
    let server = MockServer::start().await;
    mount_document(&server, "file-token").await;
    ...
    let provider = VaultKvProvider::from_config(&cfg, |_name| Some(" \n".to_owned())).unwrap();

    provider.fetch().await.unwrap();
}

#[tokio::test]
async fn an_empty_token_file_is_reported_rather_than_sent_as_a_blank_token() {
    let server = MockServer::start().await;
    let dir = tempfile::tempdir().unwrap();
    let token_path = dir.path().join("vault-token");
...
#[tokio::test]
async fn a_missing_token_file_reports_the_path_it_tried() {
    let server = MockServer::start().await;
    let dir = tempfile::tempdir().unwrap();
    let token_path = dir.path().join("absent");
    ...
    assert!(matches!(
        err,
        VaultError::CredentialFile { ref path, .. } if path == &token_path.display().to_string()
    ));
}

#[tokio::test]
async fn a_missing_service_account_jwt_reports_the_path_it_tried() {
    let server = MockServer::start().await;
    let dir = tempfile::tempdir().unwrap();
    let jwt_path = dir.path().join("absent-token");
    ...
    assert!(matches!(
        err,
        VaultError::CredentialFile { ref path, .. } if path == &jwt_path.display().to_string()
    ));
}

#[tokio::test]
async fn a_login_response_that_is_not_the_expected_shape_is_malformed() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/v1/auth/approle/login"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"lease_id": ""})))
        .expect(1)
        .mount(&server)
        .await;
...
#[tokio::test]
async fn a_login_without_a_lease_duration_still_yields_a_usable_token() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/v1/auth/approle/login"))
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_json(serde_json::json!({ "auth": { "client_token": "no-lease-token" } })),
        )
        .expect(1)
        .mount(&server)
        .await;
    mount_document(&server, "no-lease-token").await;
...
#[tokio::test]
async fn a_blank_secret_id_is_reported_before_any_login_is_attempted() {
    let server = MockServer::start().await;
    let cfg = fx::config(
        &server.uri(),
        VaultAuth::AppRole {
    ...
    assert!(matches!(
        err,
        VaultError::MissingCredential { ref name } if name == "VAULT_SECRET_ID"
    ));
}

#[tokio::test]
async fn a_rejected_login_without_an_errors_array_still_reports_the_status() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
    ...
    let dir = tempfile::tempdir().unwrap();
    let jwt_path = dir.path().join("token");
    std::fs::write(&jwt_path, "a.b.c").unwrap();
    ...
    assert!(err.to_string().ends_with("HTTP 403"));
}
