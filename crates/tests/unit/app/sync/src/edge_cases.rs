...
mod config_additional_tests {
    ...
    #[test]
    fn builder_with_sync_client_secret() {
        let config = SyncConfig::builder(TenantId::new("tenant"), "https://api.com", "token", "/services")
            .with_sync_client_secret(Some("sync-secret-token".to_string())).build();
        assert_eq!(config.sync_client_secret, Some("sync-secret-token".to_string()));
    }
    ...
    fn builder_all_options() {
        let config = SyncConfig::builder(TenantId::new("tenant-full"), "https://api.com", "api-token", "/services")
            ...
            .with_sync_client_secret(Some("sync-token".to_string()))
