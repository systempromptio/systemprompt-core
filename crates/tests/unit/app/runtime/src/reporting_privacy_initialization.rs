...
async fn committed_preinitialization_evidence_requires_real_baseline_and_delivery_before_expiry() {
    ...
    sqlx::query("INSERT INTO users(id,name,email) VALUES('pending','pending','pending@example.test')")
    ...
    sqlx::query("SELECT public.prepare_reporting_privacy()").execute(&mut *tx).await.unwrap();
    let cutoff = Utc::now() - Duration::days(90);
    assert_eq!(sqlx::query_scalar::<_, i64>("SELECT public.expire_reporting_sessions($1)")
        .bind(cutoff).fetch_one(&mut *tx).await.unwrap(), 1);
    sqlx::query("SELECT public.finish_reporting_privacy($1)")
        .bind(cutoff).execute(&mut *tx).await.unwrap();
...
async fn owners_compacting_valid_clocks_out_of_order_preserve_global_privacy_and_local_clock_checks() {
    ...
    for script in [
        ...
        sqlx::raw_sql(sqlx::AssertSqlSafe(script)).execute(&*pool).await.unwrap();
    }
    ...
    for owner in ["first", "second"] {
        sqlx::query("INSERT INTO analytics_fact_checkpoints(owner_id) VALUES($1)").bind(owner).execute(&*pool).await.unwrap();
        sqlx::query("INSERT INTO analytics_snapshot_state(owner_id) VALUES($1)").bind(owner).execute(&*pool).await.unwrap();
        sqlx::query("INSERT INTO analytics_fact_consumers(owner_id,consumer) VALUES($1,'snapshots-v1')").bind(owner).execute(&*pool).await.unwrap();
    }
    ...
    for (owner, clock) in [("first", now), ("second", now - Duration::days(1))] {
        let mut tx = pool.begin().await.unwrap();
        FeedbackSnapshotsRepository::compact_in(&mut tx, &UserId::new(owner), clock).await.unwrap();
        tx.commit().await.unwrap();
    }
    let cutoff: chrono::DateTime<Utc> = sqlx::query_scalar("SELECT evidence_cutoff FROM analytics_projection_state WHERE singleton")
        .fetch_one(&*pool).await.unwrap();
    ...
    let copied: i64 = sqlx::query_scalar("SELECT count(*) FROM analytics_report_ai_requests")
        .fetch_one(&*pool).await.unwrap();
    assert_eq!(copied, 0, "older source rows cannot rebuild after either owner's compaction");
    let mut tx = pool.begin().await.unwrap();
    assert!(FeedbackSnapshotsRepository::compact_in(&mut tx, &UserId::new("second"), now - Duration::days(1) - Duration::seconds(1)).await.is_err());
    tx.rollback().await.unwrap();
    let mut tx = pool.begin().await.unwrap();
    sqlx::query("SELECT public.prepare_reporting_privacy()").execute(&mut *tx).await.unwrap();
    let error = sqlx::query("SELECT public.finish_reporting_compaction($1)")
        .bind(Utc::now() + Duration::days(1)).execute(&mut *tx).await.unwrap_err();
