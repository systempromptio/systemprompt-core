...
async fn test_admin_token_has_correct_claims() {
    ...
    let decoded = decode::<JwtClaims>(
        ...
        &DecodingKey::from_secret(SecretsBootstrap::jwt_secret().as_bytes()),
        &validation,
    )
    .expect("Admin token should be valid JWT");

    assert!(
        decoded.claims.scope.contains(&Permission::Admin),
        "Admin token should have Admin permission"
    );
    ...
    assert!(
        decoded.claims.aud.contains(&JwtAudience::A2a),
        "Admin token should have 'a2a' audience"
    );
    assert!(
        decoded.claims.aud.contains(&JwtAudience::Api),
        "Admin token should have 'api' audience"
    );
    assert!(
        decoded.claims.aud.contains(&JwtAudience::Mcp),
...
async fn test_no_empty_tokens_for_admin_operations() {
    ...
    assert!(
        claims.scope.contains(&Permission::Admin),
        "Admin token must have Admin permission"
    );

    println!("✅ No empty tokens - admin JWT is valid and has admin permission");
}
...
async fn test_admin_token_long_lived() {
    ...
    let minimum_days = one_year_seconds - 86400;
    assert!(
        expires_in > minimum_days,
