...
async fn test_comprehensive_a2a_agent_conversation_with_full_data_validation() {
    ...
    let task_row = ctx
        .db
        .fetch_optional(
            &"SELECT task_id, context_id, status, created_at FROM agent_tasks WHERE task_id = $1",
            &[&task_id],
        )
        .await
    ...
    let message_rows = ctx
        .db
        .fetch_all(
            &"SELECT message_id, task_id, role, sequence_number, created_at FROM agent_messages \
              WHERE task_id = $1 ORDER BY sequence_number",
            &[&task_id],
        )
        .await
