//! Harness tests for `cloud auth admin-user` routing.

use systemprompt_cli::cloud::auth::{AdminUserSyncArgs, AuthCommands};
use systemprompt_cli::cloud::{self, CloudCommands};

use super::{enter, json_ctx};

fn admin_user_cmd(args: AdminUserSyncArgs) -> CloudCommands {
    CloudCommands::Auth(AuthCommands::AdminUser(args))
}
...
async fn admin_user_database_url_requires_profile() {
    ...
    let err = cloud::execute(
        admin_user_cmd(AdminUserSyncArgs {
            verbose: false,
            profile: None,
