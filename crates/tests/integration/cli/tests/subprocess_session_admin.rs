...
fn isolated_project_or_skip() -> Option<(tempfile::TempDir, PathBuf)> {
...
fn session_cmd_or_skip(home: &Path, args: &[&str]) -> Option<assert_cmd::Command> {
...
fn login_creates_session_and_reuses_it() {
    ...
    let Some(mut cmd) = session_cmd_or_skip(home.path(), &["admin", "session", "login"]) else {
        return;
    };
    cmd.assert().success();

    let Some(mut again) = session_cmd_or_skip(home.path(), &["admin", "session", "login"]) else {
        return;
    };
...
fn login_token_only_and_force_new() {
    ...
    let Some(mut forced) = session_cmd_or_skip(
        home.path(),
        &[
            "admin",
            "session",
...
fn list_show_switch_after_login() {
    ...
    let Some(mut login) = session_cmd_or_skip(home.path(), &["admin", "session", "login"]) else {
        return;
    };
    ...
    for args in [
        ...
        let Some(mut cmd) = session_cmd_or_skip(home.path(), &args) else {
            return;
        };
        let _ = cmd.assert();
    }

    let Some(mut bad_switch) = session_cmd_or_skip(
        home.path(),
...
fn logout_single_and_all() {
    ...
    let Some(mut relogin) = session_cmd_or_skip(home.path(), &["admin", "session", "login"]) else {
        return;
    };
    relogin.assert().success();

    let Some(mut logout_all) =
        session_cmd_or_skip(home.path(), &["admin", "session", "logout", "--all", "-y"])
    else {
        return;
    };
    let _ = logout_all.assert();

    let Some(mut logout_named) = session_cmd_or_skip(
        home.path(),
