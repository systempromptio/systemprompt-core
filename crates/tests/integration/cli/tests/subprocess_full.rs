...
//!
//! `subprocess_with_db.rs` uses `--database-url` to bypass profile bootstrap
//! and only exercises the `_with_db` handler dispatchers. This file goes the
//! other way: it generates a profile.yaml on disk pointing at the per-track
//! Postgres instance and runs the binary with `--profile <path>`, so each
//! command goes through `resolve_and_display_profile -> init_secrets ->
//! init_paths -> run_validation -> dispatch_command`.
//!
...
static FULL: OnceLock<FullProfile> = OnceLock::new();

fn full_profile() -> &'static FullProfile {
    FULL.get_or_init(build_full_profile)
}

fn build_full_profile() -> FullProfile {
    ...
    for dir in [
        ...
        std::fs::create_dir_all(dir).expect("mkdir profile path");
    }
    ...
    for stub in [
        ...
        std::fs::write(services_path.join(stub), "{}\n").expect("write stub");
    }

    let profile_yaml = render_profile(
        &system_path,
        &services_path,
        &bin_path,
        &storage_path,
...
fn sp_full() -> Option<Command> {
    ...
    let profile = &full_profile().profile_path;
...
fn run(args: &[&str]) {
    let Some(mut cmd) = sp_full() else { return };
    cmd.args(args);
    let _ = cmd.assert();
}

fn run_with_formats(args: &[&str]) {
    run(args);
    let Some(mut cmd) = sp_full() else { return };
