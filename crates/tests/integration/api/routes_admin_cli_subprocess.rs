...
//!
//! Points the gateway at a throwaway shell script (via the
//! `router_with_binary` entry point) so the spawn, line-forward, exit-code, and
...
async fn subprocess_spawn_failure_yields_error_frame() -> anyhow::Result<()> {
    ...
    let app = router_with_binary(CliBinaryPath::new(missing.to_string_lossy()))
        .with_state((*ctx).clone())
        .layer(Extension(request_context("cli_sub")));
