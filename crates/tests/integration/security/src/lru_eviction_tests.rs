...
async fn least_recently_used_issuer_jwks_is_evicted_and_refetched() {
    ...
    let host_a = url::Url::parse(&mock_a.issuer())
        .unwrap()
        .host_str()
        .unwrap()
        .to_string();
    let host_b = url::Url::parse(&mock_b.issuer())
        .unwrap()
        .host_str()
        .unwrap()
        .to_string();
    let host_c = url::Url::parse(&mock_c.issuer())
        .unwrap()
        .host_str()
        .unwrap()
        .to_string();
