...
- `AiRequestTrace` (`sample`, `find_usage`, `list_usage`) with `TraceSampleFilter`, `TraceSampleMode`, `TraceSample`, `TraceMessage`, `TraceRequestUsage`, `TraceRequestStatus` and `DynAiRequestTrace`: the read seam over the AI request trace for domains that do not own it.
