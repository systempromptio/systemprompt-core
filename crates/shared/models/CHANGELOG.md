...
- `net::is_blocked_ip` exposes the SSRF block list for a caller that resolves a hostname itself and must check the DNS answer, rather than having it duplicated.
...
- Streamed `reasoning_content` on the openai_chat wire is emitted as canonical thinking. The buffered parse already mapped it; the streaming half dropped it, so every thinking model behind an OpenAI-compatible wire lost its trace mid-stream. Content-block indices are handed out in arrival order rather than pinning text at 0, and the reasoning block closes the moment answer text or a tool call begins.
- Gemini's implicit thinking path sends no `thinkingConfig` at all. Sending an implicit budget switched thinking *on* for models Google ships with it off: `gemini-2.5-flash-lite` answered a one-word prompt with 4,373 thought tokens. Google's per-model default is left alone and only `maxOutputTokens` is raised to text plus the model's thinking budget, clamped to the model cap, so default thinking cannot starve the caller's text budget.
- A reasoning model is detected from the catalog's `limits.max_thinking_budget` rather than a hardcoded `gpt-5`/`o1`/`o3`/`o4` prefix list, so a MaaS thinking model gets the output-token ceiling. The prefixes are kept for the families that reason without a catalog budget, and the Responses codec shares the fix.
- A stream that ends with no terminal event states an abort on every inbound surface. The caller previously saw only a closed socket, indistinguishable from a hang, on a turn the gateway had already finished.
