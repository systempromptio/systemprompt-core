...
//! pricing). [`pricing`] adds the one cost function to `ModelPricing`. All
//! types are re-exported here so consumers use `crate::services::ai::*`
...
pub use model::{
    AiProviderConfig, ModelCapabilities, ModelDefinition, ModelGovernance, ModelLimits,
    ModelPricing,
};
