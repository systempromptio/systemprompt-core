...
//! [`register_role_directory!`][crate::register_role_directory]; the ingestion service asks it which of the
//! roles a rule names are held by nobody, so an inert rule can be reported.
//! Without a registered directory every role mention is unverifiable and
//! none is reported — the same posture group and project dimensions have.
//!
...
/// One inventory submission per [`register_role_directory!`][crate::register_role_directory] call. The
