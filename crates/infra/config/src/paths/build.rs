//! Binary location for spawned agents and MCP servers.
//!
...
impl BuildPaths {
    ...
    // Why: spawning an agent re-execs this same binary with `admin agents run`,
    // so the binary to launch is whatever is running now — a renamed product
    // binary (`systemprompt-web`) must spawn itself, not a hardcoded name.
