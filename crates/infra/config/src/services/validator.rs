//! `ConfigValidator` — environment-specific quality checks for the
//! deployment `.env` files generated by [`super::ConfigManager`].
...
impl ConfigValidator {
    pub fn validate(config: &EnvironmentConfig) -> ConfigResult<ValidationReport> {
        ...
        if report.errors.is_empty() {
            ...
            Err(ConfigError::ValidationErrors {
    ...
    fn check_empty_values(config: &EnvironmentConfig, report: &mut ValidationReport) {
        let critical_vars = vec!["DATABASE_URL", "OAUTH_AT_REST_PEPPER"];
    ...
    fn check_port_values(config: &EnvironmentConfig, report: &mut ValidationReport) {
        if let Some(port_str) = config.variables.get("PORT") {
            if let Ok(port) = port_str.parse::<u16>() {
                if port == 0 {
                    CliService::error(&format!("Invalid port number: {port} (must be 1-65535)"));
        ...
        #[cfg(unix)]
        {
            ...
            if perms_octal == "644" || perms_octal == "600" {
                ...
                CliService::warning(&format!(
                    ".env file permissions may expose secrets: {perms_octal}"
                ));
